...
class MockToolInvocation extends BaseToolInvocation {
    ...
    execute(options) {
        ...
        return this.tool.execute(this.params, signal, updateOutput, options);
    }
...
export class MockTool extends BaseDeclarativeTool {
    ...
    execute;
    constructor(options) {
        super(options.name, options.displayName ?? options.name, options.description ?? options.name, options.kind ?? Kind.Other, options.params, options.messageBus ?? createMockMessageBus(), options.isOutputMarkdown ?? false, options.canUpdateOutput ?? false);
