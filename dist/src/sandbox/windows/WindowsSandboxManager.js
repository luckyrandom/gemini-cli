...
import { GOVERNANCE_FILES, findSecretFiles, resolveSandboxPaths, } from '../../services/sandboxManager.js';
import { sanitizeEnvironment, getSecureSanitizationConfig, } from '../../services/environmentSanitization.js';
import { debugLogger } from '../../utils/debugLogger.js';
import { spawnAsync, getCommandName, initializeShellParsers, getCommandRoots, stripShellWrapper, } from '../../utils/shell-utils.js';
...
export class WindowsSandboxManager {
                ...
                if (fs.existsSync(sourcePath)) {
                    ...
                    for (const csc of cscPaths) {
                        try {
                            ...
                            debugLogger.log(`WindowsSandboxManager: Failed to compile using ${csc}: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                    if (!compiled) {
    ...
    async prepareCommand(req) {
        ...
        const allowedManifest = new Set();
        const inheritanceRoots = new Set();
        ...
        for (const allowedPath of resolvedPaths.policyAllowed) {
            try {
                ...
                throw new Error(`Sandbox request rejected: Allowed path does not exist: ${allowedPath}. ` +
        ...
        for (const writePath of resolvedPaths.policyWrite) {
            try {
                ...
                if (!isInherited) {
                    throw new Error(`Sandbox request rejected: Additional write path does not exist and its parent directory is not allowed: ${writePath}. ` +
                        'On Windows, granular sandbox access can only be granted to existing paths to avoid broad parent directory permissions.');
                }
            }
        }
        ...
        const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'gemini-cli-sandbox-'));
        const forbiddenManifestPath = path.join(tempDir, 'forbidden.txt');
        await fs.promises.writeFile(forbiddenManifestPath, Array.from(forbiddenManifest).join('\n'));
        const allowedManifestPath = path.join(tempDir, 'allowed.txt');
        await fs.promises.writeFile(allowedManifestPath, Array.from(allowedManifest).join('\n'));
