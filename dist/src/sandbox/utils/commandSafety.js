 ...
 */
function isSafeToCallWithExec(args) {
    if (!args || args.length === 0)
        return false;
    const cmd = args[0];
    ...
    if (cmd === 'base64') {
        ...
        return !args
            .slice(1)
            .some((arg) => unsafeOptions.has(arg) ||
 ...
 */
function findGitSubcommand(args, subcommands) {
    let skipNext = false;
 ...
 */
function gitHasConfigOverrideGlobalOption(args) {
 ...
 */
function gitSubcommandArgsAreReadOnly(args) {
    ...
    return !args.some((arg) => unsafeFlags.has(arg) ||
        arg.startsWith('--output=') ||
        arg.startsWith('--exec='));
}
/**
 ...
 */
function gitBranchIsReadOnly(args) {
    if (args.length === 0)
        return true;
