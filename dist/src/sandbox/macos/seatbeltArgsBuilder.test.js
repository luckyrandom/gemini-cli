    ...
    describe('buildSeatbeltProfile', () => {
        ...
        describe('git worktree paths', () => {
            it('enforces read-only binding for git worktrees even if workspaceWrite is true', () => {
                ...
                expect(profile).toContain(`(allow file-read* (subpath "${worktreeGitDir}"))`);
                expect(profile).toContain(`(allow file-read* (subpath "${mainGitDir}"))`);
                // Should NOT grant write access
                expect(profile).not.toContain(`(allow file-read* file-write* (subpath "${worktreeGitDir}"))`);
                expect(profile).not.toContain(`(allow file-read* file-write* (subpath "${mainGitDir}"))`);
            });
...
//# sourceMappingURL=seatbeltArgsBuilder.test.js.map