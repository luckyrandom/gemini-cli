...
function isPathExplicitlyAllowed(filePath, realFilePath, policyWrite) {
    return policyWrite.some((p) => p === filePath ||
        p === realFilePath ||
...
export function buildSeatbeltProfile(options) {
    ...
    const nodeRootPath = resolveToRealPath(path.dirname(path.dirname(process.execPath)));
    ...
    for (let i = 0; i < GOVERNANCE_FILES.length; i++) {
        const governanceFile = path.join(resolvedPaths.workspace.resolved, GOVERNANCE_FILES[i].path);
        ...
        profile += denyUnlessExplicitlyAllowed(governanceFile, isDirectory ? 'subpath' : 'literal', resolvedPaths.policyWrite, workspaceWrite && GOVERNANCE_FILES[i].path !== '.git');
    }
