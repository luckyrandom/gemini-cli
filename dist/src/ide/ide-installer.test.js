...
describe('AntigravityInstaller', () => {
    ...
    it('falls back to antigravity when agy is unavailable on linux', async () => {
        ...
        expect(child_process.execSync).toHaveBeenNthCalledWith(2, 'command -v antigravity', { stdio: 'ignore' });
        expect(child_process.spawnSync).toHaveBeenCalledWith('antigravity', [
            '--install-extension',
            'google.gemini-cli-vscode-ide-companion',
            '--force',
        ], { stdio: 'pipe', shell: false });
    ...
    it('falls back to antigravity.cmd when agy.cmd is unavailable on windows', async () => {
        ...
        vi.mocked(child_process.execSync)
            ...
            .mockImplementationOnce(() => 'C:\\Program Files\\Antigravity\\bin\\antigravity.cmd');
        const result = await installer.install();
        expect(result.success).toBe(true);
        expect(child_process.execSync).toHaveBeenNthCalledWith(1, 'where.exe agy.cmd');
        expect(child_process.execSync).toHaveBeenNthCalledWith(2, 'where.exe antigravity.cmd');
        expect(child_process.spawnSync).toHaveBeenCalledWith('C:\\Program Files\\Antigravity\\bin\\antigravity.cmd', [
            '--install-extension',
            'google.gemini-cli-vscode-ide-companion',
            '--force',
        ], { stdio: 'pipe', shell: true });
