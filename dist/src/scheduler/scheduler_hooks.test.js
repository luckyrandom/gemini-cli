...
function createMockConfig(overrides = {}) {
    ...
    const mockConfig = Object.assign({}, baseConfig, overrides);
...
describe('Scheduler Hooks', () => {
    it('should stop execution if BeforeTool hook requests stop', async () => {
        ...
        mockConfig.getHookSystem = () => hookSystem;
        ...
        const erroredCall = result;
        expect(erroredCall.response.error?.message).toContain('Agent execution stopped by hook: Hook stopped execution');
    ...
    it('should block tool execution if BeforeTool hook requests block', async () => {
        ...
        expect(result.status).toBe(CoreToolCallStatus.Error);
        const erroredCall = result;
        expect(erroredCall.response.error?.message).toContain('Tool execution blocked: Hook blocked execution');
        expect(executeFn).not.toHaveBeenCalled();
    });
    it('should update tool input if BeforeTool hook provides modified input', async () => {
        ...
        expect(executeFn).toHaveBeenCalledWith({ newParam: 'modifiedValue' }, expect.anything(), undefined, expect.anything());
        expect(result.request.args).toEqual({
            newParam: 'modifiedValue',
        });
    });
});
//# sourceMappingURL=scheduler_hooks.test.js.map