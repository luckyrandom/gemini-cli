    ...
    async execute(context) {
        ...
        return runInDevTraceSpan({
            ...
            try {
                ...
                const promise = executeToolWithHooks(invocation, toolName, signal, tool, liveOutputCallback, { shellExecutionConfig, setExecutionIdCallback }, this.config, request.originalRequestName, true);
                ...
                if (signal.aborted || abortedByError) {
                    completedToolCall = await this.createCancelledResult(call, isAbortError(executionError)
                        ? 'Operation cancelled.'
        ...
        if (typeof content === 'string' && toolName === SHELL_TOOL_NAME) {
            ...
            if (typeof firstPart === 'object' && typeof firstPart.text === 'string') {
                ...
                if (threshold > 0 && textContent.length > threshold) {
                    ...
                    const { outputFile: savedPath } = await saveTruncatedToolOutput(textContent, toolName, callId, this.config.storage.getProjectTempDir(), this.context.promptId);
                    outputFile = savedPath;
                    const truncatedText = formatTruncatedToolOutput(textContent, outputFile, threshold);
