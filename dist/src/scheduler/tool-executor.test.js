...
describe('ToolExecutor', () => {
    ...
    it('should return cancelled result and use originalRequestName when signal is aborted', async () => {
        ...
        expect(result.status).toBe(CoreToolCallStatus.Cancelled);
        if (result.status === CoreToolCallStatus.Cancelled) {
            expect(result.response.responseParts[0]?.functionResponse?.name).toBe('originalToolName');
        }
    });
    ...
    it('should truncate large MCP tool output with single text Part', async () => {
        ...
        const mcpTool = new DiscoveredMCPTool({}, 'my-server', 'get_big_text', 'A test MCP tool', {}, messageBus);
        ...
        // 4. Verify Truncation Logic
        expect(fileUtils.saveTruncatedToolOutput).toHaveBeenCalledWith(longText, mcpToolName, 'call-mcp-trunc', expect.any(String), 'test-session-id');
        expect(fileUtils.formatTruncatedToolOutput).toHaveBeenCalledWith(longText, '/tmp/truncated_output.txt', 10);
        expect(result.status).toBe(CoreToolCallStatus.Success);
        if (result.status === CoreToolCallStatus.Success) {
            expect(result.response.outputFile).toBe('/tmp/truncated_output.txt');
        }
    ...
    it('should report execution ID updates for backgroundable tools', async () => {
        ...
        const testPid = 12345;
        vi.mocked(coreToolHookTriggers.executeToolWithHooks).mockImplementation(async (_inv, _name, _sig, _tool, _liveCb, options, _config, _originalRequestName) => {
    ...
    it('should report execution ID updates for non-shell backgroundable tools', async () => {
        ...
        vi.mocked(coreToolHookTriggers.executeToolWithHooks).mockImplementation(async (_inv, _name, _sig, _tool, _liveCb, options) => {
    ...
    it('should truncate large shell output even on cancellation', async () => {
        ...
        expect(fileUtils.saveTruncatedToolOutput).toHaveBeenCalledWith(longOutput, SHELL_TOOL_NAME, 'call-trunc-cancel', expect.any(String), 'test-session-id');
        expect(result.status).toBe(CoreToolCallStatus.Cancelled);
        if (result.status === CoreToolCallStatus.Cancelled) {
            const response = result.response.responseParts[0]?.functionResponse
                ?.response;
