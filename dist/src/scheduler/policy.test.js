...
describe('policy.ts', () => {
    describe('checkPolicy', () => {
        it('should return the decision from the policy engine', async () => {
            ...
            expect(mockPolicyEngine.check).toHaveBeenCalledWith({ name: 'test-tool', args: {} }, undefined, undefined, undefined);
        });
        it('should pass serverName and toolAnnotations for MCP tools', async () => {
            ...
            expect(mockPolicyEngine.check).toHaveBeenCalledWith({ name: 'mcp-tool', args: {} }, 'my-server', { readOnlyHint: true }, undefined);
        });
    ...
    describe('updatePolicy', () => {
        it('should set AUTO_EDIT mode for auto-edit transition tools', async () => {
            ...
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlways, undefined, mockConfig, mockMessageBus);
        ...
        it('should handle standard policy updates with persistence', async () => {
            ...
            mockConfig.messageBus =
                mockMessageBus;
            const tool = { name: 'test-tool' };
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlwaysAndSave, undefined, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageBusType.UPDATE_POLICY,
                toolName: 'test-tool',
                persist: true,
        ...
        it('should handle shell command prefixes', async () => {
            ...
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlways, details, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageBusType.UPDATE_POLICY,
        ...
        it('should handle MCP policy updates (server scope)', async () => {
            ...
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlwaysServer, details, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageBusType.UPDATE_POLICY,
        ...
        it('should NOT publish update for ProceedOnce', async () => {
            ...
            mockConfig.messageBus =
                mockMessageBus;
            const tool = { name: 'test-tool' };
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedOnce, undefined, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).not.toHaveBeenCalled();
        ...
        it('should NOT publish update for Cancel', async () => {
            ...
            mockConfig.messageBus =
                mockMessageBus;
            const tool = { name: 'test-tool' };
            await updatePolicy(tool, ToolConfirmationOutcome.Cancel, undefined, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).not.toHaveBeenCalled();
        });
        it('should NOT publish update for ModifyWithEditor', async () => {
            ...
            mockConfig.messageBus =
                mockMessageBus;
            const tool = { name: 'test-tool' };
            await updatePolicy(tool, ToolConfirmationOutcome.ModifyWithEditor, undefined, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).not.toHaveBeenCalled();
        });
        it('should handle MCP ProceedAlwaysTool (specific tool name)', async () => {
            ...
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlwaysTool, details, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageBusType.UPDATE_POLICY,
        ...
        it('should handle MCP ProceedAlwaysAndSave (persist: true)', async () => {
            ...
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlwaysAndSave, details, mockConfig, mockMessageBus);
            expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageBusType.UPDATE_POLICY,
                toolName: 'mcp-tool',
                mcpName: 'my-server',
        ...
        it('should work when context is created via Object.create (prototype chain)', async () => {
            ...
            const protoContext = Object.create(baseContext);
            ...
            const tool = { name: 'test-tool' };
            await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlways, undefined, protoContext, mockMessageBus);
            expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageBusType.UPDATE_POLICY,
                toolName: 'test-tool',
                persist: false,
    ...
    describe('updatePolicy - context-aware modes', () => {
        ...
        testCases.forEach(({ currentMode, expectedModes, description }) => {
            it(`should ${description}`, async () => {
                ...
                const tool = { name: 'test-tool' };
                await updatePolicy(tool, ToolConfirmationOutcome.ProceedAlwaysAndSave, undefined, context, mockMessageBus);
                expect(mockMessageBus.publish).toHaveBeenCalledWith(expect.objectContaining({
                    type: MessageBusType.UPDATE_POLICY,
                    toolName: 'test-tool',
                    persist: true,
