...
import { ApprovalMode, MODES_BY_PERMISSIVENESS, PolicyDecision, } from '../policy/types.js';
 ...
 */
export async function checkPolicy(toolCall, config, subagent) {
    ...
    const result = await config
        .getPolicyEngine()
        .check({ name: toolCall.request.name, args: toolCall.request.args }, serverName, toolAnnotations, subagent);
 ...
 */
export async function updatePolicy(tool, outcome, confirmationDetails, context, messageBus, toolInvocation) {
    ...
    await handleStandardPolicyUpdate(tool, outcome, confirmationDetails, messageBus, persistScope, toolInvocation, context.config, modes);
}
/**
 ...
 */
async function handleStandardPolicyUpdate(tool, outcome, confirmationDetails, messageBus, persistScope, toolInvocation, config, modes) {
    if (outcome === ToolConfirmationOutcome.ProceedAlways ||
        outcome === ToolConfirmationOutcome.ProceedAlwaysAndSave) {
