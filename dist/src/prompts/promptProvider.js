...
import { WRITE_TODOS_TOOL_NAME, READ_FILE_TOOL_NAME, ENTER_PLAN_MODE_TOOL_NAME, GLOB_TOOL_NAME, GREP_TOOL_NAME, AGENT_TOOL_NAME, } from '../tools/tool-names.js';
...
export class PromptProvider {
     ...
     */
    getCoreSystemPrompt(context, userMemory, interactiveOverride, topicUpdateNarrationOverride) {
        ...
        const isTopicUpdateNarrationEnabled = topicUpdateNarrationOverride ??
            context.config.isTopicUpdateNarrationEnabled();
        const enabledToolNames = new Set(toolNames);
        const approvedPlanPath = context.config.getApprovedPlanPath();
        const desiredModel = resolveModel(context.config.getActiveModel(), context.config.getGemini31LaunchedSync?.() ?? false, context.config.getGemini31FlashLiteLaunchedSync?.() ?? false, false, context.config.getHasAccessToPreviewModel?.() ?? true, context.config);
        ...
        if (systemMdResolution.value && !systemMdResolution.isDisabled) {
            ...
            const options = {
                ...
                subAgents: this.withSection('agentContexts', () => context.config
                    .getAgentRegistry()
                    .getAllDefinitions()
                    .map((d) => ({
                    name: d.name,
                ...
                primaryWorkflows: this.withSection('primaryWorkflows', () => {
                    ...
                    return {
                        interactive: interactiveMode,
                        enableCodebaseInvestigator: agentRegistry.getDefinition(CodebaseInvestigatorAgent.name) !==
                            undefined,
    ...
    getCompressionPrompt(context) {
        const desiredModel = resolveModel(context.config.getActiveModel(), context.config.getGemini31LaunchedSync?.() ?? false, context.config.getGemini31FlashLiteLaunchedSync?.() ?? false, false, context.config.getHasAccessToPreviewModel?.() ?? true, context.config);
        const isModernModel = supportsModernFeatures(desiredModel);
        const activeSnippets = isModernModel ? snippets : legacySnippets;
        return activeSnippets.getCompressionPrompt(context.config.getApprovedPlanPath());
    }
