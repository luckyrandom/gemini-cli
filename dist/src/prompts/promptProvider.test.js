...
import { getAllGeminiMdFilenames, DEFAULT_CONTEXT_FILENAME, } from '../tools/memoryTool.js';
...
describe('PromptProvider', () => {
    ...
    it('should handle multiple context filenames in the system prompt', () => {
        ...
        expect(prompt).toContain(`Instructions found in \`${DEFAULT_CONTEXT_FILENAME}\`, \`CUSTOM.md\` or \`ANOTHER.md\` files are foundational mandates.`);
    });
    it('should include the task tracker storage location in the system prompt', () => {
        ...
        vi.mocked(mockConfig.storage.getProjectTempTrackerDir).mockReturnValue(mockTrackerDir);
        const provider = new PromptProvider();
        const prompt = provider.getCoreSystemPrompt(mockConfig);
    ...
    it('should handle multiple context filenames in user memory section', () => {
        ...
        expect(prompt).toContain(`# Contextual Instructions (${DEFAULT_CONTEXT_FILENAME}, CUSTOM.md)`);
    });
    describe('plan mode prompt', () => {
        ...
        it('should list all active tools from ToolRegistry in plan mode prompt', () => {
            ...
            mockConfig.getToolRegistry.mockReturnValue({
        ...
        it('should show server name for MCP tools in plan mode prompt', () => {
            const mcpTool = new DiscoveredMCPTool({}, 'my-mcp-server', 'mcp_read', 'An MCP read tool', {}, mockMessageBus, undefined, true);
            const mockTools = [
                new MockTool({ name: 'glob', displayName: 'Glob' }),
                mcpTool,
            ];
        ...
        it('should include write constraint message in plan mode prompt', () => {
            ...
            const provider = new PromptProvider();
            const prompt = provider.getCoreSystemPrompt(mockConfig);
            expect(prompt).toContain('`write_file` and `replace` may ONLY be used to write .md plan files');
    ...
    describe('getCompressionPrompt', () => {
        it('should include plan preservation instructions when an approved plan path is provided', () => {
            ...
            mockConfig.getApprovedPlanPath.mockReturnValue(planPath);
            const provider = new PromptProvider();
        ...
        it('should NOT include plan preservation instructions when no approved plan path is provided', () => {
            mockConfig.getApprovedPlanPath.mockReturnValue(undefined);
            const provider = new PromptProvider();
            const prompt = provider.getCompressionPrompt(mockConfig);
    ...
    describe('topicUpdateNarrationOverride', () => {
        ...
        beforeEach(() => {
            ...
            mockConfig.getAgentRegistry.mockReturnValue({
                getAllDefinitions: vi.fn().mockReturnValue([]),
                getDefinition: vi.fn().mockReturnValue(undefined),
            });
        });
        it('should disable topic update narration when override is false, even if config is true', () => {
            ...
            expect(prompt).not.toContain(`As you work, the user follows along by reading topic updates that you publish with ${UPDATE_TOPIC_TOOL_NAME}.`);
        });
        it('should enable topic update narration when override is true, even if config is false', () => {
            ...
            expect(prompt).toContain(`As you work, the user follows along by reading topic updates that you publish with ${UPDATE_TOPIC_TOOL_NAME}.`);
        });
    });
...
//# sourceMappingURL=promptProvider.test.js.map