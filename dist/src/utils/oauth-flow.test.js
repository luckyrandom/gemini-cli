...
import { generatePKCEParams, getPortFromUrl, buildAuthorizationUrl, startCallbackServer, exchangeCodeForToken, refreshAccessToken, REDIRECT_PATH, } from './oauth-flow.js';
...
describe('oauth-flow', () => {
    ...
    describe('buildAuthorizationUrl', () => {
        ...
        it('should use the default redirect URI based on port', () => {
            const url = buildAuthorizationUrl(baseConfig, basePkceParams, 3000);
            const parsed = new URL(url);
            expect(parsed.searchParams.get('redirect_uri')).toBe(`http://localhost:3000${REDIRECT_PATH}`);
        });
        ...
        it('should include resource parameter when provided', () => {
            const url = buildAuthorizationUrl(baseConfig, basePkceParams, 3000, 'https://mcp.example.com');
            const parsed = new URL(url);
    ...
    describe('startCallbackServer', () => {
        it('should start a server and resolve port', async () => {
            ...
            const res = await realFetch(`http://localhost:${port}${REDIRECT_PATH}?code=abc&state=test-state`);
        ...
        it('should resolve response with code and state on valid callback', async () => {
            ...
            const port = await server.port;
            await realFetch(`http://localhost:${port}${REDIRECT_PATH}?code=auth-code-123&state=my-state`);
        ...
        it('should reject on state mismatch', async () => {
            ...
            const responseResult = server.response.then(() => new Error('Expected rejection'), (e) => e);
            await realFetch(`http://localhost:${port}${REDIRECT_PATH}?code=abc&state=wrong-state`).catch(() => {
        ...
        it('should reject on OAuth error in callback', async () => {
            ...
            const responseResult = server.response.then(() => new Error('Expected rejection'), (e) => e);
            await realFetch(`http://localhost:${port}${REDIRECT_PATH}?error=access_denied&error_description=User+denied`).catch(() => {
                // Connection may be reset by server closing — expected
            });
            const error = await responseResult;
        ...
        it('should return 404 for non-callback paths', async () => {
            ...
            await realFetch(`http://localhost:${port}${REDIRECT_PATH}?code=abc&state=test-state`);
            await server.response;
        });
        it('should reject when OAUTH_CALLBACK_PORT env var is invalid', async () => {
            vi.stubEnv('OAUTH_CALLBACK_PORT', 'not-a-number');
            const server = startCallbackServer('test-state');
            await expect(server.port).rejects.toThrow('Invalid value for OAUTH_CALLBACK_PORT');
            await expect(server.response).rejects.toThrow('Invalid value for OAUTH_CALLBACK_PORT');
        });
        it('should settle on timeout without keeping the process alive', async () => {
            ...
            try {
                ...
                await server.port;
                const responsePromise = server.response.catch((e) => {
                    if (e.message !== 'OAuth callback timeout')
                        throw e;
                    return e;
                });
                ...
                expect(error.message).toBe('OAuth callback timeout');
            }
            finally {
                vi.useRealTimers();
            }
    ...
    describe('exchangeCodeForToken', () => {
        it('should exchange code for token with JSON response', async () => {
            ...
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify(tokenResponse)));
            const result = await exchangeCodeForToken(baseConfig, 'auth-code', 'code-verifier', 3000);
        ...
        it('should send correct parameters in the request body', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ access_token: 'tok', token_type: 'Bearer' })));
        ...
        it('should include resource parameter when provided', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ access_token: 'tok', token_type: 'Bearer' })));
            await exchangeCodeForToken(baseConfig, 'code', 'verifier', 3000, 'https://mcp.example.com');
            const body = new URLSearchParams(mockFetch.mock.calls[0][1].body);
            expect(body.get('resource')).toBe('https://mcp.example.com');
        });
        it('should handle form-urlencoded token response', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse('access_token=form-token&token_type=Bearer&expires_in=7200', { contentType: 'application/x-www-form-urlencoded' }));
        ...
        it('should throw on non-ok response', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse('Bad request', { status: 400 }));
            await expect(exchangeCodeForToken(baseConfig, 'code', 'verifier', 3000)).rejects.toThrow('Token exchange failed');
        });
        it('should throw on non-ok response with form-urlencoded error', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse('error=invalid_grant&error_description=Code+expired', {
                status: 400,
                contentType: 'application/x-www-form-urlencoded',
            }));
            await expect(exchangeCodeForToken(baseConfig, 'code', 'verifier', 3000)).rejects.toThrow('invalid_grant');
        });
        it('should throw when JSON response has no access_token and form-urlencoded fallback also fails', async () => {
            ...
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ error: 'server_error' })));
            await expect(exchangeCodeForToken(baseConfig, 'code', 'verifier', 3000)).rejects.toThrow('Token exchange failed');
        });
        ...
        it('should default token_type to Bearer when missing from JSON response', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ access_token: 'tok' })));
            const result = await exchangeCodeForToken(baseConfig, 'code', 'verifier', 3000);
            expect(result.token_type).toBe('Bearer');
        });
    });
    describe('refreshAccessToken', () => {
        ...
        it('should refresh a token with JSON response', async () => {
            ...
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify(tokenResponse)));
            const result = await refreshAccessToken(refreshConfig, 'old-refresh-token', 'https://auth.example.com/token');
            expect(result.access_token).toBe('new-access-token');
            expect(result.expires_in).toBe(3600);
        });
        it('should send correct parameters in the request body', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ access_token: 'tok', token_type: 'Bearer' })));
            await refreshAccessToken(refreshConfig, 'my-refresh-token', 'https://auth.example.com/token');
            const body = new URLSearchParams(mockFetch.mock.calls[0][1].body);
        ...
        it('should include client_secret when provided', async () => {
            ...
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ access_token: 'tok', token_type: 'Bearer' })));
            await refreshAccessToken(config, 'refresh-token', 'https://auth.example.com/token');
            const body = new URLSearchParams(mockFetch.mock.calls[0][1].body);
        ...
        it('should include resource parameter when provided', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse(JSON.stringify({ access_token: 'tok', token_type: 'Bearer' })));
            await refreshAccessToken(refreshConfig, 'refresh-token', 'https://auth.example.com/token', 'https://mcp.example.com');
            const body = new URLSearchParams(mockFetch.mock.calls[0][1].body);
            expect(body.get('resource')).toBe('https://mcp.example.com');
        });
        it('should throw on non-ok response', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse('Unauthorized', { status: 401 }));
            await expect(refreshAccessToken(refreshConfig, 'bad-token', 'https://auth.example.com/token')).rejects.toThrow('Token refresh failed');
        });
        it('should handle form-urlencoded token response', async () => {
            mockFetch.mockResolvedValueOnce(createMockResponse('access_token=refreshed-token&token_type=Bearer&expires_in=1800', { contentType: 'application/x-www-form-urlencoded' }));
            const result = await refreshAccessToken(refreshConfig, 'refresh-token', 'https://auth.example.com/token');
