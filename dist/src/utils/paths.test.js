...
import { escapePath, unescapePath, isSubpath, shortenPath, normalizePath, resolveToRealPath, makeRelative, deduplicateAbsolutePaths, toPathKey, } from './paths.js';
...
describe('resolveToRealPath', () => {
    ...
    it('should prevent infinite recursion on malicious symlink structures', () => {
        ...
        vi.spyOn(fs, 'lstatSync').mockImplementation(() => ({ isSymbolicLink: () => true }));
        vi.spyOn(fs, 'readlinkSync').mockImplementation(() => ['..', 'malicious', 'symlink'].join(path.sep));
        expect(() => resolveToRealPath(maliciousPath)).toThrow(/Infinite recursion detected/);
    });
});
...
describe('normalizePath', () => {
    ...
    describe.skipIf(process.platform === 'win32' || process.platform === 'darwin')('on Linux', () => {
    ...
    describe('deduplicateAbsolutePaths', () => {
        ...
        it('should throw an error if a path is not absolute', () => {
            ...
            expect(() => deduplicateAbsolutePaths(paths)).toThrow('Path must be absolute: relative/path');
        });
    });
    describe('toPathKey', () => {
