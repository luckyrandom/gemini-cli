...
import { isRenderedInHistory, belongsInConfirmationQueue, isVisibleInToolGroup, } from './tool-visibility.js';
import { CoreToolCallStatus } from '../scheduler/types.js';
import { ApprovalMode } from '../policy/types.js';
import { ASK_USER_DISPLAY_NAME, WRITE_FILE_DISPLAY_NAME, EDIT_DISPLAY_NAME, UPDATE_TOPIC_TOOL_NAME, READ_FILE_DISPLAY_NAME, } from '../tools/tool-names.js';
describe('ToolVisibility Rules', () => {
    ...
    describe('isRenderedInHistory', () => {
        it('hides tools with parents', () => {
            expect(isRenderedInHistory(createCtx({ parentCallId: 'parent-123' }))).toBe(false);
        });
        it('hides AskUser errors without results', () => {
            expect(isRenderedInHistory(createCtx({
    ...
    describe('belongsInConfirmationQueue', () => {
        it('returns false for update_topic', () => {
            expect(belongsInConfirmationQueue(createCtx({ name: UPDATE_TOPIC_TOOL_NAME }))).toBe(false);
        });
    ...
    describe('isVisibleInToolGroup', () => {
        it('shows tools with parents (agent tools)', () => {
            expect(isVisibleInToolGroup(createCtx({ parentCallId: 'parent-123' }), 'full')).toBe(true);
        });
        it('hides WriteFile/Edit in Plan Mode', () => {
            expect(isVisibleInToolGroup(createCtx({
                displayName: WRITE_FILE_DISPLAY_NAME,
                approvalMode: ApprovalMode.PLAN,
            }), 'full')).toBe(false);
        });
        ...
        it('hides confirming tools', () => {
            expect(isVisibleInToolGroup(createCtx({ status: CoreToolCallStatus.AwaitingApproval }), 'full')).toBe(false);
        });
...
//# sourceMappingURL=tool-visibility.test.js.map