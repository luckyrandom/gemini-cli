 ...
 */
export async function getCommandName(command, args) {
 ...
 */
export function extractStringFromParseEntry(entry) {
    ...
    if ('op' in entry)
        return entry.op;
    if ('comment' in entry)
        return ''; // We can typically ignore comments for safety checks
    return '';
}
...
export function parseBashCommandDetails(command) {
    if (treeSitterInitializationError) {
 ...
 */
export const spawnAsync = async (command, args, options) => {
