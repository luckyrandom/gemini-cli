...
import { DEFAULT_MAX_LINES_TEXT_FILE, MAX_LINE_LENGTH_TEXT_FILE, MAX_FILE_SIZE_MB, } from './constants.js';
...
const SUPPORTED_AUDIO_MIME_TYPES_BY_EXTENSION = new Map([
    ['.mp3', 'audio/mpeg'],
    ['.wav', 'audio/wav'],
    ['.aiff', 'audio/aiff'],
    ['.aif', 'audio/aiff'],
...
const AUDIO_MIME_TYPE_NORMALIZATION = {
...
function formatSupportedAudioFormats() {
    const displayNames = Array.from(new Set(Array.from(SUPPORTED_AUDIO_MIME_TYPES_BY_EXTENSION.keys()).map((ext) => {
        if (ext === '.aif' || ext === '.aiff') {
            return 'AIFF';
        }
