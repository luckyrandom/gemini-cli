...
import { ASK_USER_DISPLAY_NAME, WRITE_FILE_DISPLAY_NAME, EDIT_DISPLAY_NAME, UPDATE_TOPIC_TOOL_NAME, UPDATE_TOPIC_DISPLAY_NAME, } from '../tools/tool-names.js';
/**
 * Maps a core ToolCall to a ToolVisibilityContext.
 */
export function buildToolVisibilityContext(tc) {
    let hasResult = false;
 ...
 */
export function isRenderedInHistory(ctx) {
    if (ctx.parentCallId) {
        return false;
    }
 ...
 */
export function belongsInConfirmationQueue(ctx) {
    const displayName = ctx.displayName ?? ctx.name;
    // Narrative background tools auto-execute and never require confirmation
    if (ctx.name === UPDATE_TOPIC_TOOL_NAME ||
 ...
 */
export function isVisibleInToolGroup(ctx, errorVerbosity) {
    const displayName = ctx.displayName ?? ctx.name;
    ...
    // In Plan Mode, edits are redundant because the plan shows the diffs.
    if ((displayName === WRITE_FILE_DISPLAY_NAME ||
