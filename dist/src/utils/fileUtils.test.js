...
describe('fileUtils', () => {
    ...
    describe('detectFileType', () => {
        ...
        it('should detect supported audio files by extension when mime lookup is missing', async () => {
            ...
            actualNodeFs.writeFileSync(filePath, Buffer.from([0x66, 0x4c, 0x61, 0x43, 0x00, 0x00, 0x00, 0x22]));
  ...
  <InterfaceList>
    ...
    describe('processSingleFileContent', () => {
        ...
        it('should normalize supported audio mime types before returning inline data', async () => {
            ...
            const result = await processSingleFileContent(wavFilePath, tempRootDir, new StandardFileSystemService());
            expect(result.llmContent.inlineData
                .mimeType).toBe('audio/wav');
        });
        it('should reject unsupported audio mime types with a clear error', async () => {
            ...
            actualNodeFs.writeFileSync(unsupportedAudioPath, Buffer.from([0x00, 0x01, 0x02, 0x03]));
            mockMimeGetType.mockReturnValue('audio/adpcm');
            const result = await processSingleFileContent(unsupportedAudioPath, tempRootDir, new StandardFileSystemService());
        ...
        it('should process a video file', async () => {
            ...
            const result = await processSingleFileContent(testVideoFilePath, tempRootDir, new StandardFileSystemService());
            expect(result.llmContent.inlineData).toBeDefined();
            expect(result.llmContent.inlineData
                .mimeType).toBe('video/mp4');
            expect(result.llmContent.inlineData.data).toBe(fakeMp4Data.toString('base64'));
