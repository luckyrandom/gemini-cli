...
--- End of Context from: ${includedFile} ---`,
    ...
    describe('getEnvironmentMemoryPaths', () => {
        ...
        it('should recognize .git as a file (submodules/worktrees)', async () => {
            ...
            await createTestFile(path.join(repoDir, '.git'), 'gitdir: /some/other/path/.git/worktrees/worktree_repo');
            const srcDir = await createEmptyDir(path.join(repoDir, 'src'));
            const repoFile = await createTestFile(path.join(repoDir, DEFAULT_CONTEXT_FILENAME), 'Repo content');
            const srcFile = await createTestFile(path.join(srcDir, DEFAULT_CONTEXT_FILENAME), 'Src content');
    ...
    describe('case-insensitive filesystem deduplication', () => {
        it('should deduplicate files that point to the same inode (same physical file)', async () => {
            ...
            try {
                ...
                if (errorMessage.includes('cross-device') ||
        ...
        it('should handle case where files have different inodes (different files)', async () => {
            const geminiFileLower = await createTestFile(path.join(projectRoot, 'gemini.md'), 'Lowercase file content');
            const geminiFileUpper = await createTestFile(path.join(projectRoot, 'GEMINI.md'), 'Uppercase file content');
    ...
    describe('loadJitSubdirectoryMemory', () => {
        ...
        it('should deduplicate files in JIT memory loading (same inode)', async () => {
            ...
            const subDir = await createEmptyDir(path.join(rootDir, 'subdir'));
            const targetFile = path.join(subDir, 'target.txt');
            const geminiFile = await createTestFile(path.join(subDir, 'gemini.md'), 'JIT memory content');
        ...
        it('should resolve file target to its parent directory for traversal', async () => {
            ...
            const targetFile = await createTestFile(path.join(subDir, 'app.ts'), 'const x = 1;');
            const subDirMemory = await createTestFile(path.join(subDir, DEFAULT_CONTEXT_FILENAME), 'Src context rules');
            const result = await loadJitSubdirectoryMemory(targetFile, [rootDir], new Set());
        ...
        it('should handle non-existent file target by using parent directory', async () => {
            ...
            const subDirMemory = await createTestFile(path.join(subDir, DEFAULT_CONTEXT_FILENAME), 'Rules for new files');
            const result = await loadJitSubdirectoryMemory(targetFile, [rootDir], new Set());
            expect(result.files).toHaveLength(1);
            expect(result.files[0].path).toBe(subDirMemory);
        ...
        it('should fall back to trusted root as ceiling when no git root exists', async () => {
            ...
            const subDir = await createEmptyDir(path.join(rootDir, 'subdir'));
            const targetFile = path.join(subDir, 'target.txt');
            const subDirMemory = await createTestFile(path.join(subDir, DEFAULT_CONTEXT_FILENAME), 'Content without git');
            const result = await loadJitSubdirectoryMemory(targetFile, [rootDir], new Set());
        ...
        it('should stop at a custom boundary marker instead of .git', async () => {
            ...
            const rootMemory = await createTestFile(path.join(rootDir, DEFAULT_CONTEXT_FILENAME), 'Root rules');
            const subDirMemory = await createTestFile(path.join(subDir, DEFAULT_CONTEXT_FILENAME), 'App rules');
            const result = await loadJitSubdirectoryMemory(targetFile, [rootDir], new Set(), undefined, ['.monorepo-root']);
            expect(result.files).toHaveLength(2);
        ...
        it('should support multiple boundary markers', async () => {
            ...
            const rootMemory = await createTestFile(path.join(rootDir, DEFAULT_CONTEXT_FILENAME), 'Root content');
            const result = await loadJitSubdirectoryMemory(targetFile, [rootDir], new Set(), undefined, ['.git', 'package.json']);
        ...
        it('should disable parent traversal when boundary markers array is empty', async () => {
            ...
            const subDir = await createEmptyDir(path.join(rootDir, 'subdir'));
            const targetFile = path.join(subDir, 'target.txt');
            await createTestFile(path.join(rootDir, DEFAULT_CONTEXT_FILENAME), 'Root content');
            const subDirMemory = await createTestFile(path.join(subDir, DEFAULT_CONTEXT_FILENAME), 'Subdir content');
            const result = await loadJitSubdirectoryMemory(targetFile, [rootDir], new Set(), undefined, []);
