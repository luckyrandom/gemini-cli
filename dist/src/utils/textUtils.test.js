...
import { safeLiteralReplace, truncateString, safeTemplateReplace, } from './textUtils.js';
...
describe('safeTemplateReplace', () => {
    it('replaces all occurrences of known keys', () => {
        ...
        expect(safeTemplateReplace(tmpl, replacements)).toBe('Hello Alice, welcome to Wonderland. Alice is happy.');
    });
    it('ignores keys not present in replacements', () => {
        const tmpl = 'Hello {{name}}, welcome to {{unknown}}.';
        const replacements = { name: 'Bob' };
        expect(safeTemplateReplace(tmpl, replacements)).toBe('Hello Bob, welcome to {{unknown}}.');
    });
