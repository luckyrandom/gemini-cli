...
import { deleteSessionArtifactsAsync, deleteSubagentSessionDirAndArtifactsAsync, validateAndSanitizeSessionId, } from './sessionOperations.js';
describe('sessionOperations', () => {
    ...
    describe('deleteSessionArtifactsAsync', () => {
        ...
        it('should ignore ENOENT errors during deletion', async () => {
            ...
            await expect(deleteSessionArtifactsAsync('non-existent', tempDir)).resolves.toBeUndefined();
        });
    });
    describe('deleteSubagentSessionDirAndArtifactsAsync', () => {
        it('should iterate subagent files and delete their artifacts', async () => {
            ...
            await deleteSubagentSessionDirAndArtifactsAsync(parentSessionId, chatsDir, tempDir);
        ...
        it('should resolve for safe path even if input contains traversals (due to sanitization)', async () => {
            ...
            await expect(deleteSubagentSessionDirAndArtifactsAsync('../unsafe', chatsDir, tempDir)).resolves.toBeUndefined();
        });
        it('should handle ENOENT for readdir gracefully', async () => {
            // Non-existent directory should not throw
            await expect(deleteSubagentSessionDirAndArtifactsAsync('non-existent-parent', chatsDir, tempDir)).resolves.toBeUndefined();
        });
    });
});
//# sourceMappingURL=sessionOperations.test.js.map