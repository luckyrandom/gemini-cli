 ...
 */
export async function deduplicatePathsByFileIdentity(filePaths) {
    ...
    for (let i = 0; i < uniqueFilePaths.length; i += CONCURRENT_LIMIT) {
        ...
        const batchPromises = batch.map(async (filePath) => {
            try {
                ...
                logger.debug(`could not stat file for deduplication: ${filePath}. error: ${message}`);
                return {
                    path: filePath,
                    dev: null,
                    ino: null,
    ...
    const pathToIdentityMap = new Map();
    for (const { path, dev, ino } of results) {
        if (dev !== null && ino !== null) {
            ...
            if (!fileIdentityMap.has(identityKey)) {
                ...
                debugLogger.debug('[DEBUG] [MemoryDiscovery] deduplication: keeping', path, `(dev: ${dev}, ino: ${ino})`);
            }
            else {
                const existingPath = fileIdentityMap.get(identityKey);
                debugLogger.debug('[DEBUG] [MemoryDiscovery] deduplication: skipping', path, `(same file as ${existingPath})`);
            }
        }
        else {
            deduplicatedPaths.push(path);
            ...
            try {
                ...
                if (!isENOENT && !isTestEnv) {
                    if (typeof error === 'object' && error !== null && 'code' in error) {
                        // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
                        const fsError = error;
                        logger.warn(`Error checking for ${marker} at ${markerPath}: ${fsError.message}`);
                    }
                    else {
                        logger.warn(`Non-standard error checking for ${marker} at ${markerPath}: ${String(error)}`);
                    }
                }
            }
        }
...
export async function readGeminiMdFiles(filePaths, importFormat = 'tree', boundaryMarkers = ['.git']) {
    ...
    for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
        ...
        const batchPromises = batch.map(async (filePath) => {
            try {
                ...
                const processedResult = await processImports(content, path.dirname(filePath), false, undefined, undefined, importFormat, boundaryMarkers);
...
export async function getUserProjectMemoryPaths(projectMemoryDir) {
    const geminiMdFilenames = getAllGeminiMdFilenames();
    const accessChecks = geminiMdFilenames.map(async (filename) => {
...
export async function getEnvironmentMemoryPaths(trustedRoots, boundaryMarkers = ['.git']) {
    ...
    const traversalPromises = trustedRoots.map(async (root) => {
        ...
        debugLogger.debug('[DEBUG] [MemoryDiscovery] Loading environment memory for trusted root:', resolvedRoot, '(Stopping at', gitRoot
            ? `git root: ${ceiling})`
...
export function categorizeAndConcatenate(paths, contentsMap) {
 ...
 */
export async function loadServerHierarchicalMemory(currentWorkingDirectory, includeDirectoriesToReadGemini, fileService, extensionLoader, folderTrust, importFormat = 'tree', fileFilteringOptions, maxDirs = 200, boundaryMarkers = ['.git']) {
    ...
    const allFilePathsStringDeduped = Array.from(new Set([
...
export async function refreshServerHierarchicalMemory(config) {
    const result = await loadServerHierarchicalMemory(config.getWorkingDir(), config.shouldLoadMemoryFromIncludeDirectories()
        ...
        : [], config.getFileService(), config.getExtensionLoader(), config.isTrustedFolder(), config.getImportFormat(), config.getFileFilteringOptions(), config.getDiscoveryMaxDirs(), config.getMemoryBoundaryMarkers());
...
export async function loadJitSubdirectoryMemory(targetPath, trustedRoots, alreadyLoadedPaths, alreadyLoadedIdentities, boundaryMarkers = ['.git']) {
    ...
    const cachedIdentities = alreadyLoadedIdentities ?? new Set();
