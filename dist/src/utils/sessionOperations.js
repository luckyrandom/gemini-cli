 ...
 */
export function validateAndSanitizeSessionId(sessionId) {
 ...
 */
export async function deleteSessionArtifactsAsync(sessionId, tempDir) {
    try {
        ...
        await fs.unlink(logPath).catch((err) => {
            if (err.code !== 'ENOENT')
                throw err;
        });
 ...
 */
export async function deleteSubagentSessionDirAndArtifactsAsync(parentSessionId, chatsDir, tempDir) {
    ...
    try {
        ...
        debugLogger.error(`Error cleaning up subagents for parent ${parentSessionId}:`, error);
