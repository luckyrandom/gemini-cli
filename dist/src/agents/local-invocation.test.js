...
import { AgentTerminateMode, SubagentActivityErrorType, SUBAGENT_REJECTED_ERROR_PREFIX, } from './types.js';
...
describe('LocalSubagentInvocation', () => {
    ...
    describe('execute', () => {
        ...
        it('should stream THOUGHT_CHUNK activities from the executor, replacing the last running thought', async () => {
            ...
            expect(lastCall.recentActivity).not.toContainEqual(expect.objectContaining({
                type: 'thought',
                content: 'Analyzing...',
            }));
        });
        ...
        it('should handle executor run failure', async () => {
            ...
            const display = result.returnDisplay;
            expect(display.isSubagentProgress).toBe(true);
            expect(display.recentActivity).toContainEqual(expect.objectContaining({
                type: 'thought',
        ...
        it('should throw an error and bubble cancellation when execution returns ABORTED', async () => {
            ...
            mockExecutorInstance.run.mockResolvedValue(mockOutput);
            await expect(invocation.execute({ abortSignal: signal, updateOutput })).rejects.toThrow('Operation cancelled by user');
        });
