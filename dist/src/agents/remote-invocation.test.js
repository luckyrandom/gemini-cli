...
import {} from './a2a-client-manager.js';
...
describe('RemoteAgentInvocation', () => {
    ...
    describe('Constructor Validation', () => {
        it('accepts valid input with string query', () => {
            expect(() => {
                new RemoteAgentInvocation(mockDefinition, mockContext, { query: 'valid' }, mockMessageBus);
            }).not.toThrow();
        });
        ...
        it('uses "Get Started!" default when query is missing during execution', async () => {
            ...
            const invocation = new RemoteAgentInvocation(mockDefinition, mockContext, {}, mockMessageBus);
        ...
        it('throws if query is not a string', () => {
            expect(() => {
                new RemoteAgentInvocation(mockDefinition, mockContext, { query: 123 }, mockMessageBus);
    ...
    describe('Execution Logic', () => {
        it('should lazy load the agent without auth handler when no auth configured', async () => {
            ...
            const invocation = new RemoteAgentInvocation(mockDefinition, mockContext, {
                query: 'hi',
            }, mockMessageBus);
            await invocation.execute({ abortSignal: new AbortController().signal });
            expect(mockClientManager.loadAgent).toHaveBeenCalledWith('test-agent', { type: 'url', url: 'http://test-agent/card' }, undefined);
        });
        it('should use A2AAuthProviderFactory when auth is present in definition', async () => {
            ...
            A2AAuthProviderFactory.create.mockResolvedValue(mockHandler);
            mockClientManager.getClient.mockReturnValue(undefined);
            mockClientManager.sendMessageStream.mockImplementation(async function* () {
                yield {
                    kind: 'message',
            ...
            const invocation = new RemoteAgentInvocation(authDefinition, mockContext, { query: 'hi' }, mockMessageBus);
            await invocation.execute({ abortSignal: new AbortController().signal });
            ...
            expect(mockClientManager.loadAgent).toHaveBeenCalledWith('test-agent', { type: 'url', url: 'http://test-agent/card' }, mockHandler);
        });
        it('should return error when auth provider factory returns undefined for configured auth', async () => {
            ...
            expect(result.returnDisplay.result).toContain("Failed to create auth provider for agent 'test-agent'");
        });
        ...
        it('should persist contextId and taskId across invocations', async () => {
            ...
            const invocation1 = new RemoteAgentInvocation(mockDefinition, mockContext, {
                query: 'first',
            }, mockMessageBus);
            // Execute first time
            ...
            const invocation2 = new RemoteAgentInvocation(mockDefinition, mockContext, {
                query: 'second',
            }, mockMessageBus);
            const result2 = await invocation2.execute({
                abortSignal: new AbortController().signal,
            });
            expect(result2.returnDisplay.result).toBe('Response 2');
            ...
            const invocation3 = new RemoteAgentInvocation(mockDefinition, mockContext, {
                query: 'third',
            }, mockMessageBus);
            ...
            const invocation4 = new RemoteAgentInvocation(mockDefinition, mockContext, {
                query: 'fourth',
            }, mockMessageBus);
        ...
        it('should handle streaming updates and reassemble output', async () => {
            ...
            const invocation = new RemoteAgentInvocation(mockDefinition, mockContext, { query: 'hi' }, mockMessageBus);
            await invocation.execute({
                abortSignal: new AbortController().signal,
                updateOutput,
            });
            expect(updateOutput).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should use a2a helpers for extracting text', async () => {
            ...
            expect(result.returnDisplay.result).toContain('Extracted text');
        });
    ...
    describe('Error Handling', () => {
        it('should use A2AAgentError.userMessage for structured errors', async () => {
            ...
            const a2aError = new AgentConnectionError('test-agent', 'http://test-agent/card', new Error('ECONNREFUSED'));
            mockClientManager.getClient.mockReturnValue(undefined);
            ...
            expect(result.returnDisplay).toMatchObject({ state: 'error' });
            expect(result.returnDisplay.result).toContain(a2aError.userMessage);
        });
        it('should use generic message for non-A2AAgentError errors', async () => {
            ...
            expect(result.returnDisplay).toMatchObject({ state: 'error' });
            expect(result.returnDisplay.result).toContain('Error calling remote agent: something unexpected');
        });
