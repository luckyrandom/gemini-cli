...
import { EDIT_TOOL_NAME, GLOB_TOOL_NAME, GREP_TOOL_NAME, LS_TOOL_NAME, READ_FILE_TOOL_NAME, WRITE_FILE_TOOL_NAME, } from '../tools/tool-names.js';
import { PREVIEW_GEMINI_FLASH_MODEL } from '../config/models.js';
describe('SkillExtractionAgent', () => {
    const skillsDir = '/tmp/skills';
    const sessionIndex = '[NEW] Debug login flow (12 user msgs) — /tmp/chats/session-1.json';
    const existingSkillsSummary = '## Workspace Skills (.gemini/skills — do NOT duplicate)\n- **existing-skill**: Existing description';
    ...
    it('should expose expected metadata, model, and tools', () => {
        ...
        expect(agent.toolConfig?.tools).toEqual(expect.arrayContaining([
            READ_FILE_TOOL_NAME,
    ...
    it('should default to no skill unless recurrence and durability are proven', () => {
        ...
        expect(prompt).toContain('strong evidence this will recur for future agents in this repo/workflow');
        expect(prompt).toContain('broader than a single incident');
        expect(prompt).toContain('A skill MUST meet ALL of these criteria:');
        expect(prompt).toContain('Future agents in this repo/workflow are likely to need it');
    });
    ...
    it('should warn that session summaries are user-intent summaries, not workflow evidence', () => {
        ...
        expect(query).toContain('The summary is a user-intent summary, not a workflow summary.');
        expect(query).toContain('The session summaries describe user intent, not workflow details.');
        expect(query).toContain('Only write a skill if the evidence shows a durable, recurring workflow');
        expect(query).toContain('If recurrence or future reuse is unclear, create no skill and explain why.');
    });
});
//# sourceMappingURL=skill-extraction-agent.test.js.map