...
const remoteAgentJsonSchema = baseRemoteAgentSchema
    ...
    agent_card_json: z.string().refine((val) => {
        try {
            JSON.parse(val);
            return true;
        }
...
function guessIntendedKind(rawInput) {
    ...
    if (input.kind === 'remote')
        return 'remote';
    const hasLocalKeys = 'tools' in input ||
...
function formatZodError(error, context, rawInput) {
    const intendedKind = rawInput ? guessIntendedKind(rawInput) : undefined;
    const formatIssues = (issues, unionPrefix) => issues.flatMap((i) => {
...
export async function parseAgentMarkdown(filePath, content) {
    ...
    try {
        ...
        throw new AgentLoadError(filePath, `YAML frontmatter parsing failed: ${getErrorMessage(error)}`);
    }
    ...
    if (!result.success) {
        throw new AgentLoadError(filePath, `Validation failed: ${formatZodError(result.error, 'Agent Definition', rawFrontmatter)}`);
    }
...
export function markdownToAgentDefinition(markdown, metadata) {
    ...
    if (markdown.kind === 'remote') {
        ...
        if ('agent_card_json' in markdown &&
        ...
        throw new AgentLoadError(metadata?.filePath || 'unknown', 'Unexpected state: neither agent_card_json nor agent_card_url present on remote agent');
    }
    ...
    if (markdown.mcp_servers) {
        for (const [name, config] of Object.entries(markdown.mcp_servers)) {
            let authProviderType = undefined;
            let oauth = undefined;
            if (config.auth) {
            ...
            mcpServers[name] = new MCPServerConfig(config.command, config.args, config.env, config.cwd, config.url, config.http_url, config.headers, config.tcp, config.type, config.timeout, config.trust, config.description, config.include_tools, config.exclude_tools, undefined, // extension
...
export async function loadAgentsFromDirectory(dir) {
    ...
    try {
        ...
        result.errors.push(new AgentLoadError(dir, `Could not list directory: ${getErrorMessage(error)}`));
        return result;
    }
    ...
    for (const entry of files) {
        ...
        try {
            ...
            if (error instanceof AgentLoadError) {
                ...
                result.errors.push(new AgentLoadError(filePath, `Unexpected error: ${getErrorMessage(error)}`));
            }
        }
    }
    return result;
