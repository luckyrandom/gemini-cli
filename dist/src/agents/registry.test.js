...
describe('AgentRegistry', () => {
    ...
    describe('registration logic', () => {
        ...
        it('should register a remote agent with authentication configuration', async () => {
            ...
            expect(loadAgentSpy).toHaveBeenCalledWith('RemoteAgentWithAuth', { type: 'url', url: 'https://example.com/card' }, mockHandler);
        ...
        it('should not register remote agent when auth provider factory returns undefined', async () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Error loading A2A agent'), expect.any(Error));
            warnSpy.mockRestore();
        });
        ...
        it('should emit error feedback with userMessage when A2AAgentError is thrown', async () => {
            ...
            const a2aError = new AgentConnectionError('FailAgent', 'https://unreachable.example.com/card', new Error('ECONNREFUSED'));
            vi.spyOn(mockConfig, 'getA2AClientManager').mockReturnValue({
                loadAgent: vi.fn().mockRejectedValue(a2aError),
            });
            await registry.testRegisterAgent(remoteAgent);
            expect(feedbackSpy).toHaveBeenCalledWith('error', `[FailAgent] ${a2aError.userMessage}`);
        ...
        it('should emit generic error feedback for non-A2AAgentError failures', async () => {
            ...
            await registry.testRegisterAgent(remoteAgent);
            expect(feedbackSpy).toHaveBeenCalledWith('error', '[FailAgent] Failed to load remote agent: unexpected crash');
            expect(registry.getDefinition('FailAgent')).toBeUndefined();
        });
        it('should emit warning feedback when auth config is missing for secured agent', async () => {
            ...
            vi.mocked(A2AAuthProviderFactory.describeRequiredAuth).mockReturnValue('apiKey (header: x-api-key)');
            const remoteAgent = {
                kind: 'remote',
                name: 'SecuredAgent',
            ...
            expect(feedbackSpy).toHaveBeenCalledWith('warning', expect.stringContaining('SecuredAgent'));
        });
        it('should surface an error if remote agent registration fails', async () => {
            ...
            await registry.testRegisterAgent(remoteAgent);
            expect(feedbackSpy).toHaveBeenCalledWith('error', `[FailingRemoteAgent] Failed to load remote agent: 401 Unauthorized`);
        });
        ...
        it('should not accumulate descriptions on repeated registration', async () => {
            ...
            expect(firstDescription).toBe('User Description: User-provided description\nAgent Description: Card-provided description\nSkills:\nSkill1: Desc1');
        ...
        it('should result in ASK_USER policy for remote agents at runtime', async () => {
            ...
            const result = await policyEngine.check({ name: 'invoke_agent', args: { agent_name: 'RemotePolicyAgent' } }, undefined);
        ...
        it('should result in ALLOW policy for local agents at runtime (fallback to default allow)', async () => {
            ...
            const result = await policyEngine.check({ name: 'invoke_agent', args: { agent_name: 'LocalPolicyAgent' } }, undefined);
        ...
        it.skip('should replace an existing dynamic policy when an agent is overwritten', async () => {
            ...
            expect(removeRuleSpy).toHaveBeenCalledWith('OverwrittenAgent', DYNAMIC_RULE_SOURCE);
    ...
    describe('browser agent sandbox registration', () => {
        ...
        it('should NOT register browser agent in container sandbox without existing mode', async () => {
            ...
            expect(feedbackSpy).toHaveBeenCalledWith('info', expect.stringContaining('Browser agent disabled in container sandbox'));
        });
