...
class McpToolInvocation extends BaseToolInvocation {
    ...
    async execute({ abortSignal: signal }) {
        try {
            ...
            const result = await this.browserManager.callTool(this.toolName, this.params, signal);
...
class McpDeclarativeTool extends DeclarativeTool {
    ...
    shouldDisableInput;
    blockFileUploads;
    constructor(browserManager, name, description, parameterSchema, messageBus, shouldDisableInput, blockFileUploads = false) {
 ...
 */
export async function createMcpDeclarativeTools(browserManager, messageBus, shouldDisableInput = false, blockFileUploads = false) {
    ...
    const tools = mcpTools.map((mcpTool) => {
        ...
        return new McpDeclarativeTool(browserManager, mcpTool.name, augmentedDescription, schema.parametersJsonSchema, messageBus, shouldDisableInput, blockFileUploads);
    });
