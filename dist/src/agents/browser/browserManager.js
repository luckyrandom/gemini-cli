...
export class BrowserManager {
    ...
    async close() {
        ...
        if (this.rawMcpClient) {
            try {
                ...
                debugLogger.error(`Error closing MCP client: ${error instanceof Error ? error.message : String(error)}`);
            }
        ...
        if (this.mcpTransport) {
            try {
                ...
                debugLogger.error(`Error closing MCP transport: ${error instanceof Error ? error.message : String(error)}`);
            }
    ...
    async connectMcp() {
        ...
        if (browserConfig.customConfig.profilePath) {
            ...
            const defaultProfilePath = path.join(Storage.getGlobalGeminiDir(), BROWSER_PROFILE_DIR);
        ...
        if (browserConfig.customConfig.allowedDomains &&
        ...
        if (!fs.existsSync(bundleMcpPath)) {
            bundleMcpPath = path.resolve(__dirname, __dirname.includes(`${path.sep}dist${path.sep}`)
    ...
    async discoverTools() {
        ...
        debugLogger.log(`Discovered ${this.discoveredTools.length} tools from chrome-devtools-mcp: ` +
     ...
     */
    checkNavigationRestrictions(toolName, args) {
