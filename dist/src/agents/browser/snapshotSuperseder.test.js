...
import { supersedeStaleSnapshots, SNAPSHOT_SUPERSEDED_PLACEHOLDER, } from './snapshotSuperseder.js';
/** Builds a minimal mock GeminiChat around a mutable history array. */
function createMockChat(history) {
    return {
        getHistory: vi.fn(() => [...history]),
        setHistory: vi.fn((newHistory) => {
            history.length = 0;
...
function snapshotResponse(output) {
    return {
        functionResponse: {
            name: 'take_snapshot',
            response: { output },
...
function otherToolResponse(name, output) {
    return {
        functionResponse: {
            name,
            response: { output },
...
describe('supersedeStaleSnapshots', () => {
    ...
    it('should no-op when history has no snapshots', () => {
        history.push({ role: 'user', parts: [{ text: 'Click the button' }] }, {
            role: 'user',
    ...
    it('should no-op when history has exactly 1 snapshot', () => {
        history.push({ role: 'user', parts: [{ text: 'Navigate to page' }] }, {
            role: 'user',
...
//# sourceMappingURL=snapshotSuperseder.test.js.map