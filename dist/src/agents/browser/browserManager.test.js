...
describe('BrowserManager', () => {
    ...
    describe('MCP connection', () => {
        ...
        it('should throw error when invalid domain is configured in allowedDomains', async () => {
            ...
            await expect(manager.ensureConnection()).rejects.toThrow('Invalid domain in allowedDomains: invalid domain!');
        });
        ...
        it('should pass --autoConnect when sessionMode is existing', async () => {
            ...
            expect(args).not.toContain('--isolated');
            expect(coreEvents.emitFeedback).toHaveBeenCalledWith('info', expect.stringContaining('saved logins will be visible'));
        });
        ...
        it('should throw profile-lock remediation when persistent mode hits "already running"', async () => {
            vi.mocked(Client).mockImplementation(() => ({
                connect: vi
                    .fn()
                    .mockRejectedValue(new Error('Could not connect to Chrome. The browser is already running for the current profile.')),
                close: vi.fn().mockResolvedValue(undefined),
                listTools: vi.fn(),
                callTool: vi.fn(),
            }));
        ...
        it('should throw timeout-specific remediation for persistent mode', async () => {
            vi.mocked(Client).mockImplementation(() => ({
                connect: vi
                    .fn()
                    .mockRejectedValue(new Error('Timed out connecting to chrome-devtools-mcp')),
                close: vi.fn().mockResolvedValue(undefined),
                listTools: vi.fn(),
                callTool: vi.fn(),
            }));
...
//# sourceMappingURL=browserManager.test.js.map