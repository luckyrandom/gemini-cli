...
describe('BrowserAgentInvocation', () => {
    ...
    describe('execute', () => {
        ...
        beforeEach(() => {
            ...
            vi.mocked(LocalAgentExecutor.create).mockResolvedValue(mockExecutor);
        ...
        it('should return result text and call cleanup on success', async () => {
            ...
            expect(result.llmContent[0].text).toContain('Browser agent finished');
        ...
        it('should work without updateOutput (fire-and-forget)', async () => {
            ...
            await expect(invocation.execute({ abortSignal: controller.signal })).resolves.toBeDefined();
        });
        ...
        function setupActivityCapture() {
            let onActivityFn;
            vi.mocked(LocalAgentExecutor.create).mockImplementation(async (_def, _config, onActivity) => {
        ...
        it('should emit initial SubagentProgress with running state', async () => {
            ...
            const firstCall = updateOutput.mock.calls[0]?.[0];
        ...
        it('should emit completed SubagentProgress on success', async () => {
            ...
            const lastCall = updateOutput.mock.calls[updateOutput.mock.calls.length - 1]?.[0];
        ...
        it('should handle THOUGHT_CHUNK and emit structured progress', async () => {
            ...
            const thoughtProgress = progressCalls.find((p) => p.recentActivity.some((a) => a.type === 'thought' &&
        ...
        it('should overwrite the thought content with new THOUGHT_CHUNK activity', async () => {
            ...
            expect(lastCall.recentActivity).toContainEqual(expect.objectContaining({
                type: 'thought',
        ...
        it('should handle TOOL_CALL_START and TOOL_CALL_END with callId tracking', async () => {
            ...
            const toolItem = finalProgress?.recentActivity.find((a) => a.type === 'tool_call' && a.content === 'navigate_browser');
        ...
        it('should handle ERROR event with callId and mark tool as errored', async () => {
            ...
            const toolItem = allItems.find((a) => a.type === 'tool_call' && a.content === 'click_element');
        ...
        it('should mark all running tools as errored when ERROR has no callId', async () => {
            ...
            const toolA = finalActivity.find((a) => a.type === 'tool_call' && a.content === 'tool_a');
            const toolB = finalActivity.find((a) => a.type === 'tool_call' && a.content === 'tool_b');
        ...
        it('should record successful task outcome metrics', async () => {
            ...
            expect(recordBrowserAgentTaskOutcome).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                success: true,
    ...
    describe('cleanup', () => {
        it('should clean up all pages on finally', async () => {
            ...
            vi.mocked(LocalAgentExecutor.create).mockResolvedValue(mockExecutor);
            const invocation = new BrowserAgentInvocation(mockConfig, { task: 'test' }, mockMessageBus);
            await invocation.execute({ abortSignal: new AbortController().signal });
            // Verify list_pages was called
            expect(mockBrowserManager.callTool).toHaveBeenCalledWith('list_pages', expect.anything(), expect.anything(), true);
            // Verify select_page was called for each page
            expect(mockBrowserManager.callTool).toHaveBeenCalledWith('select_page', { pageId: 0, bringToFront: false }, expect.anything(), true);
            expect(mockBrowserManager.callTool).toHaveBeenCalledWith('select_page', { pageId: 1, bringToFront: false }, expect.anything(), true);
