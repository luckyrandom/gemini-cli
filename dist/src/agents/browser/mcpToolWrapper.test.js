...
describe('mcpToolWrapper', () => {
    ...
    describe('createMcpDeclarativeTools', () => {
        it('should create declarative tools from discovered MCP tools', async () => {
            const tools = await createMcpDeclarativeTools(mockBrowserManager, mockMessageBus, false);
    ...
    describe('Input blocker suspend/resume', () => {
        it('should suspend and resume input blocker around click (interactive tool)', async () => {
            const tools = await createMcpDeclarativeTools(mockBrowserManager, mockMessageBus, true);
            ...
            expect(mockBrowserManager.callTool).toHaveBeenNthCalledWith(1, 'evaluate_script', expect.objectContaining({
                function: expect.stringContaining('__gemini_input_blocker'),
            }), expect.any(AbortSignal), true);
            // Second call: click
            expect(mockBrowserManager.callTool).toHaveBeenNthCalledWith(2, 'click', { uid: 'elem-42' }, expect.any(AbortSignal));
            // Third call: resume blocker (pointer-events: auto)
            expect(mockBrowserManager.callTool).toHaveBeenNthCalledWith(3, 'evaluate_script', expect.objectContaining({
                function: expect.stringContaining('__gemini_input_blocker'),
            }), expect.any(AbortSignal), true);
        });
        it('should NOT suspend/resume for take_snapshot (read-only tool)', async () => {
            ...
            expect(mockBrowserManager.callTool).toHaveBeenCalledWith('take_snapshot', {}, expect.any(AbortSignal));
        });
    ...
    describe('Hard Block: upload_file', () => {
        ...
        it('should block upload_file when blockFileUploads is true', async () => {
            const tools = await createMcpDeclarativeTools(mockBrowserManager, mockMessageBus, false, true);
        ...
        it('should NOT block upload_file when blockFileUploads is false', async () => {
            const tools = await createMcpDeclarativeTools(mockBrowserManager, mockMessageBus, false, false);
            const uploadTool = tools.find((t) => t.name === 'upload_file');
            const invocation = uploadTool.build({ path: 'test.txt' });
            const result = await invocation.execute({
                abortSignal: new AbortController().signal,
            ...
            expect(result.error).toBeUndefined();
            expect(result.llmContent).toBe('Tool result');
            expect(mockBrowserManager.callTool).toHaveBeenCalledWith('upload_file', expect.anything(), expect.anything());
        });
    });
});
