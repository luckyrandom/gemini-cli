...
import { createBrowserAgentDefinition, cleanupBrowserAgent, resetBrowserSession, } from './browserAgentFactory.js';
import { injectAutomationOverlay } from './automationOverlay.js';
import { makeFakeConfig } from '../../test-utils/config.js';
import { PolicyDecision, PRIORITY_SUBAGENT_TOOL } from '../../policy/types.js';
import { recordBrowserAgentToolDiscovery, recordBrowserAgentVisionStatus, recordBrowserAgentCleanup, } from '../../telemetry/metrics.js';
...
vi.mock('./browserManager.js', () => {
    ...
    const MockBrowserManager = vi.fn();
...
import { buildBrowserSystemPrompt, BROWSER_AGENT_NAME, } from './browserAgentDefinition.js';
describe('browserAgentFactory', () => {
    ...
    beforeEach(async () => {
        ...
        MockBM['getInstance'].mockReturnValue(mockBrowserManager);
    ...
    describe('createBrowserAgentDefinition', () => {
        ...
        it('should return agent definition with discovered tools', async () => {
            const { definition } = await createBrowserAgentDefinition(mockConfig, mockMessageBus);
        ...
        it('should return browser manager for cleanup', async () => {
            const { browserManager } = await createBrowserAgentDefinition(mockConfig, mockMessageBus);
        ...
        it('should include visual prompt section when visualModel is configured', async () => {
            ...
            const { definition } = await createBrowserAgentDefinition(configWithVision, mockMessageBus);
            const systemPrompt = definition.promptConfig?.systemPrompt ?? '';
            expect(systemPrompt).toContain('analyze_screenshot');
            expect(systemPrompt).toContain('VISUAL IDENTIFICATION');
            expect(recordBrowserAgentVisionStatus).toHaveBeenCalledWith(configWithVision, { enabled: true, disabled_reason: undefined });
        });
        it('should include analyze_screenshot tool when visualModel is configured', async () => {
            ...
            const toolNames = definition.toolConfig?.tools
                ?.filter((t) => typeof t === 'object' && 'name' in t)
                .map((t) => t.name) ?? [];
        ...
        it('should include domain restrictions in system prompt when configured', async () => {
            ...
            const { definition } = await createBrowserAgentDefinition(configWithDomains, mockMessageBus);
            const systemPrompt = definition.promptConfig?.systemPrompt ?? '';
        ...
        it('should trigger telemetry recording for tool discovery', async () => {
            ...
            expect(recordBrowserAgentToolDiscovery).toHaveBeenCalledWith(configWithVision, 6, // 6 mock tools from getDiscoveredTools
        ...
        it('should trigger telemetry recording for missing semantic tools', async () => {
            ...
            await createBrowserAgentDefinition(configWithVision, mockMessageBus);
            expect(recordBrowserAgentToolDiscovery).toHaveBeenCalledWith(configWithVision, 1, // 1 mock tool from getDiscoveredTools
            ['click', 'fill', 'navigate_page'], 'persistent');
        });
    });
    describe('resetBrowserSession', () => {
        it('should delegate to BrowserManager.resetAll', async () => {
            ...
            expect(MockBrowserManager['resetAll']).toHaveBeenCalled();
        });
    });
    describe('Policy Registration', () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(mockConfig, 'getPolicyEngine').mockReturnValue(mockPolicyEngine);
        });
        it('should register sensitive action rules', async () => {
            ...
            vi.spyOn(mockConfig, 'getPolicyEngine').mockReturnValue(mockPolicyEngine);
            await createBrowserAgentDefinition(mockConfig, mockMessageBus);
            expect(mockPolicyEngine.addRule).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should register fill rule even when confirmSensitiveActions is disabled', async () => {
            ...
            expect(mockPolicyEngine.addRule).not.toHaveBeenCalledWith(expect.objectContaining({
                toolName: 'mcp_browser_agent_upload_file',
            }));
        });
    ...
    describe('cleanupBrowserAgent', () => {
        it('should call close on browser manager', async () => {
            ...
            await cleanupBrowserAgent(mockBrowserManager, mockConfig, 'persistent');
        ...
        it('should record successful cleanup metrics', async () => {
            const mockConfig = makeFakeConfig({});
            await cleanupBrowserAgent(mockBrowserManager, mockConfig, 'isolated');
            expect(mockBrowserManager.close).toHaveBeenCalled();
            expect(recordBrowserAgentCleanup).toHaveBeenCalledWith(mockConfig, expect.any(Number), {
        ...
        it('should record failed cleanup metrics when browserManager.close() throws', async () => {
            ...
            await cleanupBrowserAgent(mockBrowserManager, mockConfig, 'existing');
            expect(recordBrowserAgentCleanup).toHaveBeenCalledWith(mockConfig, expect.any(Number), {
...
//# sourceMappingURL=browserAgentFactory.test.js.map