...
import { A2AAgentError, AgentCardNotFoundError, AgentCardAuthError, AgentAuthConfigMissingError, AgentConnectionError, classifyAgentError, } from './a2a-errors.js';
describe('A2A Error Types', () => {
    ...
    describe('AgentCardAuthError', () => {
        it('should produce a user-friendly 401 message', () => {
            const error = new AgentCardAuthError('secure-agent', 'https://example.com/card', 401);
        ...
        it('should produce a user-friendly 403 message', () => {
            const error = new AgentCardAuthError('secure-agent', 'https://example.com/card', 403);
    ...
    describe('AgentAuthConfigMissingError', () => {
        it('should list missing config fields', () => {
            const error = new AgentAuthConfigMissingError('api-agent', 'API Key (x-api-key): Send x-api-key in header', [
    ...
    describe('AgentConnectionError', () => {
        it('should wrap the original error cause', () => {
            ...
            const error = new AgentConnectionError('my-agent', 'https://example.com/card', cause);
        ...
        it('should handle non-Error causes', () => {
            const error = new AgentConnectionError('my-agent', 'https://example.com/card', 'raw string error');
    ...
    describe('classifyAgentError', () => {
        ...
        it('should handle non-Error values', () => {
            const result = classifyAgentError('agent-e', 'https://example.com/card', 'some string error');
            expect(result).toBeInstanceOf(AgentConnectionError);
        });
        describe('cause chain inspection', () => {
            it('should detect 404 in a nested cause', () => {
                ...
                const result = classifyAgentError('agent-nested', 'https://example.com/card', outer);
                expect(result).toBeInstanceOf(AgentCardNotFoundError);
            });
            it('should detect 401 in a deeply nested cause', () => {
                ...
                const result = classifyAgentError('agent-deep', 'https://example.com/card', outer);
                expect(result).toBeInstanceOf(AgentCardAuthError);
                expect(result.statusCode).toBe(401);
            });
            it('should detect ECONNREFUSED error code in cause chain', () => {
                ...
                const outer = new Error('fetch failed', { cause: inner });
                const result = classifyAgentError('agent-conn', 'https://example.com/card', outer);
                expect(result).toBeInstanceOf(AgentConnectionError);
            });
            it('should detect status property on error objects in cause chain', () => {
                ...
                const result = classifyAgentError('agent-status', 'https://example.com/card', outer);
                expect(result).toBeInstanceOf(AgentCardAuthError);
                expect(result.statusCode).toBe(403);
            });
            it('should detect status on a plain-object cause (non-Error)', () => {
                ...
                const result = classifyAgentError('agent-plain-cause', 'https://example.com/card', outer);
                expect(result).toBeInstanceOf(AgentCardAuthError);
                expect(result.statusCode).toBe(401);
            });
            it('should detect statusCode on a plain-object cause (non-Error)', () => {
                ...
                const result = classifyAgentError('agent-plain-cause-403', 'https://example.com/card', outer);
                expect(result).toBeInstanceOf(AgentCardAuthError);
                expect(result.statusCode).toBe(403);
            });
            it('should classify ENOTFOUND as AgentConnectionError, not 404', () => {
                ...
                const outer = new Error('fetch failed', { cause: inner });
                const result = classifyAgentError('agent-dns', 'https://example.invalid/card', outer);
                expect(result).toBeInstanceOf(AgentConnectionError);
...
//# sourceMappingURL=a2a-errors.test.js.map