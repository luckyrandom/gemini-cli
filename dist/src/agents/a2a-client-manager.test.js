...
describe('A2AClientManager', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(ClientFactory).mockReturnValue(factoryInstance);
        vi.mocked(DefaultAgentCardResolver).mockReturnValue(resolverInstance);
        vi.spyOn(factoryInstance, 'createFromUrl').mockResolvedValue(mockClient);
        vi.spyOn(factoryInstance, 'createFromAgentCard').mockResolvedValue(mockClient);
        vi.spyOn(resolverInstance, 'resolve').mockResolvedValue({
            ...mockAgentCard,
            url: 'http://test.agent/real/endpoint',
        });
        vi.spyOn(ClientFactoryOptions, 'createFrom').mockImplementation((_defaults, overrides) => overrides);
        vi.mocked(createAuthenticatingFetchWithRetry).mockImplementation(() => authFetchMock.mockResolvedValue({
            ok: true,
            json: async () => ({}),
        }));
    ...
    describe('getInstance / dispatcher initialization', () => {
        it('should use UndiciAgent when no proxy is configured', async () => {
            ...
            const resolverOptions = vi.mocked(DefaultAgentCardResolver).mock
                .calls[0][0];
            const cardFetch = resolverOptions?.fetchImpl;
            await cardFetch('http://test.agent/card');
            const fetchCall = vi
                .mocked(fetch)
                .mock.calls.find((call) => call[0] === 'http://test.agent/card');
            expect(fetchCall).toBeDefined();
            expect(fetchCall[1]?.dispatcher).toBeInstanceOf(UndiciAgent);
            expect(fetchCall[1]?.dispatcher).not.toBeInstanceOf(ProxyAgent);
        });
        it('should use ProxyAgent when a proxy is configured via Config', async () => {
            ...
            expect(fetchCall).toBeDefined();
            expect(fetchCall[1]?.dispatcher).toBeInstanceOf(ProxyAgent);
        });
    });
    describe('loadAgent', () => {
        ...
        it('should throw an error if an agent with the same name is already loaded', async () => {
            ...
            await expect(manager.loadAgent('TestAgent', {
                type: 'url',
                url: 'http://test.agent/card',
        ...
        it('should use provided custom authentication handler for transports only', async () => {
            ...
            await manager.loadAgent('TestAgent', { type: 'url', url: 'http://test.agent/card' }, customAuthHandler);
        ...
        it('should use unauthenticated fetch for card resolver and avoid authenticated fetch if success', async () => {
            ...
            await manager.loadAgent('AuthCardAgent', { type: 'url', url: 'http://authcard.agent/card' }, customAuthHandler);
            const resolverOptions = vi.mocked(DefaultAgentCardResolver).mock
                .calls[0][0];
            const cardFetch = resolverOptions?.fetchImpl;
        ...
        it('should retry with authenticating fetch if agent card fetch returns 401', async () => {
            ...
            await manager.loadAgent('AuthCardAgent401', { type: 'url', url: 'http://authcard.agent/card' }, customAuthHandler);
            const resolverOptions = vi.mocked(DefaultAgentCardResolver).mock
                .calls[0][0];
            const cardFetch = resolverOptions?.fetchImpl;
            await cardFetch('http://test.url');
        ...
        it('should log a debug message upon loading an agent', async () => {
            ...
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Loaded agent 'TestAgent'"));
        });
        ...
        it('should throw a descriptive error for invalid inline JSON', async () => {
            await expect(manager.loadAgent('BadJsonAgent', {
                type: 'json',
                json: 'not valid json {{',
            })).rejects.toThrow(/Failed to parse inline agent card JSON for agent 'BadJsonAgent'/);
        });
        it('should log "inline JSON" for JSON-loaded agents', async () => {
            ...
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('inline JSON'));
        });
        it('should throw if resolveAgentCard fails', async () => {
            ...
            vi.mocked(DefaultAgentCardResolver).mockReturnValue(resolverInstance);
            await expect(manager.loadAgent('FailAgent', {
                type: 'url',
                url: 'http://fail.agent',
    ...
    describe('sendMessageStream', () => {
        ...
        it('should use contextId and taskId when provided', async () => {
            ...
            expect(mockClient.sendMessageStream).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should correctly propagate AbortSignal to the stream', async () => {
            ...
            expect(mockClient.sendMessageStream).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({ signal: controller.signal }));
        });
        ...
        it('should throw prefixed error on failure', async () => {
            ...
            }).rejects.toThrow('[A2AClientManager] sendMessageStream Error [TestAgent]: Network failure');
        });
    ...
    describe('getTask', () => {
        ...
        it('should throw prefixed error on failure', async () => {
            ...
            await expect(manager.getTask('TestAgent', 'task123')).rejects.toThrow('A2AClient getTask Error [TestAgent]: Not found');
        });
        it('should throw an error if the agent is not found', async () => {
    ...
    describe('cancelTask', () => {
        ...
        it('should throw prefixed error on failure', async () => {
            ...
            await expect(manager.cancelTask('TestAgent', 'task123')).rejects.toThrow('A2AClient cancelTask Error [TestAgent]: Cannot cancel');
        });
        it('should throw an error if the agent is not found', async () => {
