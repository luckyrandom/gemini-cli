...
describe('AgentTool', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(registry, 'getDefinition').mockImplementation((name) => {
            if (name === 'TestLocalAgent')
                return testLocalDefinition;
            if (name === 'TestRemoteAgent')
                return testRemoteDefinition;
    ...
    it('should map prompt to objective for local agent', async () => {
        ...
        expect(LocalSubagentInvocation).toHaveBeenCalledWith(testLocalDefinition, mockConfig, { objective: 'Do something' }, mockMessageBus);
    });
    it('should map prompt to query for remote agent', async () => {
        ...
        // Trigger deferred instantiation
        await invocation.shouldConfirmExecute(new AbortController().signal);
        expect(RemoteAgentInvocation).toHaveBeenCalledWith(testRemoteDefinition, mockConfig, { query: 'Search something' }, mockMessageBus);
    });
    ...
    it('should map prompt to task and use BrowserAgentInvocation for browser agent', async () => {
        ...
        // Trigger deferred instantiation
        await invocation.shouldConfirmExecute(new AbortController().signal);
        expect(BrowserAgentInvocation).toHaveBeenCalledWith(mockConfig, { task: 'Open page' }, mockMessageBus, 'invoke_agent', 'Invoke Browser Agent');
    });
});
//# sourceMappingURL=agent-tool.test.js.map