...
export class GoogleCredentialsAuthProvider extends BaseA2AAuthProvider {
    ...
    constructor(config, targetUrl) {
        ...
        const scopes = this.config.scopes && this.config.scopes.length > 0
            ? this.config.scopes
    ...
    async headers() {
        ...
        if (this.useIdToken) {
            try {
                ...
                const errorMessage = `Failed to get ADC ID token: ${e instanceof Error ? e.message : String(e)}`;
        ...
        try {
            ...
            const errorMessage = `Failed to get ADC access token: ${e instanceof Error ? e.message : String(e)}`;
            debugLogger.error(errorMessage, e);
            throw new Error(errorMessage);
        }
    }
...
//# sourceMappingURL=google-credentials-provider.js.map