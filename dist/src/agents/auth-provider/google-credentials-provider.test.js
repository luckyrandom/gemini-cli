...
describe('GoogleCredentialsAuthProvider', () => {
    ...
    describe('Initialization', () => {
        it('throws if no targetUrl is provided', () => {
            expect(() => new GoogleCredentialsAuthProvider(mockConfig)).toThrow(/targetUrl must be provided/);
        });
        it('throws if targetHost is not allowed', () => {
            expect(() => new GoogleCredentialsAuthProvider(mockConfig, 'https://example.com')).toThrow(/is not an allowed host/);
        });
        it('initializes seamlessly with .googleapis.com', () => {
            expect(() => new GoogleCredentialsAuthProvider(mockConfig, 'https://language.googleapis.com/v1/models')).not.toThrow();
        });
        it('initializes seamlessly with .run.app', () => {
            expect(() => new GoogleCredentialsAuthProvider(mockConfig, 'https://my-cloud-run-service.run.app')).not.toThrow();
        });
    });
    describe('Token Fetching', () => {
        it('fetches an access token for googleapis.com endpoint', async () => {
            const provider = new GoogleCredentialsAuthProvider(mockConfig, 'https://language.googleapis.com');
        ...
        it('fetches an identity token for run.app endpoint', async () => {
            ...
            vi.spyOn(OAuthUtils, 'parseTokenExpiry').mockReturnValue(Date.now() + 1000000);
            const provider = new GoogleCredentialsAuthProvider(mockConfig, 'https://my-service.run.app/some-path');
            const headers = await provider.headers();
        ...
        it('returns cached id token on subsequent calls', async () => {
            vi.spyOn(OAuthUtils, 'parseTokenExpiry').mockReturnValue(Date.now() + 1000000);
            const provider = new GoogleCredentialsAuthProvider(mockConfig, 'https://my-service.run.app');
            await provider.headers();
            await provider.headers();
...
//# sourceMappingURL=google-credentials-provider.test.js.map