...
import { DEFAULT_MAX_TIME_MINUTES, DEFAULT_MAX_TURNS, getAgentCardLoadOptions, getRemoteAgentTargetUrl, } from './types.js';
describe('loader', () => {
    let tempDir;
    beforeEach(async () => {
...
---
`);
            await expect(parseAgentMarkdown(filePath)).rejects.toThrow(/agent_card_json must be valid JSON/);
        });
...
Body`);
                const error = await parseAgentMarkdown(filePath).catch((e) => e);
                expect(error).toBeInstanceOf(AgentLoadError);
                expect(error.message).toContain("Unrecognized key(s) in object: 'unknown_field'");
    ...
    describe('markdownToAgentDefinition', () => {
        ...
        it('should throw for remote agent with neither agent_card_url nor agent_card_json', () => {
            ...
            expect(() => markdownToAgentDefinition(markdown)).toThrow(/neither agent_card_json nor agent_card_url/);
        });
    });
...
---
`);
            await expect(parseAgentMarkdown(filePath)).rejects.toThrow(/Basic authentication requires "password"/);
        });
...
`);
        ...
        it('should throw an error for an unknown auth type in markdownToAgentDefinition', () => {
            ...
            expect(() => markdownToAgentDefinition(markdown)).toThrow(/Unknown auth type: some-unknown-type/);
        });
    });
    describe('getAgentCardLoadOptions', () => {
        ...
        it('should throw when neither is present', () => {
            const def = { name: 'orphan' };
            expect(() => getAgentCardLoadOptions(def)).toThrow(/Remote agent 'orphan' has neither agentCardUrl nor agentCardJson/);
        });
    });
