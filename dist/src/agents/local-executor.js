...
import { AgentTerminateMode, DEFAULT_QUERY_STRING, DEFAULT_MAX_TURNS, DEFAULT_MAX_TIME_MINUTES, SubagentActivityErrorType, SUBAGENT_REJECTED_ERROR_PREFIX, SUBAGENT_CANCELLED_ERROR_MESSAGE, } from './types.js';
...
export class LocalAgentExecutor {
     ...
     */
    static async create(definition, context, onActivity) {
    ...
    onWaitingForConfirmation) {
        ...
        try {
            ...
            logRecoveryAttempt(this.context.config, new RecoveryAttemptEvent(this.agentId, this.definition.name, reason, Date.now() - recoveryStartTime, success, turnCounter));
        }
    }
    /**
     * Runs the agent.
    ...
    async createChatObject(inputs, tools) {
        ...
        const systemInstruction = promptConfig.systemPrompt
            ...
            : undefined;
        try {
            const chat = new GeminiChat(this.executionContext, systemInstruction, [{ functionDeclarations: tools }], startHistory, undefined, undefined);
