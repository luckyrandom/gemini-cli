...
function createModifyingProcessor(id) {
    return {
        id,
...
function createThrowingProcessor(id) {
    return {
        id,
        name: 'Throwing',
        process: async () => {
...
function createMockAsyncProcessor(id, executeSpy) {
    return {
        id,
...
describe('PipelineOrchestrator (Component)', () => {
    ...
    const setupOrchestrator = (pipelines, asyncPipelines = []) => {
        const orchestrator = new PipelineOrchestrator(pipelines, asyncPipelines, env, eventBus, env.tracer);
        return orchestrator;
    };
    describe('Synchronous Pipeline Execution', () => {
        it('applies processors in sequence on matching trigger', async () => {
            ...
            const processed = await orchestrator.executeTriggerSync('new_message', [originalNode], new Set([originalNode.id]), new Set());
            expect(processed.length).toBe(1);
            const resultingNode = processed[0];
