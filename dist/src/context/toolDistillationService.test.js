...
describe('ToolOutputDistillationService', () => {
    ...
    beforeEach(() => {
        ...
        service = new ToolOutputDistillationService(mockConfig, mockGeminiClient, 'test-prompt-id');
    });
    it('should generate a structural map for oversized content within limits', async () => {
        ...
        const text = typeof result.truncatedContent === 'string'
            ...
            : result.truncatedContent[0].text;
    ...
    it('should structurally truncate functionResponse while preserving schema', async () => {
        ...
        const truncatedParts = result.truncatedContent;
        ...
        expect(resp['stdout']).toContain('[Message Normalized');
        expect(resp['stdout']).toContain('Full output saved to');
    });
...
//# sourceMappingURL=toolDistillationService.test.js.map