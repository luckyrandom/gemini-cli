...
function hashStringSlice(content, start = 0, end = 12) {
    return crypto
        .createHash('sha256')
        .update(content)
        .digest('hex')
...
export class ContextCompressionService {
    ...
    async compressHistory(history, userPrompt, abortSignal) {
        ...
        const pendingFiles = [];
        const pendingFilesSet = new Set(); // deduplicate by filepath
        for (let i = 0; i < history.length; i++) {
            const turn = history[i];
        ...
        const routingDecisions = await this.batchQueryModel(pendingFiles.map((f) => ({
            filepath: f.filepath,
            lineCount: f.lineCount,
            preview: f.preview,
        })), userPrompt, abortSignal);
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            const newParts = await Promise.all(turn.parts.map((part) => this.applyCompressionDecision(part, protectedFiles, userPrompt, abortSignal)));
            ...
            if (turn.parts) {
                for (const part of turn.parts) {
                    if (part.functionCall) {
                        ...
                        const hasMatchingResponse = nextTurn.parts.some((p) => p.functionResponse &&
    ...
    async applyCompressionDecision(part, protectedFiles, userPrompt, abortSignal) {
        const resp = part.functionResponse;
        if (!resp)
            return part;
        if (resp.name !== 'read_file' && resp.name !== 'read_many_files')
    ...
    async batchQueryModel(files, userPrompt, abortSignal) {
...
{"level":"FULL"|"PARTIAL"|"SUMMARY"|"EXCLUDED","start_line":null,"end_line":null}`;
        ...
        try {
            ...
            for (const f of files) {
                ...
                const decision = responseJson[f.filepath];
    ...
    async generateSummary(filepath, content, abortSignal) {
...
//# sourceMappingURL=contextCompressionService.js.map