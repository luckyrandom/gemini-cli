...
export function getStableId(obj, nodeIdentityMap) {
...
function isCompleteEpisode(ep) {
    return (typeof ep.id === 'string' &&
        Array.isArray(ep.concreteNodes) &&
        ep.concreteNodes.length > 0);
}
export function toGraph(history, tokenCalculator, nodeIdentityMap) {
    const episodes = [];
    let currentEpisode = null;
    const pendingCallParts = new Map();
    ...
    for (const msg of history) {
        ...
        if (msg.role === 'user') {
            ...
            const hasUserParts = msg.parts.some((p) => !!p.text || !!p.inlineData || !!p.fileData);
            if (hasToolResponses) {
                currentEpisode = parseToolResponses(msg, currentEpisode, pendingCallParts, tokenCalculator, nodeIdentityMap);
            }
            if (hasUserParts) {
                finalizeEpisode();
            ...
            currentEpisode = parseModelParts(msg, currentEpisode, pendingCallParts, nodeIdentityMap);
        }
    }
    if (currentEpisode) {
...
function parseToolResponses(msg, currentEpisode, pendingCallParts, tokenCalculator, nodeIdentityMap) {
    if (!currentEpisode) {
        currentEpisode = {
...
function parseModelParts(msg, currentEpisode, pendingCallParts, nodeIdentityMap) {
    if (!currentEpisode) {
        currentEpisode = {
            id: getStableId(msg, nodeIdentityMap),
            concreteNodes: [],
