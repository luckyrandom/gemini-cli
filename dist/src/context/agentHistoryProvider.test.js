...
import { TEXT_TRUNCATION_PREFIX, TOOL_TRUNCATION_PREFIX, truncateProportionally, } from './truncation.js';
describe('AgentHistoryProvider', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(estimateTokenCountSync).mockImplementation((parts) => parts.length * 100);
    ...
    const createMockHistory = (count) => Array.from({ length: count }).map((_, i) => ({
    ...
    it('should truncate when total tokens exceed budget, preserving structural integrity', async () => {
        ...
        vi.mocked(estimateTokenCountSync).mockImplementation((parts) => parts.length * 4000);
    ...
    it('should handle summarizer failures gracefully', async () => {
        ...
        expect(result[0].parts[0].text).toContain('[System Note: Conversation History Truncated]');
    });
    it('should pass the contextual bridge to the summarizer', async () => {
        ...
        vi.mocked(estimateTokenCountSync).mockImplementation((parts) => parts.length * 10);
        const history = [
    ...
    describe('Tiered Normalization Logic', () => {
        it('normalizes large messages incrementally: newest and exit-grace', async () => {
            ...
            vi.mocked(estimateTokenCountSync).mockImplementation((parts) => {
                if (!parts?.[0])
                    return 10;
        ...
        it('normalize function responses correctly by targeting large string values', async () => {
            ...
            expect(resp['stdout'].length).toBeLessThan(hugeValue.length);
...
//# sourceMappingURL=agentHistoryProvider.test.js.map