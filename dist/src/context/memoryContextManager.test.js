...
describe('MemoryContextManager', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(memoryDiscovery.deduplicatePathsByFileIdentity).mockImplementation(async (paths) => ({
            paths,
            identityMap: new Map(),
        }));
    });
    describe('refresh', () => {
        it('should load and format global and environment memory', async () => {
            ...
            expect(memoryDiscovery.getEnvironmentMemoryPaths).toHaveBeenCalledWith(['/app'], ['.git']);
            expect(memoryDiscovery.readGeminiMdFiles).toHaveBeenCalledWith(expect.arrayContaining([...globalPaths, ...envPaths]), 'tree', ['.git']);
        ...
        it('should deduplicate files by file identity in case-insensitive filesystems', async () => {
            ...
            await memoryContextManager.refresh();
            expect(memoryDiscovery.deduplicatePathsByFileIdentity).toHaveBeenCalledWith(expect.arrayContaining([
                '/home/user/.gemini/GEMINI.md',
                '/app/gemini.md',
                '/app/GEMINI.md',
            ]));
            expect(memoryDiscovery.readGeminiMdFiles).toHaveBeenCalledWith(['/home/user/.gemini/GEMINI.md', '/app/gemini.md'], 'tree', ['.git']);
    ...
    describe('discoverContext', () => {
        it('should discover and load new context', async () => {
            ...
            const result = await memoryContextManager.discoverContext('/app/src/file.ts', ['/app']);
            expect(memoryDiscovery.loadJitSubdirectoryMemory).toHaveBeenCalledWith('/app/src/file.ts', ['/app'], expect.any(Set), expect.any(Set), ['.git']);
            expect(result).toMatch(/--- Context from: \/app\/src\/GEMINI\.md ---/);
            expect(result).toContain('Src Content');
            expect(memoryContextManager.getLoadedPaths()).toContain('/app/src/GEMINI.md');
        });
        ...
        it('should pass custom boundary markers from config', async () => {
            ...
            expect(memoryDiscovery.loadJitSubdirectoryMemory).toHaveBeenCalledWith('/app/src/file.ts', ['/app'], expect.any(Set), expect.any(Set), customMarkers);
        });
    });
});
//# sourceMappingURL=memoryContextManager.test.js.map