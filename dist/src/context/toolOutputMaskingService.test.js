...
describe('ToolOutputMaskingService', () => {
    ...
    it('should respect remote configuration overrides', async () => {
        ...
        mockedEstimateTokenCountSync.mockImplementation((parts) => {
            ...
            const content = resp?.['output'] ?? JSON.stringify(resp);
    ...
    it('should protect the latest turn and mask older outputs beyond 50k window if total > 30k', async () => {
        ...
        mockedEstimateTokenCountSync.mockImplementation((parts) => {
            ...
            const resp = parts[0].functionResponse?.response;
            const content = resp?.['output'] ?? JSON.stringify(resp);
            if (content.includes(`<${MASKING_INDICATOR_TAG}`))
                return 100;
            if (toolName === 't1')
                return 60000;
            if (toolName === 't2')
    ...
    it('should preserve multimodal parts while masking tool responses', async () => {
        ...
        mockedEstimateTokenCountSync.mockImplementation((parts) => {
            ...
            if (content.includes(`<${MASKING_INDICATOR_TAG}`))
                return 100;
            if (parts[0].functionResponse?.name === 't1')
                return 60000;
            if (parts[0].functionResponse?.name === 'p')
                return 60000;
            return 100;
        });
        const result = await service.mask(history, mockConfig);
        ...
        expect((result.newHistory[0].parts?.[0].functionResponse?.response)['output']).toContain(`<${MASKING_INDICATOR_TAG}`);
    ...
    it('should match the expected snapshot for a masked tool output', async () => {
        ...
        mockedEstimateTokenCountSync.mockImplementation((parts) => {
            ...
            if (content.includes(`<${MASKING_INDICATOR_TAG}`))
                return 100;
            if (parts[0].functionResponse?.name === SHELL_TOOL_NAME)
                return 1000;
            if (parts[0].functionResponse?.name === 'padding')
                return 60000;
            return 10;
        });
        const result = await service.mask(history, mockConfig);
        ...
        const responseObj = result.newHistory[0].parts?.[0].functionResponse
            ?.response;
        expect(Object.keys(responseObj)).toEqual(['output']);
        const response = responseObj['output'];
    ...
    it('should not mask if masking increases token count (due to overhead)', async () => {
        ...
        mockedEstimateTokenCountSync.mockImplementation((parts) => {
            if (parts[0].functionResponse?.name === 'tiny_tool')
                return 5;
            if (parts[0].functionResponse?.name === 'padding')
                return 60000;
    ...
    it('should never mask exempt tools (like activate_skill) even if they are deep in history', async () => {
        ...
        expect((result.newHistory[0].parts?.[0].functionResponse?.response)['output']).toBe('High value instructions for skill');
        expect(result.newHistory[1].parts?.[0].functionResponse?.name).toBe(MEMORY_TOOL_NAME);
        expect((result.newHistory[1].parts?.[0].functionResponse?.response)['output']).toBe('Important user preference');
        expect(result.newHistory[2].parts?.[0].functionResponse?.name).toBe('bulky_tool');
        expect((result.newHistory[2].parts?.[0].functionResponse?.response)['output']).toContain(MASKING_INDICATOR_TAG);
    });
});
//# sourceMappingURL=toolOutputMaskingService.test.js.map