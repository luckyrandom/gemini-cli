...
import { createSyntheticHistory, createMockContextConfig, setupContextComponentTest, } from './testing/contextTestUtils.js';
describe('ContextManager Sync Pressure Barrier Tests', () => {
    ...
    it('should instantly truncate history when maxTokens is exceeded using truncate strategy', async () => {
        ...
        const { chatHistory, contextManager } = setupContextComponentTest(config, testTruncateProfile);
        ...
        const contentNodes = projection.filter((p) => p.parts && p.parts.some((part) => part.text && part.text !== 'Yield'));
...
//# sourceMappingURL=contextManager.barrier.test.js.map