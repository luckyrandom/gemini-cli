 ...
 */
export const createMockGenerateContentResponse = (text) => 
// eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
({
    candidates: [{ content: { role: 'model', parts: [{ text }] }, index: 0 }],
});
export function createDummyNode(logicalParentId, type, tokens = 100, overrides, id) {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    return {
        id: id || randomUUID(),
...
export function createDummyToolNode(logicalParentId, intentTokens = 100, obsTokens = 200, overrides, id) {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    return {
        id: id || randomUUID(),
        episodeId: logicalParentId,
...
export function createMockEnvironment(overrides) {
    ...
    const env = new ContextEnvironmentImpl(llmClient, 'mock-session', 'mock-prompt-id', '/tmp/.gemini/trace', '/tmp/.gemini/tool-outputs', tracer, 1, eventBus);
    if (overrides) {
 ...
 */
export function createMockProcessArgs(targets, bufferNodes = [], inboxMessages = []) {
    return {
        targets,
...
export function createSyntheticHistory(numTurns, tokensPerTurn) {
    const history = [];
 ...
 */
export function createMockContextConfig(overrides, llmClientOverride) {
    const defaultConfig = {
        isContextManagementEnabled: vi.fn().mockReturnValue(true),
        storage: {
            getProjectTempDir: vi.fn().mockReturnValue('/tmp/gemini-test'),
        },
        getBaseLlmClient: vi.fn().mockReturnValue(llmClientOverride || {
 ...
 */
export function setupContextComponentTest(config, sidecarOverride) {
    ...
    const eventBus = new ContextEventBus();
    const env = new ContextEnvironmentImpl(config.getBaseLlmClient(), 'test prompt-id', 'test-session', '/tmp', '/tmp/gemini-test', tracer, 1, eventBus);
...
//# sourceMappingURL=contextTestUtils.js.map