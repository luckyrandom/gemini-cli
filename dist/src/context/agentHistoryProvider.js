...
import { MIN_TARGET_TOKENS, MIN_CHARS_FOR_TRUNCATION, TEXT_TRUNCATION_PREFIX, estimateCharsFromTokens, truncateProportionally, normalizeFunctionResponse, } from './truncation.js';
export class AgentHistoryProvider {
    providerConfig;
    config;
    // TODO(joshualitt): just pass the BaseLlmClient instead of the whole Config.
    constructor(providerConfig, config) {
    ...
    async manageHistory(history, abortSignal) {
        ...
        const totalTokens = estimateTokenCountSync(normalizedHistory.flatMap((c) => c.parts || []));
    ...
    enforceMessageSizeLimits(history) {
        ...
        const newHistory = history.map((msg, i) => {
            const targetTokens = i < graceStartIndex
    ...
    normalizeMessage(msg, targetTokens) {
        ...
        for (const part of msg.parts || []) {
            if (part.text) {
                ...
                if (part.text.length > targetChars &&
...
//# sourceMappingURL=agentHistoryProvider.js.map