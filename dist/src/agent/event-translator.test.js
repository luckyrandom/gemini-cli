...
import { translateEvent, createTranslationState, mapFinishReason, mapHttpToGrpcStatus, mapError, mapUsage, } from './event-translator.js';
...
describe('translateEvent', () => {
    ...
    describe('AgentExecutionBlocked events', () => {
        ...
        it('uses systemMessage in the final error message when available', () => {
            ...
            const result = translateEvent(event, state);
            const err = result[0];
            expect(err.message).toBe('Agent execution blocked: Blocked by policy hook');
        });
    });
    ...
    describe('Events with no output', () => {
        ...
        it('returns empty for ChatCompressed with null', () => {
            const result = translateEvent({ type: GeminiEventType.ChatCompressed, value: null }, state);
            expect(result).toEqual([]);
        });
    ...
    describe('Event IDs', () => {
        ...
        it('includes streamId in events', () => {
            const events = translateEvent({ type: GeminiEventType.Content, value: 'hi' }, state);
...
//# sourceMappingURL=event-translator.test.js.map