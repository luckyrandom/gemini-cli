...
function makeToolRequest(callId, name) {
    return {
        callId,
        name,
        args: {},
...
function makeMessageSend(text, displayContent) {
    return {
        message: {
...
function makeCompletedToolCall(callId, name, responseText) {
    return {
...
async function collectEvents(session, options) {
    const events = [];
...
describe('LegacyAgentSession', () => {
    ...
    describe('send', () => {
        it('returns streamId', async () => {
            const sendMock = deps.client.sendMessageStream;
            sendMock.mockReturnValue(makeStream([
        ...
        it('records the sent user message in the trajectory before send resolves', async () => {
            ...
            const userMessage = session.events.find((e) => e.type === 'message' && e.role === 'user' && e.streamId === streamId);
            expect(userMessage?.content).toEqual([
                { type: 'text', text: 'raw input' },
            ]);
            expect(userMessage?._meta).toEqual({ source: 'user-test' });
            await vi.advanceTimersByTimeAsync(0);
            expect(sendMock).toHaveBeenCalledWith([{ text: 'hi' }], expect.any(AbortSignal), 'test-prompt', undefined, false, 'raw input');
        ...
        it('throws for non-message payloads', async () => {
            const session = new LegacyAgentSession(deps);
            await expect(session.send({ update: { title: 'test' } })).rejects.toThrow('only supports message sends');
        });
        it('throws if send is called while a stream is active', async () => {
            let resolveHang;
            const sendMock = deps.client.sendMessageStream;
            sendMock.mockReturnValue((async function* () {
            ...
            await vi.advanceTimersByTimeAsync(0);
            await expect(session.send(makeMessageSend('second'))).rejects.toThrow('cannot be called while a stream is active');
            resolveHang?.();
            await collectEvents(session, { streamId: streamId ?? undefined });
        });
        it('creates a new streamId after the previous stream completes', async () => {
            ...
            const userMessages = session.events.filter((e) => e.type === 'message' && e.role === 'user');
            expect(first.streamId).not.toBe(second.streamId);
            expect(userMessages.some((e) => e.streamId === first.streamId &&
                e.content[0]?.type === 'text' &&
                e.content[0].text === 'first')).toBe(true);
            expect(userMessages.some((e) => e.streamId === second.streamId &&
                e.content[0]?.type === 'text' &&
                e.content[0].text === 'second')).toBe(true);
    ...
    describe('stream - basic flow', () => {
        it('emits agent_start, content messages, and agent_end', async () => {
            ...
            const messages = events.filter((e) => e.type === 'message' && e.role === 'agent');
            expect(messages).toHaveLength(2);
            expect(messages[0]?.content).toEqual([{ type: 'text', text: 'Hello' }]);
            const streamEnd = events.find((e) => e.type === 'agent_end');
    ...
    describe('stream - tool calls', () => {
        it('handles a tool call round-trip', async () => {
            ...
            sendMock.mockReturnValueOnce(makeStream([
                {
            ...
            const scheduleMock = deps.scheduler.schedule;
            ...
            const toolResp = events.find((e) => e.type === 'tool_response');
        ...
        it('treats fatal tool errors as tool_response followed by agent_end failed', async () => {
            ...
            expect(events.some((e) => e.type === 'error' && e.fatal === true)).toBe(false);
            const streamEnd = events.findLast((e) => e.type === 'agent_end');
    ...
    describe('stream - terminal events', () => {
        ...
        it('handles AgentExecutionBlocked as non-terminal and continues the stream', async () => {
            ...
            await session.send(makeMessageSend('hi'));
            const events = await collectEvents(session);
            const blocked = events.find((e) => e.type === 'error' && e._meta?.['code'] === 'AGENT_EXECUTION_BLOCKED');
            expect(blocked?.fatal).toBe(false);
            expect(blocked?.message).toBe('Agent execution blocked: Blocked by hook');
            const messages = events.filter((e) => e.type === 'message' && e.role === 'agent');
            expect(messages.some((message) => message.content[0]?.type === 'text' &&
                message.content[0].text === 'Final answer')).toBe(true);
            const streamEnd = events.find((e) => e.type === 'agent_end');
            expect(streamEnd?.reason).toBe('completed');
        });
        ...
        it('handles LoopDetected as non-terminal warning event', async () => {
            ...
            await session.send(makeMessageSend('hi'));
            const events = await collectEvents(session);
            const warning = events.find((e) => e.type === 'error' && e._meta?.['code'] === 'LOOP_DETECTED');
            expect(warning).toBeDefined();
            expect(warning?.fatal).toBe(false);
            // Stream should have continued — content after loop detected
            const messages = events.filter((e) => e.type === 'message' && e.role === 'agent');
            expect(messages.some((m) => m.content[0]?.type === 'text' &&
                m.content[0].text === 'continuing after loop')).toBe(true);
    ...
    describe('stream - max turns', () => {
        it('emits agent_end with max_turns when the session turn limit is exceeded', async () => {
            const configMock = deps.config.getMaxSessionTurns;
    ...
    describe('abort', () => {
        it('treats abort before the first model event as aborted without fatal error', async () => {
            ...
            const streamEnd = events.findLast((event) => event.type === 'agent_end');
        ...
        it('treats abort during pending scheduler work as aborted without fatal error', async () => {
            ...
            const scheduleMock = deps.scheduler.schedule;
            scheduleMock.mockReturnValue(new Promise((resolve) => {
    ...
    describe('subscription and stream scoping', () => {
        ...
        it('subscribe is live-only and does not replay old history when idle', async () => {
            ...
            unsubscribe();
            expect(liveEvents.length).toBeGreaterThan(0);
            expect(liveEvents.every((event) => event.streamId === second.streamId)).toBe(true);
            expect(liveEvents.some((event) => event.type === 'message' &&
                event.role === 'user' &&
                event.content[0]?.type === 'text' &&
                event.content[0].text === 'first request')).toBe(false);
        });
        it('streams only the requested streamId', async () => {
            ...
            expect(firstStreamEvents.every((event) => event.streamId === first.streamId)).toBe(true);
            expect(firstStreamEvents.some((e) => e.type === 'message' &&
                e.role === 'agent' &&
                e.content[0]?.type === 'text' &&
                e.content[0].text === 'first answer')).toBe(true);
            expect(firstStreamEvents.some((e) => e.type === 'message' &&
                e.role === 'agent' &&
                e.content[0]?.type === 'text' &&
                e.content[0].text === 'second answer')).toBe(false);
        });
        it('resumes from eventId within the same stream only', async () => {
            ...
            await collectEvents(session);
            const firstAgentMessage = session.events.find((e) => e.type === 'message' &&
                e.role === 'agent' &&
            ...
            expect(resumedEvents.every((event) => event.streamId === first.streamId)).toBe(true);
            expect(resumedEvents.map((event) => event.type)).toEqual(['agent_end']);
            expect(resumedEvents.some((e) => e.type === 'message' &&
                e.role === 'agent' &&
                e.content[0]?.type === 'text' &&
                e.content[0].text === 'second answer')).toBe(false);
        });
    ...
    describe('_emitErrorAndAgentEnd metadata', () => {
        ...
        it('preserves status in _meta for errors with status property', async () => {
            ...
            statusError.status = 'RESOURCE_EXHAUSTED';
            sendMock.mockImplementation(() => {
                throw statusError;
            });
            const session = new LegacyAgentSession(deps);
...
//# sourceMappingURL=legacy-agent-session.test.js.map