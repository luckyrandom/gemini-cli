...
export type AgentEventData<EventType extends keyof AgentEvents = keyof AgentEvents> = AgentEvents[EventType] & {
    type: EventType;
};
/**
...
export type AgentEvent<EventType extends keyof AgentEvents = keyof AgentEvents> = {
    [K in EventType]: AgentEventCommon & AgentEvents[K] & {
        type: K;
    };
}[EventType];
...
export interface ErrorData {
    status: 'INVALID_ARGUMENT' | 'FAILED_PRECONDITION' | 'OUT_OF_RANGE' | 'UNAUTHENTICATED' | 'PERMISSION_DENIED' | 'NOT_FOUND' | 'ABORTED' | 'ALREADY_EXISTS' | 'RESOURCE_EXHAUSTED' | 'CANCELLED' | 'UNKNOWN' | 'INTERNAL' | 'DATA_LOSS' | 'UNIMPLEMENTED' | 'UNAVAILABLE' | 'DEADLINE_EXCEEDED' | (string & {});
...
export type StreamEndReason = 'completed' | 'failed' | 'aborted' | 'max_turns' | 'max_budget' | 'max_time' | 'refusal' | 'elicitation' | (string & {});
