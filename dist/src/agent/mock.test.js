...
const waitForStreamEnd = (session) => new Promise((resolve) => {
    const events = [];
...
describe('MockAgentProtocol', () => {
    ...
    it('should handle abort on a waiting stream', async () => {
        ...
        const events = [];
        let resolveStream;
        const streamPromise = new Promise((res) => {
            resolveStream = res;
        });
        session.subscribe((e) => {
            events.push(e);
        ...
        expect(finalEvents[3].reason).toBe('aborted');
    });
    ...
    it('should handle pushToStream with close option', async () => {
        ...
        expect(events[3].reason).toBe('completed');
    });
    ...
    it('should throw on action', async () => {
        const session = new MockAgentProtocol();
        await expect(session.send({ action: { type: 'foo', data: {} } })).rejects.toThrow('Actions not supported in MockAgentProtocol: foo');
    });
});
