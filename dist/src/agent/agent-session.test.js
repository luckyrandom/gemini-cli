...
describe('AgentSession', () => {
    it('should passthrough simple methods', async () => {
        ...
        await session.send({ update: { title: 't' } });
        await session.abort();
        expect(session.events.some((e) => e.type === 'agent_end' && e.reason === 'aborted')).toBe(true);
    });
    ...
    describe('stream()', () => {
        ...
        it('should complete immediately when resuming from agent_end', async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, 10));
            const endEvent = session.events.findLast((event) => event.type === 'agent_end' && event.streamId === streamId);
            expect(endEvent).toBeDefined();
            const iterator = session
                .stream({ eventId: endEvent.id })[Symbol.asyncIterator]();
        ...
        it('should throw for an unknown eventId', async () => {
            ...
            const session = new AgentSession(protocol);
            const iterator = session
                .stream({ eventId: 'missing-event' })[Symbol.asyncIterator]();
        ...
        it('should throw when resuming from an event before agent_start on a stream with no agent activity', async () => {
            ...
            const updateEvent = session.events.find((event) => event.type === 'session_update');
            expect(updateEvent).toBeDefined();
            const iterator = session
                .stream({ eventId: updateEvent.id })[Symbol.asyncIterator]();
            await expect(iterator.next()).rejects.toThrow(`Cannot resume from eventId ${updateEvent.id} before agent_start for stream ${updateEvent.streamId}`);
        });
        it('should replay from agent_start when resuming from a pre-agent_start event after activity is in history', async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, 10));
            const userMessage = session.events.find((event) => event.type === 'message' && event.role === 'user');
        ...
        it('should throw when resuming from a pre-agent_start event before activity is in history', async () => {
            ...
            const session = new AgentSession(protocol);
            const iterator = session
                .stream({ eventId: 'e-1' })[Symbol.asyncIterator]();
            await expect(iterator.next()).rejects.toThrow('Cannot resume from eventId e-1 before agent_start for stream stream-1');
        });
        it('should resume from an in-stream event within the same stream only', async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, 10));
            const resumeEvent = session.events.find((event) => event.type === 'message' &&
            ...
            const resumedMessage = streamedEvents[0];
        ...
        it('should replay events for streamId starting with agent_start', async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, 10));
            const allEvents = session.events;
            const startEventIndex = allEvents.findIndex((e) => e.type === 'agent_start' && e.streamId === streamId);
        ...
        it('should not drop agent_end that arrives while replay events are being yielded', async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, 10));
            const iterator = session
                .stream({ streamId: streamId })[Symbol.asyncIterator]();
