...
describe('CheckerRegistry', () => {
    ...
    it('should resolve built-in in-process checkers', () => {
        const allowedPathChecker = registry.resolveInProcess(InProcessCheckerType.ALLOWED_PATH);
        expect(allowedPathChecker).toBeInstanceOf(AllowedPathChecker);
        const consecaChecker = registry.resolveInProcess(InProcessCheckerType.CONSECA);
