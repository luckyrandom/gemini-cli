...
export class ConsecaSafetyChecker {
    ...
    async check(input) {
        ...
        logConsecaVerdict(this.context.config, new ConsecaVerdictEvent(userPrompt || '', JSON.stringify(this.currentPolicy || {}), JSON.stringify(input.toolCall), result.decision, result.reason || '', 'error' in result ? result.error : undefined));
        return result;
    }
