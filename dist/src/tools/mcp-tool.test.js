...
describe('generateValidName', () => {
    ...
    it('should replace invalid characters with underscores', () => {
        expect(generateValidName('invalid-name with spaces')).toBe('mcp_invalid-name_with_spaces');
    });
    it('should truncate long names', () => {
        expect(generateValidName('x'.repeat(80))).toBe('mcp_xxxxxxxxxxxxxxxxxxxxxxxxxx...xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
    });
...
describe('DiscoveredMCPTool', () => {
    ...
    describe('getDisplayTitle and getExplanation', () => {
        const commandTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, {
            type: 'object',
            properties: { command: { type: 'string' }, path: { type: 'string' } },
            required: ['command'],
        }, createMockMessageBus(), undefined, undefined, undefined, undefined, undefined, undefined);
        ...
        it('should truncate and summarize long json payloads for getExplanation', () => {
            ...
            expect(explanation).toMatch(/^\[Payload omitted due to length with parameters: command, text, other\]$/);
        });
    });
    describe('execute', () => {
        ...
        it('should propagate rejection if mcpTool.callTool rejects', async () => {
            ...
            const invocation = tool.build(params);
            await expect(invocation.execute({ abortSignal: new AbortController().signal })).rejects.toThrow(expectedError);
        });
        it.each([
        ...
        describe('AbortSignal support', () => {
            ...
            it('should abort immediately if signal is already aborted', async () => {
                ...
                controller.abort();
                const invocation = tool.build(params);
                await expect(invocation.execute({ abortSignal: controller.signal })).rejects.toThrow('Tool call aborted');
            ...
            it('should handle callTool rejection with abort signal', async () => {
                ...
                mockCallTool.mockRejectedValue(expectedError);
                const invocation = tool.build(params);
                await expect(invocation.execute({ abortSignal: controller.signal })).rejects.toThrow(expectedError);
            });
            it.each([
                {
...
describe('MCP Tool Naming Regression Fixes', () => {
    ...
    describe('DiscoveredMCPTool qualified names', () => {
        it('should generate a valid qualified name even with spaces in server name', () => {
            const tool = new DiscoveredMCPTool({}, 'My Server', 'my-tool', 'desc', {}, {});
        ...
        it('should handle long server and tool names in qualified name', () => {
            ...
            const tool = new DiscoveredMCPTool({}, serverName, toolName, 'desc', {}, {});
            const qn = tool.getFullyQualifiedName();
            expect(qn.length).toBeLessThanOrEqual(63);
            expect(qn).toContain('...');
        });
        it('should handle server names starting with digits', () => {
            const tool = new DiscoveredMCPTool({}, '123-server', 'tool', 'desc', {}, {});
            const qn = tool.getFullyQualifiedName();
