...
import { ListBackgroundProcessesTool, ReadBackgroundOutputTool, } from './shellBackgroundTools.js';
...
describe('Background Tools', () => {
    ...
    it('list_background_processes should return empty message when no processes', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        invocation.context = { config: { getSessionId: () => 'default' } };
    ...
    it('read_background_output should return Access Denied for processes in other sessions', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ShellExecutionService.backgroundProcessHistory.set('other-session', history);
        const invocation = readTool.build({ pid });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        invocation.context = { config: { getSessionId: () => 'default' } }; // Asking for PID from another session
        const result = await invocation.execute({
            abortSignal: new AbortController().signal,
        });
        expect(result.error).toBeDefined();
...
//# sourceMappingURL=shellBackgroundTools.test.js.map