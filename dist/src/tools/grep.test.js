...
describe('GrepTool', () => {
    ...
    describe('execute', () => {
        it('should find matches for a simple pattern in all files', async () => {
            ...
            expect(result.returnDisplay?.summary).toBe('Found 3 matches');
        }, 30000);
        ...
        it('should find matches in a specific path', async () => {
            ...
            expect(result.llmContent).toContain('L1: another world in sub dir');
            expect(result.returnDisplay?.summary).toBe('Found 1 match');
        }, 30000);
    ...
    describe('multi-directory workspace', () => {
        ...
        it('should respect total_max_matches and truncate results', async () => {
            ...
            expect(result.returnDisplay?.summary).toBe('Found 2 matches (limited)');
        });
        ...
        it('should truncate excessively long lines', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, 'longline.txt'), `Target match ${longString}`);
            const params = { pattern: 'Target match' };
            const invocation = grepTool.build(params);
            const result = await invocation.execute({ abortSignal });
