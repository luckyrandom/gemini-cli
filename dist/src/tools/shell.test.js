...
describe('ShellTool', () => {
    ...
    describe('execute', () => {
        ...
        itWindowsOnly('should not wrap command on windows', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith('dir', tempRootDir, expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({
                pager: 'cat',
                sanitizationConfig: {},
        ...
        it('should summarize output when configured', async () => {
            ...
            const result = await promise;
            expect(summarizer.summarizeToolOutput).toHaveBeenCalledWith(mockConfig, { model: 'summarizer-shell' }, expect.any(String), mockConfig.geminiClient, mockAbortSignal);
    ...
    describe('getDisplayTitle and getExplanation', () => {
        ...
        it('should return the context for getExplanation', () => {
            ...
            expect(invocation.getExplanation?.()).toBe('[in foo/bar] (prints hello) [background]');
        });
    ...
    describe('sandbox heuristics', () => {
        ...
        it('should suggest proactive permissions for npm commands', async () => {
            ...
            expect(details.additionalPermissions.fileSystem.read).toContain(path.join(homeDir, '.npm'));
            expect(details.additionalPermissions.fileSystem.read).toContain(path.join(homeDir, '.cache'));
            expect(details.additionalPermissions.fileSystem.write).toContain(path.join(homeDir, '.npm'));
        });
        ...
        it('should proactively suggest expansion for npm install in confirmation', async () => {
            ...
            mockHomedir.mockReturnValue(homeDir);
            const invocation = shellTool.build({ command: 'npm install' });
            const details = (await invocation.shouldConfirmExecute(new AbortController().signal, 'ask_user'));
