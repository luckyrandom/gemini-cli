...
export function isWriteFileToolParams(args) {
...
class WriteFileToolInvocation extends BaseToolInvocation {
    ...
    constructor(config, params, messageBus, toolName, displayName) {
        super(params, messageBus, toolName, displayName, undefined, undefined, true, () => this.config.getApprovalMode());
        this.config = config;
        if (this.config.isPlanMode()) {
            const safeFilename = path.basename(this.params.file_path);
            this.resolvedPath = path.join(this.config.storage.getPlansDir(), safeFilename);
        }
        else {
