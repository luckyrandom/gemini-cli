            ...
            if (os.platform() !== 'win32') {
                ...
                if (tempFileExists) {
                    ...
                    for (const line of pgrepLines) {
                        if (!/^\d+$/.test(line)) {
                            if (line.includes('sysmond service not found') ||
...
export class ShellTool extends BaseDeclarativeTool {
    ...
    constructor(context, messageBus) {
        ...
        const definition = getShellDefinition(context.config.getEnableInteractiveShell(), context.config.getEnableShellOutputEfficiency(), context.config.getSandboxEnabled());
    ...
    validateToolParamValues(params) {
        ...
        if (params.dir_path) {
            const resolvedPath = path.resolve(this.context.config.getTargetDir(), params.dir_path);
