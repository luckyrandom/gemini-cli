...
describe('jit-context', () => {
    describe('discoverJitContext', () => {
        ...
        it('should call memoryContextManager.discoverContext with correct args when JIT is enabled', async () => {
            vi.mocked(mockConfig.isJitContextEnabled).mockReturnValue(true);
            vi.mocked(mockMemoryContextManager.discoverContext).mockResolvedValue('Subdirectory context content');
            const result = await discoverJitContext(mockConfig, '/app/src/file.ts');
            expect(mockMemoryContextManager.discoverContext).toHaveBeenCalledWith('/app/src/file.ts', ['/app']);
        ...
        it('should pass all workspace directories as trusted roots', async () => {
            ...
            expect(mockMemoryContextManager.discoverContext).toHaveBeenCalledWith('/app/src/file.ts', ['/app', '/lib']);
        });
        ...
        it('should return empty string when discoverContext throws', async () => {
            vi.mocked(mockConfig.isJitContextEnabled).mockReturnValue(true);
            vi.mocked(mockMemoryContextManager.discoverContext).mockRejectedValue(new Error('Permission denied'));
            const result = await discoverJitContext(mockConfig, '/app/src/file.ts');
            expect(result).toBe('');
        });
    });
...
//# sourceMappingURL=jit-context.test.js.map