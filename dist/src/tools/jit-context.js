 ...
 */
export async function discoverJitContext(config, accessedPath) {
 ...
 */
export function appendJitContext(llmContent, jitContext) {
    if (!jitContext) {
        return llmContent;
    }
 ...
 */
export function appendJitContextToParts(llmContent, jitContext) {
    const jitPart = {
        text: `${JIT_CONTEXT_PREFIX}${jitContext}${JIT_CONTEXT_SUFFIX}`,
    };
    const existingParts = Array.isArray(llmContent)
        ? llmContent
        : [llmContent];
    return [...existingParts, jitPart];
}
//# sourceMappingURL=jit-context.js.map