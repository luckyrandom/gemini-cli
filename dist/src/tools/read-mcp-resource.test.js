...
describe('ReadMcpResourceTool', () => {
    ...
    beforeEach(() => {
        ...
        tool = new ReadMcpResourceTool(mockContext, createMockMessageBus());
    });
    it('should successfully read a resource', async () => {
        ...
        const invocation = tool.createInvocation({ uri });
        // Verify description
        expect(invocation.getDescription()).toBe(`Read MCP resource "${resourceName}" from server "${serverName}"`);
    ...
    it('should pass raw URI to client when using qualified URI', async () => {
        ...
        mockMcpManager.getClient.mockReturnValue(mockClient);
        const invocation = tool.createInvocation({ uri: qualifiedUri });
        const result = (await invocation.execute({ abortSignal }));
    ...
    it('should return error if MCP Client Manager not available', async () => {
        ...
        const invocation = tool.createInvocation({ uri: 'uri' });
        const result = (await invocation.execute({ abortSignal }));
...
//# sourceMappingURL=read-mcp-resource.test.js.map