...
import { MemoryTool, setGeminiMdFilename, getCurrentGeminiMdFilename, getAllGeminiMdFilenames, DEFAULT_CONTEXT_FILENAME, getProjectMemoryFilePath, } from './memoryTool.js';
...
describe('MemoryTool', () => {
    ...
    describe('project-scope memory', () => {
        const mockProjectMemoryDir = path.join('/mock', '.gemini', 'memory', 'test-project');
        ...
        it('should reject scope=project when storage is not initialized', () => {
            ...
            const params = { fact: 'project fact', scope: 'project' };
            expect(memoryToolNoStorage.validateToolParams(params)).toBe('Project-level memory is not available: storage is not initialized.');
        });
        it('should write to global path when scope is not specified', async () => {
            ...
            await invocation.execute({ abortSignal: mockAbortSignal });
            const expectedFilePath = path.join(os.homedir(), GEMINI_DIR, getCurrentGeminiMdFilename());
            expect(fs.writeFile).toHaveBeenCalledWith(expectedFilePath, expect.any(String), 'utf-8');
        });
        it('should write to project memory path when scope is project', async () => {
            ...
            const invocation = memoryToolWithStorage.build(params);
            await invocation.execute({ abortSignal: mockAbortSignal });
            const expectedFilePath = path.join(mockProjectMemoryDir, getCurrentGeminiMdFilename());
            expect(fs.mkdir).toHaveBeenCalledWith(mockProjectMemoryDir, {
                recursive: true,
            });
            expect(fs.writeFile).toHaveBeenCalledWith(expectedFilePath, expect.stringContaining('- project-specific fact'), 'utf-8');
        });
        it('should use project path in confirmation details when scope is project', async () => {
            ...
            expect(result).not.toBe(false);
            if (result && result.type === 'edit') {
                expect(result.fileName).toBe(getProjectMemoryFilePath(createMockStorage()));
