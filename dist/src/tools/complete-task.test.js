...
describe('CompleteTaskTool', () => {
    ...
    describe('Default Configuration (no outputConfig)', () => {
        ...
        it('should fail validation if result is only whitespace', () => {
            ...
            expect(result).toContain('Missing required "result" argument. You must provide your findings when calling complete_task.');
        });
    ...
    describe('Structured Configuration (with outputConfig)', () => {
        ...
        it('should generate schema based on outputConfig', () => {
            ...
            expect(toolSchema.parametersJsonSchema).toHaveProperty('properties.my_output.type', 'object');
            expect(toolSchema.parametersJsonSchema).toHaveProperty('properties.my_output.properties.report');
            expect(toolSchema.parametersJsonSchema).toHaveProperty('properties.my_output.properties.score');
            expect(toolSchema.parametersJsonSchema).toHaveProperty('required', expect.arrayContaining(['my_output']));
        });
        ...
        it('should execute and return structured data', async () => {
            ...
            expect(result.data?.['submittedOutput']).toBe(JSON.stringify(outputValue, null, 2));
        });
        it('should use processOutput if provided', async () => {
            const processOutput = (val) => `Score was ${val.score}`;
            const toolWithProcess = new CompleteTaskTool(mockMessageBus, outputConfig, processOutput);
            const outputValue = { report: 'Final findings', score: 42 };
