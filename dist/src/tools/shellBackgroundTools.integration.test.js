...
describe('Background Tools Integration', () => {
    ...
    it('should support interaction cycle: start background -> list -> read logs', async () => {
        ...
        const realHandle = await ShellExecutionService.execute(commandString, process.cwd(), () => { }, controller.signal, true, {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        listInvocation.context = {
            config: { getSessionId: () => 'default' },
        };
        const listResult = await listInvocation.execute({
            abortSignal: new AbortController().signal,
        });
        expect(listResult.llmContent).toContain(`[PID ${pid}] RUNNING: \`node continuous_log\``);
...
//# sourceMappingURL=shellBackgroundTools.integration.test.js.map