...
class ListBackgroundProcessesInvocation extends BaseToolInvocation {
    context;
    constructor(context, params, messageBus, toolName, toolDisplayName) {
    ...
    async execute({ abortSignal: _signal }) {
        ...
        const lines = processes.map((p) => `- [PID ${p.pid}] ${p.status.toUpperCase()}: \`${p.command}\`${p.exitCode !== undefined ? ` (Exit Code: ${p.exitCode})` : ''}${p.signal ? ` (Signal: ${p.signal})` : ''}`);
...
export class ListBackgroundProcessesTool extends BaseDeclarativeTool {
    ...
    constructor(context, messageBus) {
        super(ListBackgroundProcessesTool.Name, 'List Background Processes', 'Lists all active and recently completed background shell processes orchestrating by the agent.', Kind.Read, {
            type: 'object',
            properties: {},
        }, messageBus);
        this.context = context;
...
class ReadBackgroundOutputInvocation extends BaseToolInvocation {
    ...
    async execute({ abortSignal: _signal }) {
        ...
        try {
            const fileHandle = await fs.promises.open(logPath, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW);
            let content = '';
            let position = 0;
            try {
            ...
            const header = requestedLinesCount < logLines.length
...
export class ReadBackgroundOutputTool extends BaseDeclarativeTool {
    ...
    constructor(context, messageBus) {
        super(ReadBackgroundOutputTool.Name, 'Read Background Output', 'Reads the output log of a background shell process. Support reading tail snapshot.', Kind.Read, {
            type: 'object',
            properties: {
                pid: {
                    type: 'integer',
...
//# sourceMappingURL=shellBackgroundTools.js.map