...
export class McpClientManager {
    ...
    async disconnectClient(clientKey, skipRefresh = false) {
        ...
        if (existing) {
            ...
            try {
                ...
                debugLogger.warn(`Error stopping client '${serverName}': ${getErrorMessage(error)}`);
            }
            finally {
                if (!skipRefresh) {
    ...
    mergeMcpConfigs(base, override) {
        ...
        if (base.includeTools && override.includeTools) {
            includeTools = base.includeTools.filter((t) => override.includeTools.includes(t));
    ...
    async maybeDiscoverMcpServer(name, config, registries) {
        const existingConfig = this.allServerConfigs.get(name);
        if (existingConfig?.extension?.id &&
            config.extension?.id &&
        ...
        if (!registries) {
            const existingSameName = Array.from(this.clients.values()).find((c) => c.getServerName() === name);
            if (existingSameName) {
        ...
        const currentDiscoveryPromise = new Promise((resolve) => {
            void (async () => {
                try {
                    let client = existing;
                    if (!client) {
                        client = new McpClient(name, finalConfig, this.cliConfig.getWorkspaceContext(), this.cliConfig, this.cliConfig.getDebugMode(), this.clientVersion, async () => {
                    ...
                    const errorMessage = getErrorMessage(error);
                    this.emitDiagnostic('error', `Fatal error ensuring MCP server '${name}' is connected: ${errorMessage}`, error);
                }
                finally {
                    resolve();
                }
