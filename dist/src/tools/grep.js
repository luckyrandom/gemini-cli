...
class GrepToolInvocation extends BaseToolInvocation {
    ...
    async execute({ abortSignal: signal }) {
        try {
            ...
            const timeoutMs = configTimeout && configTimeout > DEFAULT_SEARCH_TIMEOUT_MS
                ? configTimeout
        ...
        try {
            ...
            if (sandboxManager) {
                try {
                    ...
                    debugLogger.debug(`[GrepTool] Sandbox preparation failed for '${command}':`, err);
                }
            }
            try {
...
export class GrepTool extends BaseDeclarativeTool {
    ...
    constructor(config, messageBus) {
        super(GrepTool.Name, GREP_DISPLAY_NAME, GREP_DEFINITION.base.description, Kind.Search, GREP_DEFINITION.base.parametersJsonSchema, messageBus, true, false);
        this.config = config;
    }
    /**
