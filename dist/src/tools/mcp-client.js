...
import { ListResourcesResultSchema, ListRootsRequestSchema, ReadResourceResultSchema, ResourceListChangedNotificationSchema, ToolListChangedNotificationSchema, ErrorCode, McpError, PromptListChangedNotificationSchema, ProgressNotificationSchema, } from '@modelcontextprotocol/sdk/types.js';
...
import {} from '../resources/resource-registry.js';
...
(function (MCPServerStatus) {
    ...
    MCPServerStatus["BLOCKED"] = "blocked";
            ...
            do {
                ...
                try {
                    ...
                    for (const registries of this.registeredRegistries) {
                        ...
                        const resourceMatch = newResources.length === currentResources.length &&
                            newResources.every((nr) => currentResources.some((cr) => cr.uri === nr.uri));
                ...
                try {
                    ...
                    for (const registries of this.registeredRegistries) {
                        ...
                        const currentPrompts = registries.promptRegistry.getPromptsByServer(this.serverName) ||
                            [];
                        const promptsMatch = newPrompts.length === currentPrompts.length &&
                            newPrompts.every((np) => currentPrompts.some((cp) => cp.name === np.name));
            ...
            do {
                ...
                try {
                    for (const registries of this.registeredRegistries) {
                        ...
                        const toolNamesMatch = newTools.length === currentTools.length &&
                            newTools.every((nt) => currentTools.some((ct) => ct.name === nt.name ||
