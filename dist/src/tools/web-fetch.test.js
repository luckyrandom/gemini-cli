...
import { WebFetchTool, parsePrompt, convertGithubUrlToRaw, normalizeUrl, } from './web-fetch.js';
 ...
 */
const mockFetch = (url, response) => vi
...
describe('convertGithubUrlToRaw', () => {
    it('should convert valid github blob urls', () => {
        expect(convertGithubUrlToRaw('https://github.com/user/repo/blob/main/README.md')).toBe('https://raw.githubusercontent.com/user/repo/main/README.md');
    });
    it('should not convert non-blob github urls', () => {
        expect(convertGithubUrlToRaw('https://github.com/user/repo')).toBe('https://github.com/user/repo');
    });
    it('should not convert urls with similar domain names', () => {
        expect(convertGithubUrlToRaw('https://mygithub.com/user/repo/blob/main')).toBe('https://mygithub.com/user/repo/blob/main');
    });
    it('should only replace the /blob/ that separates repo from branch', () => {
        expect(convertGithubUrlToRaw('https://github.com/blob/repo/blob/main/test.ts')).toBe('https://raw.githubusercontent.com/blob/repo/main/test.ts');
    });
    it('should not convert urls if blob is not in path', () => {
        expect(convertGithubUrlToRaw('https://github.com/user/repo/tree/main')).toBe('https://github.com/user/repo/tree/main');
    });
    it('should handle invalid urls gracefully', () => {
...
describe('WebFetchTool', () => {
    ...
    describe('validateToolParamValues', () => {
        ...
        describe('experimental mode', () => {
            ...
            it('should throw if url is missing', () => {
                const tool = new WebFetchTool(mockConfig, bus);
                expect(() => tool.build({ prompt: 'foo' })).toThrow("params must have required property 'url'");
            });
            it('should throw if url is invalid', () => {
                const tool = new WebFetchTool(mockConfig, bus);
                expect(() => tool.build({ url: 'not-a-url' })).toThrow('Invalid URL: "not-a-url"');
            });
    ...
    describe('execute', () => {
        ...
        it('should skip rate-limited URLs but fetch others', async () => {
            ...
            expect(result.llmContent).toContain('[Rate limit exceeded] https://ratelimit-multi.com/');
        });
        it('should skip private or local URLs but fetch others and log telemetry', async () => {
            vi.mocked(fetchUtils.isPrivateIp).mockImplementation((url) => url === 'https://private.com/');
            const tool = new WebFetchTool(mockConfig, bus);
            const params = {
                prompt: 'fetch https://private.com and https://healthy.com and http://localhost',
            };
            const invocation = tool.build(params);
            mockGenerateContent.mockResolvedValueOnce({
                candidates: [{ content: { parts: [{ text: 'healthy response' }] } }],
            ...
            expect(logWebFetchFallbackAttempt).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ reason: 'private_ip_skipped' }));
            expect(result.llmContent).toContain('healthy response');
            expect(result.llmContent).toContain('[Warning] The following URLs were skipped:');
        ...
        it('should log telemetry when falling back due to primary fetch failure', async () => {
            ...
            const invocation = tool.build(params);
            await invocation.execute({ abortSignal: new AbortController().signal });
            expect(logWebFetchFallbackAttempt).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ reason: 'primary_failed' }));
    ...
    describe('getPolicyUpdateOptions', () => {
        it('should return empty object for any outcome to allow global approval', () => {
            ...
            expect(invocation.getPolicyUpdateOptions(ToolConfirmationOutcome.ProceedAlways)).toEqual({});
            expect(invocation.getPolicyUpdateOptions(ToolConfirmationOutcome.ProceedAlwaysAndSave)).toEqual({});
        });
    });
    ...
    describe('execute (experimental)', () => {
        ...
        it('should perform direct fetch and return text for plain text content', async () => {
            ...
            expect(fetchUtils.fetchWithTimeout).toHaveBeenCalledWith('https://example.com/', expect.any(Number), expect.objectContaining({
        ...
        it('should use html-to-text and preserve links for HTML content', async () => {
            ...
            const invocation = tool.build(params);
            await invocation.execute({ abortSignal: new AbortController().signal });
            expect(convert).toHaveBeenCalledWith(content, expect.objectContaining({
                selectors: [
        ...
        it('should block private IP (experimental)', async () => {
            ...
            expect(result.llmContent).toContain('Error: Access to blocked or private host http://localhost/ is not allowed.');
            expect(result.error?.type).toBe(ToolErrorType.WEB_FETCH_PROCESSING_ERROR);
        });
        it('should bypass truncation if isContextManagementEnabled is true', async () => {
            ...
            expect(result.llmContent.length).toBe(300000); // No truncation
        });
        it('should truncate if isContextManagementEnabled is false', async () => {
            ...
            expect(result.llmContent.length).toBeLessThan(300000);
