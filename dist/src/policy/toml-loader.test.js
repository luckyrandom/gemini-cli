...
import { loadPoliciesFromToml, validateMcpPolicyToolNames, } from './toml-loader.js';
...
`);
    ...
    describe('Built-in Plan Mode Policy', () => {
        it('should allow MCP tools with readOnlyHint annotation in Plan Mode (ASK_USER, not DENY)', async () => {
            ...
            try {
                ...
                const denyResult = await engine.check({ name: 'mcp_github_create_issue' }, 'github', undefined);
                ...
                const denyResult2 = await engine.check({ name: 'mcp_github_delete_issue' }, 'github', { readOnlyHint: false });
                ...
                const qualifiedResult = await engine.check({ name: 'mcp_github_list_repos' }, undefined, { readOnlyHint: true });
        ...
        it('should override default subagent rules when in Plan Mode for unknown subagents', async () => {
            ...
            try {
                ...
                const checkResult = await engine.check({ name: 'invoke_agent', args: { agent_name: 'unknown_subagent' } }, undefined);
                expect(checkResult.decision, 'Unknown subagent should be DENIED in Plan Mode').toBe(PolicyDecision.DENY);
                ...
                expect(codebaseResult.decision, 'codebase_investigator should be ALLOWED in Plan Mode').toBe(PolicyDecision.ALLOW);
                const cliHelpResult = await engine.check({ name: 'invoke_agent', args: { agent_name: 'cli_help' } }, undefined);
                expect(cliHelpResult.decision, 'cli_help should be ALLOWED in Plan Mode').toBe(PolicyDecision.ALLOW);
                // 7. Verify MCP resource tools are ALLOWED
                const listMcpResult = await engine.check({ name: 'list_mcp_resources' }, undefined);
                expect(listMcpResult.decision, 'list_mcp_resources should be ALLOWED in Plan Mode').toBe(PolicyDecision.ALLOW);
                const readMcpResult = await engine.check({ name: 'read_mcp_resource', args: { uri: 'test://resource' } }, undefined);
                expect(readMcpResult.decision, 'read_mcp_resource should be ALLOWED in Plan Mode').toBe(PolicyDecision.ALLOW);
            }
            finally {
                await fs.rm(tempPolicyDir, { recursive: true, force: true });
            }
    ...
    describe('validateMcpPolicyToolNames', () => {
        it('should warn for MCP tool names that are likely typos', () => {
            const warnings = validateMcpPolicyToolNames('google-workspace', ['people.getMe', 'calendar.list', 'calendar.get'], [
                {
        ...
        it('should not warn for matching MCP tool names', () => {
            const warnings = validateMcpPolicyToolNames('google-workspace', ['people.getMe', 'calendar.list'], [
                {
        ...
        it('should not warn for wildcard MCP rules', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1', 'tool2'], [{ toolName: 'mcp_my-server_*', mcpName: 'my-server' }]);
            expect(warnings).toHaveLength(0);
        });
        it('should not warn for rules targeting other servers', () => {
            const warnings = validateMcpPolicyToolNames('server-a', ['tool1'], [{ toolName: 'mcp_server-b_toolx', mcpName: 'server-b' }]);
            expect(warnings).toHaveLength(0);
        });
        it('should not warn for tool names far from any discovered tool', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1', 'tool2'], [
                {
        ...
        it('should skip wildcard rules (matching all tools)', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1'], [{ toolName: '*', mcpName: 'my-server' }]);
            expect(warnings).toHaveLength(0);
        });
        it('should include source in warning when available', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1'], [
                {
