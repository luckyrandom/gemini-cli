...
import { ApprovalMode, PolicyDecision, ALWAYS_ALLOW_PRIORITY_OFFSET, } from './types.js';
 ...
 */
export function clearEmittedPolicyWarnings() {
...
export const ALWAYS_ALLOW_PRIORITY = WORKSPACE_POLICY_TIER + ALWAYS_ALLOW_PRIORITY_OFFSET;
/**
 * Returns the fractional priority of ALWAYS_ALLOW_PRIORITY scaled to 1000.
 */
export function getAlwaysAllowPriorityFraction() {
 ...
 */
export function getPolicyDirectories(defaultPoliciesDir, policyPaths, workspacePoliciesDir, adminPolicyPaths) {
    return [
 ...
 */
export function getPolicyTier(dir, context) {
...
export async function createPolicyEngineConfig(settings, approvalMode, defaultPoliciesDir, interactive = true) {
    ...
    if (adminPolicyPaths?.length) {
        try {
            ...
            if (!isNodeError(e) || e.code !== 'ENOENT') {
                debugLogger.warn(`Failed to check system policies in ${systemPoliciesDir}`, e);
            }
        }
    }
...
function findMatchingRule(rules, criteria) {
    return rules.find((r) => r.toolName === criteria.toolName &&
...
export function createPolicyUpdater(policyEngine, messageBus, storage) {
    ...
    messageBus.subscribe(MessageBusType.UPDATE_POLICY, async (message) => {
        ...
        if (message.commandPrefix) {
            ...
            const tier = message.persistScope === 'user'
                ? USER_POLICY_TIER
        ...
        if (message.persist) {
            persistenceQueue = persistenceQueue.then(async () => {
                try {
                    const policyFile = message.persistScope === 'workspace'
