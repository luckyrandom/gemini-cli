...
import { SHELL_TOOL_NAMES, initializeShellParsers, splitCommands, hasRedirection, extractStringFromParseEntry, } from '../utils/shell-utils.js';
import { parse as shellParse } from 'shell-quote';
import { isSubpath } from '../utils/paths.js';
import { PolicyDecision, ApprovalMode, ALWAYS_ALLOW_PRIORITY_FRACTION, } from './types.js';
...
import { MCP_TOOL_PREFIX, isMcpToolAnnotation, parseMcpToolName, formatMcpToolName, isMcpToolName, } from '../tools/mcp-tool.js';
...
export class PolicyEngine {
    ...
    isAlwaysAllowRule(rule) {
        return (rule.priority !== undefined &&
    ...
    async applyShellHeuristics(command, decision) {
        ...
        try {
            ...
            if (this.sandboxManager.isKnownSafeCommand(parsedArgs) &&
    ...
    async check(toolCall, serverName, toolAnnotations, subagent) {
        ...
        for (const rule of this.rules) {
            ...
            const match = toolCallsToTry.some((tc) => ruleMatches(rule, tc, stringifiedArgs, serverName, this.approvalMode, this.nonInteractive, toolAnnotations, subagent));
            if (match) {
        ...
        if (decision === undefined) {
            ...
            if (toolName && SHELL_TOOL_NAMES.includes(toolName)) {
                ...
                const shellResult = await this.checkShellCommand(toolName, command, heuristicDecision, serverName, shellDirPath, false, undefined, toolAnnotations, subagent);
                decision = shellResult.decision;
                matchedRule = shellResult.rule;
            }
            else {
        ...
        if (decision === PolicyDecision.ALLOW) {
            const args = toolCall.args;
            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
            const additionalPermissions = args?.[PARAM_ADDITIONAL_PERMISSIONS];
    ...
    getExcludedTools(toolMetadata, allToolNames) {
        ...
        for (const toolName of allToolNames) {
            ...
            for (const rule of this.rules) {
                ...
                const ruleWithoutArgs = { ...rule, argsPattern: undefined };
                const toolCall = { name: toolName, args: {} };
                const appliesToTool = ruleMatches(ruleWithoutArgs, toolCall, undefined, // stringifiedArgs
                serverName, this.approvalMode, this.nonInteractive, annotations);
                if (appliesToTool) {
                    if (rule.argsPattern) {
