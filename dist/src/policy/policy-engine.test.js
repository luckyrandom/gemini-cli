...
import { PolicyDecision, InProcessCheckerType, ApprovalMode, PRIORITY_SUBAGENT_TOOL, ALWAYS_ALLOW_PRIORITY_FRACTION, PRIORITY_YOLO_ALLOW_ALL, } from './types.js';
import { SafetyCheckDecision } from '../safety/protocol.js';
import { initializeShellParsers } from '../utils/shell-utils.js';
import { buildArgsPatterns } from './utils.js';
import { NoopSandboxManager, LocalSandboxManager, } from '../services/sandboxManager.js';
...
describe('PolicyEngine', () => {
    ...
    describe('check', () => {
        ...
        it('should respect tools approved by the SandboxManager', async () => {
            ...
            const { decision } = await engine.check({ name: 'run_shell_command', args: { command: 'npm install' } }, undefined);
            expect(decision).toBe(PolicyDecision.ALLOW);
        });
    ...
    describe('complex scenarios', () => {
        ...
        it('should NOT automatically DENY redirected shell commands in non-interactive mode if rules permit it', async () => {
            ...
            expect((await engine.check({ name: toolName, args: { command } }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
        });
    ...
    describe('checker matching logic', () => {
        ...
        it('should match global wildcard (*) for checkers', async () => {
            ...
            engine = new PolicyEngine({ rules, checkers: [globalChecker] }, mockCheckerRunner);
            vi.mocked(mockCheckerRunner.runChecker).mockResolvedValue({
                decision: SafetyCheckDecision.ALLOW,
            });
            await engine.check({ name: 'any_tool' }, undefined);
            expect(mockCheckerRunner.runChecker).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ name: 'global' }));
    ...
    describe('interactive matching', () => {
        ...
        it('should apply rules without interactive flag to both', async () => {
            ...
            expect((await engineInteractive.check({ name: 'my_tool', args: {} }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            expect((await engineNonInteractive.check({ name: 'my_tool', args: {} }, undefined)).decision).toBe(PolicyDecision.ALLOW);
        });
    });
    describe('additional_permissions', () => {
        ...
        it('should downgrade ALLOW to ASK_USER if a read path is outside workspace', async () => {
            ...
            expect((await engine.check(call, undefined)).decision).toBe(PolicyDecision.ASK_USER);
        });
