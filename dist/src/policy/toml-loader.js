 ...
 */
function validateToolName(name, ruleIndex) {
    if (name.includes('__')) {
    ...
    for (const p of policyPaths) {
        ...
        for (const { path: filePath, content: fileContent } of policyFiles) {
            ...
            try {
                ...
                for (let i = 0; i < tomlRules.length; i++) {
                    const rule = tomlRules[i];
                    const toolNamesRaw = Array.isArray(rule.toolName)
                        ? rule.toolName
                        : [rule.toolName];
                ...
                for (let i = 0; i < tomlCheckerRules.length; i++) {
                    ...
                    const checkerToolNamesRaw = Array.isArray(checker.toolName)
                        ? checker.toolName
                        : [checker.toolName];
                ...
                const parsedCheckers = (validationResult.data.safety_checker ?? [])
                    ...
                    return argsPatterns.flatMap((argsPattern) => {
                        const toolNames = Array.isArray(checker.toolName)
                            ? checker.toolName
                            : [checker.toolName];
                        return toolNames.map((toolName) => {
 ...
 */
export function validateMcpPolicyToolNames(serverName, discoveredToolNames, policyRules) {
    ...
    for (const rule of policyRules) {
        ...
        if (discoveredToolNames.length === 0)
            continue;
        const minDistance = Math.min(...discoveredToolNames.map((n) => levenshtein.get(toolPart, n)));
        if (minDistance > MAX_TYPO_DISTANCE)
            continue;
