...
describe('createPolicyEngineConfig', () => {
    ...
    beforeEach(async () => {
        ...
        vi.spyOn(Storage, 'getUserPoliciesDir').mockReturnValue(nodePath.resolve('/non/existent/user/policies'));
        vi.spyOn(Storage, 'getSystemPoliciesDir').mockReturnValue(nodePath.resolve('/non/existent/system/policies'));
    ...
    it('should filter out insecure system policy directories', async () => {
        ...
        expect(calledDirs).toContain(nodePath.resolve('/non/existent/user/policies'));
    ...
    it('should NOT filter out insecure supplemental admin policy directories', async () => {
        ...
        const loadPoliciesSpy = vi
            .spyOn(tomlLoader, 'loadPoliciesFromToml')
            .mockResolvedValue({ rules: [], checkers: [], errors: [] });
        await createPolicyEngineConfig({ adminPolicyPaths: [adminPolicyDir] }, ApprovalMode.DEFAULT, MOCK_DEFAULT_DIR);
        const calledDirs = loadPoliciesSpy.mock.calls[0][0];
        expect(calledDirs).toContain(adminPolicyDir);
        expect(calledDirs).toContain(nodePath.resolve('/non/existent/system/policies'));
        expect(calledDirs).toContain(nodePath.resolve('/non/existent/user/policies'));
        expect(calledDirs).toContain(MOCK_DEFAULT_DIR);
    });
    ...
    it('should allow tools from trusted MCP servers', async () => {
        ...
        const untrustedRule = config.rules?.find((r) => r.mcpName === 'untrusted-server' && r.decision === PolicyDecision.ALLOW);
    ...
    it('should handle multiple MCP server configurations together', async () => {
        ...
        const excludedRule = config.rules?.find((r) => r.mcpName === 'excluded-server' && r.decision === PolicyDecision.DENY);
    ...
    it('should handle MCP server allows and tool excludes', async () => {
        ...
        const serverAllowRule = config.rules?.find((r) => r.mcpName === 'my-server' && r.decision === PolicyDecision.ALLOW);
        const toolDenyRule = config.rules?.find((r) => r.toolName === 'mcp_my-server_dangerous-tool' &&
            r.decision === PolicyDecision.DENY);
    ...
    it('should handle MCP servers with undefined trust property', async () => {
        ...
        const noTrustRule = config.rules?.find((r) => r.mcpName === 'no-trust-property' &&
            r.decision === PolicyDecision.ALLOW);
        const explicitFalseRule = config.rules?.find((r) => r.mcpName === 'explicit-false' && r.decision === PolicyDecision.ALLOW);
...
`);
    ...
    it('should support mcpName in policy rules from TOML', async () => {
        mockPolicyFile(nodePath.join(MOCK_DEFAULT_DIR, 'mcp.toml'), `
  [[rule]]
  toolName = "my-tool"
  mcpName = "my-server"
  decision = "allow"
  ...
  `);
        const config = await createPolicyEngineConfig({}, ApprovalMode.DEFAULT, MOCK_DEFAULT_DIR);
        const rule = config.rules?.find((r) => r.toolName === 'mcp_my-server_my-tool' &&
            r.mcpName === 'my-server' &&
            r.decision === PolicyDecision.ALLOW);
        expect(rule).toBeDefined();
        expect(rule?.priority).toBeCloseTo(1.15, 5);
...
`);
    ...
    it('should deduplicate security warnings when called multiple times', async () => {
        ...
        const feedbackSpy = vi
            ...
            .mockImplementation(() => { });
        // First call
        await createPolicyEngineConfig({ adminPolicyPaths: [nodePath.resolve('/tmp/other/admin/policies')] }, ApprovalMode.DEFAULT);
