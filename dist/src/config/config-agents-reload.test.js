...
Test System Prompt`;
        ...
        vi.spyOn(config.getAcknowledgedAgentsService(), 'isAcknowledged').mockResolvedValue(true);
...
Test System Prompt`;
        ...
        await config.initialize();
        const agentRegistry = config.getAgentRegistry();
        expect(agentRegistry.getAllDefinitions().map((d) => d.name)).not.toContain(agentName);
...
//# sourceMappingURL=config-agents-reload.test.js.map