...
import { createScopedWorkspaceContext, runWithScopedWorkspaceContext, getWorkspaceContextOverride, } from './scoped-config.js';
...
describe('createScopedWorkspaceContext', () => {
    ...
    it('should throw when parent context has no directories', () => {
        const emptyCtx = { getDirectories: () => [] };
        expect(() => createScopedWorkspaceContext(emptyCtx, [extraDir])).toThrow('parent has no directories');
    });
    ...
    it('should throw when adding a filesystem root directory', () => {
        expect(() => createScopedWorkspaceContext(config.getWorkspaceContext(), ['/'])).toThrow('Cannot add filesystem root');
    });
});
...
//# sourceMappingURL=scoped-config.test.js.map