...
describe('Storage – additional helpers', () => {
    ...
    it('getProjectMemoryDir returns ~/.gemini/tmp/<identifier>/memory', async () => {
        await storage.initialize();
        const expected = path.join(os.homedir(), GEMINI_DIR, 'tmp', PROJECT_SLUG, 'memory');
    ...
    it('updates session-scoped directories when the sessionId changes', async () => {
        ...
        expect(storageWithSession.getProjectTempPlansDir()).toBe(path.join(tempDir, 'session-two', 'plans'));
        expect(storageWithSession.getProjectTempTrackerDir()).toBe(path.join(tempDir, 'session-two', 'tracker'));
        expect(storageWithSession.getProjectTempTasksDir()).toBe(path.join(tempDir, 'session-two', 'tasks'));
    });
