 ...
 */
export function resolveModel(requestedModel, useGemini3_1 = false, useGemini3_1FlashLite = false, useCustomToolModel = false, hasAccessToPreview = true, config) {
 ...
 * @returns The resolved concrete model name.
 */
export function resolveClassifierModel(requestedModel, modelAlias, useGemini3_1 = false, useGemini3_1FlashLite = false, useCustomToolModel = false, hasAccessToPreview = true, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
...
export function getDisplayString(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
 ...
 */
export function isPreviewModel(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
 ...
 */
export function isProModel(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
 ...
 */
export function isGemini3Model(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
        // Legacy behavior resolves the model first.
        const resolved = resolveModel(model);
        return (config.modelConfigService.getModelDefinition(resolved)?.family ===
            'gemini-3');
    }
    const resolved = resolveModel(model);
 ...
 */
export function isCustomModel(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
        const resolved = resolveModel(model, false, false, false, true, config);
        return (config.modelConfigService.getModelDefinition(resolved)?.tier ===
 ...
 */
export function isAutoModel(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
 ...
 */
export function supportsMultimodalFunctionResponse(model, config) {
    if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
        return (config.modelConfigService.getModelDefinition(model)?.features
 ...
 */
export function isActiveModel(model, useGemini3_1 = false, useGemini3_1FlashLite = false, useCustomToolModel = false) {
