...
import { INTEGRITY_FILENAME, INTEGRITY_KEY_FILENAME, KEYCHAIN_SERVICE_NAME, SECRET_KEY_ACCOUNT, } from '../constants.js';
...
import { IntegrityDataStatus, IntegrityStoreSchema, } from './integrityTypes.js';
    ...
    async getSecretKey() {
        ...
        if (await this.keychainService.isAvailable()) {
            try {
                ...
                debugLogger.warn(`Keychain access failed, falling back to file-based key: ${getErrorMessage(e)}`);
            }
        }
...
export class ExtensionIntegrityManager {
     ...
     */
    async verify(extensionName, metadata) {
        if (!metadata) {
        ...
        try {
            ...
            debugLogger.warn(`Error verifying integrity for "${extensionName}": ${getErrorMessage(e)}`);
            return IntegrityDataStatus.INVALID;
        }
    }
    /**
     ...
     */
    async store(extensionName, metadata) {
