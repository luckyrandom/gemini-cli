...
describe('ExtensionIntegrityManager', () => {
    ...
    describe('getSecretKey', () => {
        ...
        it('should generate and store file-based key if not exists', async () => {
            mockKeychainService.isAvailable.mockResolvedValue(false);
            vi.mocked(fs.promises.readFile).mockRejectedValueOnce(Object.assign(new Error(), { code: 'ENOENT' }));
            const key = await manager.getSecretKey();
            expect(key).toBeDefined();
            expect(fs.promises.writeFile).toHaveBeenCalledWith(path.join('/mock/home', '.gemini', 'integrity.key'), key, { mode: 0o600 });
        });
    });
    describe('store and verify', () => {
        ...
        beforeEach(() => {
            storedContent = '';
            const isIntegrityStore = (p) => typeof p === 'string' &&
        ...
        it('should throw error in store if existing store is modified', async () => {
            ...
            storedContent = JSON.stringify(data);
            await expect(manager.store('other-ext', metadata)).rejects.toThrow('Extension integrity store cannot be verified');
        });
        it('should throw error in store if store file is corrupted', async () => {
            storedContent = 'not-json';
            await expect(manager.store('other-ext', metadata)).rejects.toThrow('Failed to parse extension integrity store');
        });
    });
});
