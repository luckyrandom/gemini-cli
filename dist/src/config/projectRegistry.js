    ...
    async loadData() {
        try {
            ...
            if (error instanceof SyntaxError) {
                debugLogger.warn('Failed to load registry (JSON corrupted), resetting to empty: ', error);
        ...
        try {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    debugLogger.debug(`Rename failed with ${code}, retrying in ${delayMs}ms (attempt ${attempt + 1}/${maxRetries})...`);
    ...
    async ensureOwnershipMarkers(slug, projectPath) {
        ...
        for (const baseDir of this.baseDirs) {
            ...
            if (fs.existsSync(markerPath)) {
                ...
                // Collision!
                const error = Object.assign(new Error(`Slug ${slug} is already owned by ${owner}`), { code: 'EEXIST' });
                throw error;
            }
