...
describe('Server Config (config.ts)', () => {
    ...
    describe('refreshAuth', () => {
        ...
        it('should strip thoughts when switching from GenAI to Vertex', async () => {
            ...
            await config.refreshAuth(AuthType.USE_GEMINI);
            await config.refreshAuth(AuthType.LOGIN_WITH_GOOGLE);
            const loopContext = config;
            expect(loopContext.geminiClient.stripThoughtsFromHistory).toHaveBeenCalledWith();
        });
        ...
        it('should not strip thoughts when switching from Vertex to GenAI', async () => {
            ...
            await config.refreshAuth(AuthType.USE_GEMINI);
            const loopContext = config;
            expect(loopContext.geminiClient.stripThoughtsFromHistory).not.toHaveBeenCalledWith();
        });
    ...
    describe('createToolRegistry', () => {
        ...
        it('should register EnterPlanModeTool and ExitPlanModeTool when plan is enabled', async () => {
            ...
            await config.initialize();
            const registerToolMock = (await vi.importMock('../tools/tool-registry')).ToolRegistry.prototype.registerTool;
            const registeredTools = registerToolMock.mock.calls.map((call) => call[0].constructor.name);
    ...
    describe('Sandbox Configuration', () => {
        ...
        it('lazily resolves forbidden paths when first accessed', async () => {
            ...
            const resolved = await config.getSandboxForbiddenPaths();
...
describe('BaseLlmClient Lifecycle', () => {
    ...
    it('should throw an error if getBaseLlmClient is called before experiments have been fetched', () => {
        ...
        expect(() => config.getBaseLlmClient()).toThrow('BaseLlmClient not initialized. Ensure experiments have been fetched and configuration is ready.');
    });
