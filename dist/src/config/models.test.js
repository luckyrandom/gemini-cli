...
import { resolveModel, resolveClassifierModel, isGemini3Model, isGemini2Model, isCustomModel, supportsModernFeatures, isAutoModel, getDisplayString, DEFAULT_GEMINI_MODEL, PREVIEW_GEMINI_MODEL, DEFAULT_GEMINI_FLASH_MODEL, DEFAULT_GEMINI_FLASH_LITE_MODEL, supportsMultimodalFunctionResponse, GEMINI_MODEL_ALIAS_PRO, GEMINI_MODEL_ALIAS_FLASH, GEMINI_MODEL_ALIAS_FLASH_LITE, GEMINI_MODEL_ALIAS_AUTO, PREVIEW_GEMINI_FLASH_MODEL, PREVIEW_GEMINI_MODEL_AUTO, DEFAULT_GEMINI_MODEL_AUTO, isActiveModel, PREVIEW_GEMINI_3_1_MODEL, PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL, isPreviewModel, isProModel, } from './models.js';
        ...
        for (const model of modelsToTest) {
            for (const flags of flagCombos) {
                for (const hasAccess of [true, false]) {
                    ...
                    const legacy = resolveModel(model, flags.useGemini3_1, flags.useGemini3_1FlashLite, flags.useCustomToolModel, hasAccess, mockLegacyConfig);
                    const dynamic = resolveModel(model, flags.useGemini3_1, flags.useGemini3_1FlashLite, flags.useCustomToolModel, hasAccess, mockDynamicConfig);
        ...
        for (const hasAccess of [true, false]) {
            ...
            for (const tier of classifierTiers) {
                for (const anchor of anchorModels) {
                    for (const flags of flagCombos) {
                        const legacy = resolveClassifierModel(anchor, tier, flags.useGemini3_1, flags.useGemini3_1FlashLite, flags.useCustomToolModel, hasAccess, mockLegacyConfig);
                        const dynamic = resolveClassifierModel(anchor, tier, flags.useGemini3_1, flags.useGemini3_1FlashLite, flags.useCustomToolModel, hasAccess, mockDynamicConfig);
                        expect(dynamic).toBe(legacy);
                    }
                }
            }
...
describe('resolveModel', () => {
    ...
    describe('hasAccessToPreview logic', () => {
        it('should return default model when access to preview is false and preview model is requested', () => {
            expect(resolveModel(PREVIEW_GEMINI_MODEL, false, false, false, false)).toBe(DEFAULT_GEMINI_MODEL);
        });
        it('should return default flash model when access to preview is false and preview flash model is requested', () => {
            expect(resolveModel(PREVIEW_GEMINI_FLASH_MODEL, false, false, false, false)).toBe(DEFAULT_GEMINI_FLASH_MODEL);
        });
        it('should return default flash lite model when access to preview is false and preview flash lite model is requested', () => {
            expect(resolveModel(PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, false, false, false, false)).toBe(DEFAULT_GEMINI_FLASH_LITE_MODEL);
        });
        it('should return default model when access to preview is false and auto-gemini-3 is requested', () => {
            expect(resolveModel(PREVIEW_GEMINI_MODEL_AUTO, false, false, false, false)).toBe(DEFAULT_GEMINI_MODEL);
        });
        it('should return default model when access to preview is false and Gemini 3.1 is requested', () => {
            expect(resolveModel(PREVIEW_GEMINI_MODEL_AUTO, true, false, false, false)).toBe(DEFAULT_GEMINI_MODEL);
        });
        it('should still return default model when access to preview is false and auto-gemini-2.5 is requested', () => {
            expect(resolveModel(DEFAULT_GEMINI_MODEL_AUTO, false, false, false, false)).toBe(DEFAULT_GEMINI_MODEL);
        });
    });
});
...
describe('resolveClassifierModel', () => {
    ...
    it('should return Gemini 3.1 Pro Custom Tools when alias is pro, useGemini3_1 is true, and useCustomToolModel is true', () => {
        expect(resolveClassifierModel(PREVIEW_GEMINI_MODEL_AUTO, GEMINI_MODEL_ALIAS_PRO, true, false, true)).toBe(PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL);
    });
});
describe('isActiveModel', () => {
    ...
    it('should return true for PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL only when useGemini3_1FlashLite is true', () => {
        expect(isActiveModel(PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, false, true)).toBe(true);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, true, true)).toBe(true);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, true, false)).toBe(false);
    });
    it('should correctly filter Gemini 3.1 models based on useCustomToolModel when useGemini3_1 is true', () => {
        ...
        expect(isActiveModel(PREVIEW_GEMINI_3_1_MODEL, true, false, true)).toBe(false);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL, true, false, true)).toBe(true);
        // When custom tools are NOT preferred, custom tools 3.1 should be inactive
        expect(isActiveModel(PREVIEW_GEMINI_3_1_MODEL, true, false, false)).toBe(true);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL, true, false, false)).toBe(false);
    });
    it('should return false for Gemini 3.1 models when useGemini3_1 and useGemini3_1FlashLite are false', () => {
        expect(isActiveModel(PREVIEW_GEMINI_3_1_MODEL, false, false, true)).toBe(false);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_MODEL, false, false, false)).toBe(false);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL, false, false, true)).toBe(false);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL, false, false, false)).toBe(false);
        expect(isActiveModel(PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, false, false)).toBe(false);
    });
});
