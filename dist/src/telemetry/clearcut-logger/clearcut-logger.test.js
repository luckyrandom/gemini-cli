...
import { StartSessionEvent, UserPromptEvent, makeChatCompressionEvent, ModelRoutingEvent, ToolCallEvent, AgentStartEvent, AgentFinishEvent, WebFetchFallbackAttemptEvent, HookCallEvent, OnboardingStartEvent, OnboardingSuccessEvent, } from '../types.js';
...
import { CreditsUsedEvent, OverageOptionSelectedEvent, EmptyWalletMenuShownEvent, CreditPurchaseClickEvent, } from '../billingEvents.js';
expect.extend({
...
describe('ClearcutLogger', () => {
    ...
    describe('GITHUB_EVENT_NAME metadata', () => {
        ...
        it('does not include event name when GITHUB_EVENT_NAME is not set', () => {
            ...
            const event = logger?.createLogEvent(EventNames.API_ERROR, []);
            const hasEventName = event?.event_metadata[0].some((item) => item.gemini_cli_key === EventMetadataKey.GEMINI_CLI_GH_EVENT_NAME);
    ...
    describe('GH_PR_NUMBER metadata', () => {
        ...
        it('does not include PR number when GH_PR_NUMBER is not set', () => {
            ...
            const { logger } = setup({});
            const event = logger?.createLogEvent(EventNames.API_ERROR, []);
            const hasPRNumber = event?.event_metadata[0].some((item) => item.gemini_cli_key === EventMetadataKey.GEMINI_CLI_GH_PR_NUMBER);
    ...
    describe('GH_ISSUE_NUMBER metadata', () => {
        ...
        it('does not include issue number when GH_ISSUE_NUMBER is not set', () => {
            ...
            const { logger } = setup({});
            const event = logger?.createLogEvent(EventNames.API_ERROR, []);
            const hasIssueNumber = event?.event_metadata[0].some((item) => item.gemini_cli_key === EventMetadataKey.GEMINI_CLI_GH_ISSUE_NUMBER);
    ...
    describe('GH_CUSTOM_TRACKING_ID metadata', () => {
        ...
        it('does not include custom tracking ID when GH_CUSTOM_TRACKING_ID is not set', () => {
            ...
            const { logger } = setup({});
            const event = logger?.createLogEvent(EventNames.API_ERROR, []);
            const hasTrackingId = event?.event_metadata[0].some((item) => item.gemini_cli_key ===
    ...
    describe('logOverageOptionSelectedEvent', () => {
        it('logs an event with model, selected option, and credit balance', () => {
            const { logger } = setup();
            const event = new OverageOptionSelectedEvent('gemini-3-pro-preview', 'use_credits', 350);
