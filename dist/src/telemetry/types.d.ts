...
export declare class LoopDetectedEvent implements BaseTelemetryEvent {
    ...
    analysis?: string;
    confidence?: number;
    constructor(loop_type: LoopType, prompt_id: string, count: number, confirmed_by_model?: string, analysis?: string, confidence?: number);
    toOpenTelemetryAttributes(config: Config): LogAttributes;
    toLogBody(): string;
}
...
export declare const EVENT_NETWORK_RETRY_ATTEMPT = "gemini_cli.network_retry_attempt";
export declare class NetworkRetryAttemptEvent implements BaseTelemetryEvent {
    ...
    delay_ms: number;
    model: string;
    constructor(attempt: number, max_attempts: number, error_type: string, delay_ms: number, model: string);
    toOpenTelemetryAttributes(config: Config): LogAttributes;
    toLogBody(): string;
}
...
export type WebFetchFallbackReason = 'private_ip' | 'primary_failed' | 'private_ip_skipped';
