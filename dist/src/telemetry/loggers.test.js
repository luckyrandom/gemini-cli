...
import { logApiError, logApiRequest, logApiResponse, logCliConfiguration, logUserPrompt, logToolCall, logFlashFallback, logChatCompression, logMalformedJsonResponse, logInvalidChunk, logFileOperation, logRipgrepFallback, logToolOutputTruncated, logModelRouting, logExtensionEnable, logExtensionDisable, logExtensionInstallEvent, logExtensionUninstall, logAgentStart, logAgentFinish, logWebFetchFallbackAttempt, logNetworkRetryAttempt, logExtensionUpdateEvent, logHookCall, logOnboardingStart, logOnboardingSuccess, } from './loggers.js';
import { ToolCallDecision } from './tool-call-decision.js';
import { EVENT_API_ERROR, EVENT_API_REQUEST, EVENT_API_RESPONSE, EVENT_CLI_CONFIG, EVENT_TOOL_CALL, EVENT_USER_PROMPT, EVENT_FLASH_FALLBACK, EVENT_MALFORMED_JSON_RESPONSE, EVENT_FILE_OPERATION, EVENT_RIPGREP_FALLBACK, EVENT_MODEL_ROUTING, EVENT_EXTENSION_ENABLE, EVENT_EXTENSION_DISABLE, EVENT_EXTENSION_INSTALL, EVENT_EXTENSION_UNINSTALL, EVENT_TOOL_OUTPUT_TRUNCATED, EVENT_AGENT_START, EVENT_AGENT_FINISH, EVENT_WEB_FETCH_FALLBACK_ATTEMPT, EVENT_INVALID_CHUNK, EVENT_NETWORK_RETRY_ATTEMPT, EVENT_ONBOARDING_START, EVENT_ONBOARDING_SUCCESS, ApiErrorEvent, ApiRequestEvent, ApiResponseEvent, StartSessionEvent, ToolCallEvent, UserPromptEvent, FlashFallbackEvent, RipgrepFallbackEvent, MalformedJsonResponseEvent, InvalidChunkEvent, makeChatCompressionEvent, FileOperationEvent, ToolOutputTruncatedEvent, ModelRoutingEvent, ExtensionEnableEvent, ExtensionDisableEvent, ExtensionInstallEvent, ExtensionUninstallEvent, AgentStartEvent, AgentFinishEvent, WebFetchFallbackAttemptEvent, NetworkRetryAttemptEvent, ExtensionUpdateEvent, EVENT_EXTENSION_UPDATE, HookCallEvent, EVENT_HOOK_CALL, OnboardingStartEvent, OnboardingSuccessEvent, LlmRole, } from './types.js';
...
describe('loggers', () => {
    ...
    describe('logInvalidChunk', () => {
        ...
        it('logs the event to Clearcut and OTEL', () => {
            ...
            expect(ClearcutLogger.prototype.logInvalidChunkEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logNetworkRetryAttempt', () => {
        ...
        it('logs the network retry attempt event to Clearcut and OTEL', () => {
            const event = new NetworkRetryAttemptEvent(2, 5, 'Overloaded', 1000, 'test-model');
            logNetworkRetryAttempt(mockConfig, event);
            expect(ClearcutLogger.prototype.logNetworkRetryAttemptEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logOnboardingStart', () => {
        ...
        it('should log onboarding start event to Clearcut and OTEL, and record metrics', () => {
            ...
            expect(ClearcutLogger.prototype.logOnboardingStartEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logOnboardingSuccess', () => {
        ...
        it('should log onboarding success event to Clearcut and OTEL, and record metrics', () => {
            ...
            expect(ClearcutLogger.prototype.logOnboardingSuccessEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
            ...
            expect(metrics.recordOnboardingSuccess).toHaveBeenCalledWith(mockConfig, 'standard-tier', 100);
        });
    });
