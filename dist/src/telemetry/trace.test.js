...
import { GEN_AI_AGENT_DESCRIPTION, GEN_AI_AGENT_NAME, GEN_AI_CONVERSATION_ID, GEN_AI_INPUT_MESSAGES, GEN_AI_OPERATION_NAME, GEN_AI_OUTPUT_MESSAGES, GeminiCliOperation, SERVICE_DESCRIPTION, SERVICE_NAME, } from './constants.js';
...
describe('truncateForTelemetry', () => {
    ...
    it('should stringify and truncate objects if exceeding maxLength', () => {
        ...
        expect(result).toBe(stringified.substring(0, 10) +
...
describe('runInDevTraceSpan', () => {
    ...
    it('should start an active span', async () => {
        ...
        const result = await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, sessionId: 'test-session-id' }, fn);
    ...
    it('should set default attributes on the span metadata', async () => {
        await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, sessionId: 'test-session-id' }, async ({ metadata }) => {
    ...
    it('should handle errors in the wrapped function', async () => {
        ...
        await expect(runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, sessionId: 'test-session-id' }, async () => {
            throw error;
    ...
    it('should auto-wrap async iterators and end span when iterator completes', async () => {
        ...
        const resultStream = await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, sessionId: 'test-session-id' }, async () => testStream());
    ...
    it('should end span automatically on error in async iterators', async () => {
        ...
        const resultStream = await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, sessionId: 'test-session-id' }, async () => errorStream());
        await expect(async () => {
            for await (const _ of resultStream) {
                // iterate
            }
