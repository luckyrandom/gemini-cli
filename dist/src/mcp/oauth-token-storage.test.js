    ...
    describe('with encrypted flag false', () => {
        ...
        describe('getAllCredentials', () => {
            ...
            it('should load tokens from file successfully', async () => {
                ...
                expect(fs.readFile).toHaveBeenCalledWith(path.join('/mock/home', GEMINI_DIR, 'mcp-oauth-tokens.json'), 'utf-8');
            });
            it('should handle corrupted token file gracefully', async () => {
                vi.mocked(fs.readFile).mockResolvedValue('invalid json');
                const tokens = await tokenStorage.getAllCredentials();
                expect(tokens.size).toBe(0);
                expect(coreEvents.emitFeedback).toHaveBeenCalledWith('error', expect.stringContaining('Failed to load MCP OAuth tokens'), expect.any(Error));
            });
            it('should handle file read errors other than ENOENT', async () => {
                ...
                const tokens = await tokenStorage.getAllCredentials();
                expect(tokens.size).toBe(0);
                expect(coreEvents.emitFeedback).toHaveBeenCalledWith('error', 'Failed to load MCP OAuth tokens: Permission denied', error);
            });
        });
        describe('saveToken', () => {
            it('should save token with restricted permissions', async () => {
                ...
                expect(fs.mkdir).toHaveBeenCalledWith(path.join('/mock/home', GEMINI_DIR), { recursive: true });
                expect(fs.writeFile).toHaveBeenCalledWith(path.join('/mock/home', GEMINI_DIR, 'mcp-oauth-tokens.json'), expect.stringContaining('test-server'), { mode: 0o600 });
            });
            ...
            it('should handle write errors gracefully', async () => {
                ...
                expect(coreEvents.emitFeedback).toHaveBeenCalledWith('error', 'Failed to save MCP OAuth token: Disk full', writeError);
            });
        });
        ...
        describe('deleteCredentials', () => {
            ...
            it('should remove token file when no tokens remain', async () => {
                ...
                expect(fs.unlink).toHaveBeenCalledWith(path.join('/mock/home', GEMINI_DIR, 'mcp-oauth-tokens.json'));
            ...
            it('should handle file operation errors gracefully', async () => {
                ...
                await tokenStorage.deleteCredentials('test-server');
                expect(coreEvents.emitFeedback).toHaveBeenCalledWith('error', 'Failed to remove MCP OAuth token: Permission denied', unlinkError);
            });
        });
        ...
        describe('clearAll', () => {
            ...
            it('should handle other file errors gracefully', async () => {
                ...
                vi.mocked(fs.unlink).mockRejectedValue(unlinkError);
                await tokenStorage.clearAll();
                expect(coreEvents.emitFeedback).toHaveBeenCalledWith('error', 'Failed to clear MCP OAuth tokens: Permission denied', unlinkError);
            });
        });
    });
