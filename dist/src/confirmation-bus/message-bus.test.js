...
describe('MessageBus', () => {
    ...
    describe('publish', () => {
        ...
        it('should forward toolAnnotations to policyEngine.check', async () => {
            ...
            await messageBus.publish(request);
            expect(checkSpy).toHaveBeenCalledWith({ name: 'test-tool', args: {} }, 'test-server', annotations, undefined);
        });
    ...
    describe('derive', () => {
        it('should receive responses from parent bus on derived bus', async () => {
            ...
            const requestPromise = subagentBus.request(request, MessageBusType.TOOL_CONFIRMATION_RESPONSE, 2000);
            ...
            await expect(requestPromise).resolves.toEqual(expect.objectContaining({
                type: MessageBusType.TOOL_CONFIRMATION_RESPONSE,
                confirmed: true,
            }));
        });
        it('should correctly chain subagent names for nested subagents', async () => {
            ...
            const requestPromise = subagentBus2.request(request, MessageBusType.TOOL_CONFIRMATION_RESPONSE, 2000);
            await new Promise((resolve) => {
                messageBus.subscribe(MessageBusType.TOOL_CONFIRMATION_REQUEST, (msg) => {
