...
import { NoopSandboxManager, findSecretFiles, isSecretFile, resolveSandboxPaths, } from './sandboxManager.js';
...
describe('findSecretFiles', () => {
    ...
    it('should NOT find secret files recursively (shallow scan only)', async () => {
        ...
        expect(fsPromises.readdir).not.toHaveBeenCalledWith(path.join(workspace, 'packages'), expect.anything());
    });
});
describe('SandboxManager', () => {
    ...
    describe('resolveSandboxPaths', () => {
        it('should resolve allowed and forbidden paths', async () => {
            ...
            const result = await resolveSandboxPaths(options, req);
    ...
    describe('createSandboxManager', () => {
        it('should return NoopSandboxManager if sandboxing is disabled', () => {
            const manager = createSandboxManager({ enabled: false }, { workspace: path.resolve('/workspace') });
        ...
        ])('should return $expected.name if sandboxing is enabled and platform is $platform', ({ platform, expected }) => {
            vi.spyOn(os, 'platform').mockReturnValue(platform);
            const manager = createSandboxManager({ enabled: true }, { workspace: path.resolve('/workspace') });
