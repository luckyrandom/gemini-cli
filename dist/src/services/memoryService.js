...
function isExtractionRun(value) {
    return (typeof value === 'object' &&
        value !== null &&
        'runAt' in value &&
 ...
 */
export async function tryAcquireLock(lockPath, retries = 1) {
    const lockInfo = {
        pid: process.pid,
...
export async function releaseLock(lockPath) {
    try {
        ...
        debugLogger.warn(`[MemoryService] Failed to release lock: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Reads the extraction state file, or returns a default state.
 */
export async function readExtractionState(statePath) {
    try {
 ...
 */
export async function writeExtractionState(statePath, state) {
...
function shouldProcessConversation(parsed) {
    ...
    const userMessageCount = parsed.messages.filter((m) => m.type === 'user').length;
    ...
    for (const file of sessionFiles) {
        ...
        try {
            ...
            if (!isConversationRecord(parsed))
                continue;
            if (!shouldProcessConversation(parsed))
                continue;
 ...
 */
async function buildExistingSkillsSummary(skillsDir, config) {
    const sections = [];
 ...
 */
function buildAgentLoopContext(config) {
 ...
 */
export async function startMemoryService(config) {
    ...
    const handle = ExecutionLifecycleService.createExecution('', // no initial output
    () => abortController.abort(), // onKill
    'none', undefined, // no format injection
    'Skill extraction', 'silent');
    ...
    try {
        ...
        debugLogger.log(`[MemoryService] Session scan: ${totalInIndex} eligible session(s) found, ${newSessionIds.length} new`);
        ...
        const agentDefinition = SkillExtractionAgent(skillsDir, sessionIndex, existingSkillsSummary);
        ...
        if (skillsCreated.length > 0 || patchesCreatedThisRun.length > 0) {
            ...
            debugLogger.log(`[MemoryService] Completed in ${elapsed}s. ${completionParts.join('; ')} (processed ${newSessionIds.length} session(s))`);
            const feedbackParts = [];
            if (skillsCreated.length > 0) {
            ...
            debugLogger.log(`[MemoryService] Completed in ${elapsed}s. No new skills or patches created (processed ${newSessionIds.length} session(s))`);
        }
    }
    catch (error) {
        const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
        if (abortController.signal.aborted) {
            ...
            debugLogger.log(`[MemoryService] Failed after ${elapsed}s: ${error instanceof Error ? error.message : String(error)}`);
        }
        completionResult = {
