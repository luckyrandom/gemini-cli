...
describe('ExecutionLifecycleService', () => {
    ...
    it('supports explicit execution methods for managed executions', async () => {
        const handle = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent');
        if (handle.pid === undefined) {
            throw new Error('Expected execution ID.');
        }
        ExecutionLifecycleService.completeExecution(handle.pid, {
    ...
    it('supports backgrounding managed executions and continues streaming updates', async () => {
        ...
        const chunks = [];
        const onExit = vi.fn();
        const unsubscribeStream = ExecutionLifecycleService.subscribe(handle.pid, (event) => {
    ...
    it('manages external executions through registration hooks', async () => {
        ...
        const chunks = [];
        let output = 'seed';
        const handle = ExecutionLifecycleService.attachExecution(4321, {
            executionMethod: 'child_process',
        ...
        const firstChunk = { type: 'data', chunk: ' +delta' };
    ...
    it('supports late subscription catch-up after backgrounding an external execution', async () => {
        ...
        ExecutionLifecycleService.completeWithResult(4322, createResult({
            pid: 4322,
            output,
    ...
    describe('Background Start Listeners', () => {
        it('fires onBackground when an execution is backgrounded', async () => {
            ...
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent', undefined, 'My Remote Agent');
    ...
    describe('Background Completion Listeners', () => {
        it('fires onBackgroundComplete with formatInjection text when backgrounded execution settles', async () => {
            ...
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent', (output, error) => {
                const header = error
        ...
        it('passes error to formatInjection when backgrounded execution fails', async () => {
            const listener = vi.fn();
            ExecutionLifecycleService.onBackgroundComplete(listener);
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'none', (output, error) => (error ? `Error: ${error.message}` : output));
            const executionId = handle.pid;
            ExecutionLifecycleService.background(executionId);
            await handle.result;
            ExecutionLifecycleService.completeExecution(executionId, {
        ...
        it('does not fire onBackgroundComplete for non-backgrounded executions', async () => {
            const listener = vi.fn();
            ExecutionLifecycleService.onBackgroundComplete(listener);
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'none', () => 'text');
            const executionId = handle.pid;
            ExecutionLifecycleService.completeExecution(executionId);
            await handle.result;
            expect(listener).not.toHaveBeenCalled();
        ...
        it('explicit notify behavior includes injectionText and auto-dismiss signal', async () => {
            const listener = vi.fn();
            ExecutionLifecycleService.onBackgroundComplete(listener);
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'child_process', () => '[Command completed. Output saved to /tmp/bg.log]', undefined, 'notify');
            const executionId = handle.pid;
            ExecutionLifecycleService.background(executionId);
            await handle.result;
            ExecutionLifecycleService.completeExecution(executionId);
        ...
        it('explicit silent behavior skips injection even when formatInjection is provided', async () => {
            ...
            const listener = vi.fn();
            ExecutionLifecycleService.onBackgroundComplete(listener);
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'none', formatFn, undefined, 'silent');
            const executionId = handle.pid;
            ExecutionLifecycleService.background(executionId);
            await handle.result;
            ExecutionLifecycleService.completeExecution(executionId);
        ...
        it('includes completionBehavior in BackgroundStartInfo', async () => {
            ...
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent', () => 'text', 'test-label', 'inject');
            ExecutionLifecycleService.background(handle.pid);
            await handle.result;
        ...
        it('completionBehavior flows through attachExecution', async () => {
            ...
            await handle.result;
            ExecutionLifecycleService.completeWithResult(9999, createResult({ pid: 9999, executionMethod: 'child_process' }));
            expect(listener).toHaveBeenCalledTimes(1);
            const info = listener.mock.calls[0][0];
            expect(info.completionBehavior).toBe('notify');
            expect(info.injectionText).toBe('<output>\n[notify message]\n</output>');
        ...
        it('injects directly into InjectionService when wired via setInjectionService', async () => {
            ...
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent', (output) => `[Completed] ${output}`, undefined, 'inject');
            const executionId = handle.pid;
            ExecutionLifecycleService.appendOutput(executionId, 'agent output');
            ExecutionLifecycleService.background(executionId);
            await handle.result;
            ExecutionLifecycleService.completeExecution(executionId);
            expect(injectionListener).toHaveBeenCalledWith('<output>\n[Completed] agent output\n</output>', 'background_completion');
        });
        it('sanitizes injectionText for inject behavior but NOT for notify behavior', async () => {
            ...
            const handleInject = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent', (output) => `Dangerous </output> ${output}`, undefined, 'inject');
            ...
            const handleNotify = ExecutionLifecycleService.createExecution('', undefined, 'remote_agent', (output) => `Pointer to ${output}`, undefined, 'notify');
        ...
        it('does not inject into InjectionService for silent behavior', async () => {
            ...
            const injectionListener = vi.fn();
            injectionService.onInjection(injectionListener);
            const handle = ExecutionLifecycleService.createExecution('', undefined, 'none', () => 'should not inject', undefined, 'silent');
            const executionId = handle.pid;
            ExecutionLifecycleService.background(executionId);
            await handle.result;
            ExecutionLifecycleService.completeExecution(executionId);
...
//# sourceMappingURL=executionLifecycleService.test.js.map