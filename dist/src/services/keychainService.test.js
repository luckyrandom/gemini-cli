...
describe('KeychainService', () => {
    ...
    describe('isAvailable', () => {
        ...
        it('should return true (via fallback), log error, and emit telemetry indicating native is unavailable on failed functional test', async () => {
            ...
            expect(available).toBe(true);
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('encountered an error'), 'locked');
            expect(coreEvents.emitTelemetryKeychainAvailability).toHaveBeenCalledWith(expect.objectContaining({ available: false }));
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Using FileKeychain fallback'));
        ...
        it('should return true (via fallback), log validation error, and emit telemetry on module load failure', async () => {
            ...
            const available = await service.isAvailable();
            expect(available).toBe(true);
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('failed structural validation'), expect.objectContaining({ getPassword: expect.any(Array) }));
            expect(coreEvents.emitTelemetryKeychainAvailability).toHaveBeenCalledWith(expect.objectContaining({ available: false }));
            expect(FileKeychain).toHaveBeenCalled();
        ...
        it('should log failure if functional test cycle returns false, then fallback', async () => {
            ...
            const available = await service.isAvailable();
            expect(available).toBe(true);
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('functional verification failed'));
            expect(FileKeychain).toHaveBeenCalled();
        });
    ...
    describe('macOS Keychain Probing', () => {
        ...
        it('should skip functional test and fallback if security default-keychain fails', async () => {
            ...
            const available = await service.isAvailable();
            expect(available).toBe(true);
            expect(vi.mocked(spawnSync)).toHaveBeenCalledWith('security', ['default-keychain'], expect.any(Object));
            expect(mockKeytar.setPassword).not.toHaveBeenCalled();
            expect(FileKeychain).toHaveBeenCalled();
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('MacOS default keychain not found'));
        });
