...
const findLastContentLine = (buffer, startLine) => {
...
const getFullBufferText = (terminal, startLine = 0) => {
...
const writeBufferToLogStream = (terminal, stream, startLine = 0) => {
    const buffer = terminal.buffer.active;
    const lastContentLine = findLastContentLine(buffer, startLine);
    if (lastContentLine === -1 || lastContentLine < startLine)
        return startLine;
...
export class ShellExecutionService {
    ...
    static backgroundProcessHistory = new Map();
    static getLogDir() {
        return path.join(Storage.getGlobalTempDir(), 'background-processes');
    }
    static formatShellBackgroundCompletion(pid, behavior, output, error) {
    ...
    static async prepareExecution(commandToExecute, cwd, shellExecutionConfig, isInteractive) {
        ...
        const isStrictSandbox = isWindows &&
        ...
        try {
            ...
            const lifecycleHandle = child.pid
                ? ExecutionLifecycleService.attachExecution(child.pid, {
                    ...
                    formatInjection: (output, error) => ShellExecutionService.formatShellBackgroundCompletion(child.pid, shellExecutionConfig.backgroundCompletionBehavior || 'silent', output, error ?? undefined),
            ...
            const handleExit = (code, signal) => {
                ...
                const exitCode = code;
                const exitSignal = signal && os.constants.signals
    ...
    static async executeWithPty(commandToExecute, cwd, onOutputEvent, abortSignal, shellExecutionConfig, ptyInfo) {
        ...
        try {
            ...
            const result = ExecutionLifecycleService.attachExecution(ptyPid, {
                ...
                formatInjection: (output, error) => ShellExecutionService.formatShellBackgroundCompletion(ptyPid, shellExecutionConfig.backgroundCompletionBehavior || 'silent', output, error ?? undefined),
                completionBehavior: shellExecutionConfig.backgroundCompletionBehavior || 'silent',
            }).result;
    ...
    static background(pid, sessionId, command) {
        ...
        const history = this.backgroundProcessHistory.get(resolvedSessionId) ??
            new Map();
