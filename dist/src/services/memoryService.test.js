...
describe('memoryService', () => {
    ...
    describe('readExtractionState / writeExtractionState', () => {
        ...
        it('writes state atomically via temp file + rename', async () => {
            const { writeExtractionState, readExtractionState } = await import('./memoryService.js');
            const statePath = path.join(tmpDir, '.extraction-state.json');
            const state = {
                runs: [
                    {
    ...
    describe('startMemoryService', () => {
        ...
        it('releases lock on error', async () => {
            const { startMemoryService } = await import('./memoryService.js');
            const { LocalAgentExecutor } = await import('../agents/local-executor.js');
            const { ExecutionLifecycleService } = await import('./executionLifecycleService.js');
            ...
            vi.mocked(LocalAgentExecutor.create).mockRejectedValueOnce(new Error('Agent creation failed'));
            const mockConfig = {
                storage: {
                    getProjectMemoryDir: vi.fn().mockReturnValue(memoryDir),
                    getProjectMemoryTempDir: vi.fn().mockReturnValue(memoryDir),
            ...
            expect(ExecutionLifecycleService.completeExecution).toHaveBeenCalledWith(42, expect.objectContaining({
    ...
    describe('buildSessionIndex', () => {
        ...
        it('includes file path and summary in each line', async () => {
            ...
            await fs.writeFile(path.join(chatsDir, fileName), JSON.stringify(conversation));
            const result = await buildSessionIndex(chatsDir, { runs: [] });
        ...
        it('filters out sessions with fewer than 10 user messages', async () => {
            ...
            await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-short001.json`), JSON.stringify(conversation));
            const result = await buildSessionIndex(chatsDir, { runs: [] });
            expect(result.sessionIndex).toBe('');
            expect(result.newSessionIds).toEqual([]);
        });
        it('caps at MAX_SESSION_INDEX_SIZE (50)', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-0${i + 1}T00-00-cap${paddedIndex}.json`), JSON.stringify(conversation));
            }
            const result = await buildSessionIndex(chatsDir, { runs: [] });
        ...
        it('returns newSessionIds only for unprocessed sessions', async () => {
            ...
            await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-proc0001.json`), JSON.stringify(oldConv));
            const newConv = createConversation({
                sessionId: 'fresh-one',
                summary: 'New',
                messageCount: 20,
            });
            await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-02T00-00-fres0001.json`), JSON.stringify(newConv));
            const state = {
                runs: [
                    {
                        runAt: '2025-01-01T00:00:00Z',
