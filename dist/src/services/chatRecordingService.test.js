...
describe('ChatRecordingService', () => {
    ...
    describe('deleteSession', () => {
        ...
        it('should delete subagent files and their logs when parent is deleted', async () => {
            ...
            const parentToolOutputDir = path.join(toolOutputsDir, `session-${parentSessionId}`);
            fs.mkdirSync(parentToolOutputDir, { recursive: true });
            const subagentToolOutputDir = path.join(toolOutputsDir, `session-${subagentSessionId}`);
        ...
        it('should not throw if session file does not exist', async () => {
            await expect(chatRecordingService.deleteSession('non-existent')).resolves.not.toThrow();
        });
    });
