...
import {} from './fileSystemService.js';
import {} from './sandboxManager.js';
 ...
 */
export class SandboxedFileSystemService {
    sandboxManager;
    cwd;
    ...
    sanitizeAndValidatePath(filePath) {
        ...
        for (const allowed of allowedPaths) {
            ...
            if (isSubpath(resolvedAllowed, resolvedPath) ||
            ...
            return await new Promise((resolve, reject) => {
                ...
                child.on('close', (code) => {
                    if (code === 0) {
                        ...
                        const isEnoent = error.toLowerCase().includes('no such file or directory') ||
                        ...
                        const err = new Error(`Sandbox Error: read_file failed for '${filePath}'. Exit code ${code}. ${error ? 'Details: ' + error : ''}`);
                        if (isEnoent) {
                ...
                child.on('error', (err) => {
                    reject(new Error(`Sandbox Error: Failed to spawn read_file for '${filePath}': ${err.message}`));
                });
            });
        }
        finally {
        ...
        try {
            return await new Promise((resolve, reject) => {
                ...
                child.stdin?.on('error', (err) => {
                    ...
                    debugLogger.error(`Sandbox Error: stdin error for '${filePath}': ${err instanceof Error ? err.message : String(err)}`);
                });
                ...
                child.on('close', (code) => {
                    if (code === 0) {
                        ...
                        reject(new Error(`Sandbox Error: write_file failed for '${filePath}'. Exit code ${code}. ${error ? 'Details: ' + error : ''}`));
                    }
                });
                child.on('error', (err) => {
                    reject(new Error(`Sandbox Error: Failed to spawn write_file for '${filePath}': ${err.message}`));
                });
            });
        }
        finally {
...
//# sourceMappingURL=sandboxedFileSystemService.js.map