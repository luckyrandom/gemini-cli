...
describe('ShellExecutionService', () => {
    ...
    describe('Backgrounding', () => {
        ...
        beforeEach(async () => {
            ...
            mockBgChildProcess = new EventEmitter();
            mockBgChildProcess.stdout = new EventEmitter();
            mockBgChildProcess.stderr = new EventEmitter();
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.backgroundLogStreams.clear();
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.activePtys.clear();
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.activeChildProcesses.clear();
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.backgroundProcessHistory.clear();
        });
        afterEach(() => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.backgroundLogStreams.clear();
        ...
        it('should move a running pty process to the background and start logging', async () => {
            const abortController = new AbortController();
            const handle = await ShellExecutionService.execute('long-running-pty', '/', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
            ...
            expect(mockMkdirSync).toHaveBeenCalledWith(expect.stringContaining('background-processes'), { recursive: true, mode: 0o700 });
            // Verify initial output was written
            expect(mockWriteStream.write.mock.calls.some((call) => call[0].includes('initial pty output'))).toBe(true);
        ...
        it('should continue logging after backgrounding for child_process', async () => {
            ...
            const abortController = new AbortController();
            const handle = await ShellExecutionService.execute('long-running-cp', '/', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
            ...
            expect(mockWriteStream.write.mock.calls.some((call) => call[0].includes('initial cp output'))).toBe(true);
            // Subsequent output
        ...
        it('should log a warning if background log setup fails', async () => {
            const abortController = new AbortController();
            const handle = await ShellExecutionService.execute('failing-log-setup', '/', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
            ...
            const result = await handle.result;
            expect(result.backgrounded).toBe(true);
            expect(mockDebugLogger.warn).toHaveBeenCalledWith('Failed to setup background logging:', error);
            await ShellExecutionService.kill(handle.pid);
        });
        it('should track background process history', async () => {
            await simulateExecution('history-test-cmd', async (pty) => {
        ...
        it('should evict oldest process history when exceeding max size', () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.backgroundProcessHistory.set('default', history);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.activeChildProcesses.set(101, {
                process: {},
                state: { output: '' },
                command: 'cmd-101',
                sessionId: 'default',
        ...
        it('should throw error if sessionId is missing for background operations', () => {
            expect(() => ShellExecutionService.background(102)).toThrow('Session ID is required for background operations');
        });
        it('should throw error if sessionId is missing for listBackgroundProcesses', () => {
            expect(() => 
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ShellExecutionService.listBackgroundProcesses(undefined)).toThrow('Session ID is required');
        });
    });
    ...
    describe('Resource Management', () => {
        ...
        it('should destroy the PTY when kill() is called', async () => {
            ...
            const abortController = new AbortController();
            await ShellExecutionService.execute('long-running', '/test/dir', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
            await new Promise((resolve) => process.nextTick(resolve));
            ...
            const storedDestroySpy = vi.spyOn(activePty.ptyProcess, 'destroy');
        ...
        it('should destroy the PTY when an exception occurs after spawn in executeWithPty', async () => {
            ...
            const handle = await ShellExecutionService.execute('will-fail-after-spawn', '/test/dir', onOutputEventMock, new AbortController().signal, true, shellExecutionConfig);
            const result = await handle.result;
            expect(result.exitCode).toBe(1);
...
describe('ShellExecutionService child_process fallback', () => {
    ...
    describe('Aborting Commands', () => {
        describe.each([
            ...
            it('should abort a running process and set the aborted flag', async () => {
                ...
                const { result } = await simulateExecution('sleep 10', async (cp, abortController) => {
                    abortController.abort();
    ...
    describe('Binary Output', () => {
        ...
        it('should correctly measure sniffedBytes with >20 small chunks to prevent OOM (regression #22170)', async () => {
            ...
            const lastCallBuffer = mockIsBinary.mock.calls[mockIsBinary.mock.calls.length - 1][0];
...
describe('ShellExecutionService environment variables', () => {
    ...
    it('should call prepareCommand on sandboxManager when provided', async () => {
        ...
        const handle = await ShellExecutionService.execute('ls', '/test/cwd', () => { }, new AbortController().signal, false, // child_process path
        configWithSandbox);
        expect(mockResolveExecutable).toHaveBeenCalledWith(expect.any(String));
        expect(mockSandboxManager.prepareCommand).toHaveBeenCalledWith(expect.objectContaining({
            command: '/bin/bash/resolved',
            args: expect.arrayContaining([expect.stringContaining('ls')]),
            cwd: '/test/cwd',
        }));
        expect(mockCpSpawn).toHaveBeenCalledWith('sandboxed-bash', ['-c', 'ls'], expect.objectContaining({
    ...
    it('should include headless git and gh environment variables in non-interactive mode and append git config safely', async () => {
        ...
        const { ShellExecutionService } = await import('./shellExecutionService.js');
        mockGetPty.mockResolvedValue(null); // Force child_process fallback
        await ShellExecutionService.execute('test-cp-headless-git', '/', vi.fn(), new AbortController().signal, false, // non-interactive
        shellExecutionConfig);
        expect(mockCpSpawn).toHaveBeenCalled();
        const cpEnv = mockCpSpawn.mock.calls[0][2].env;
    ...
    it('should NOT include headless git and gh environment variables in interactive fallback mode', async () => {
        ...
        const { ShellExecutionService } = await import('./shellExecutionService.js');
        mockGetPty.mockResolvedValue(null); // Force child_process fallback
        await ShellExecutionService.execute('test-cp-interactive-fallback', '/', vi.fn(), new AbortController().signal, true, // isInteractive (shouldUseNodePty)
        shellExecutionConfig);
        expect(mockCpSpawn).toHaveBeenCalled();
        const cpEnv = mockCpSpawn.mock.calls[0][2].env;
