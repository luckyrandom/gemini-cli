 ...
 */
import { createDefaultPolicy, createSingleModelChain, getModelPolicyChain, getFlashLitePolicyChain, SILENT_ACTIONS, } from './policyCatalog.js';
...
export function resolvePolicyChain(config, preferredModel, wrapsAround = false) {
    ...
    const resolvedModel = resolveModel(modelFromConfig, useGemini31, useGemini31FlashLite, useCustomToolModel, hasAccessToPreview, config);
    ...
    if (config.getExperimentalDynamicModelConfiguration?.() === true) {
        ...
        if (resolvedModel === DEFAULT_GEMINI_FLASH_LITE_MODEL) {
            ...
            if (!chain) {
                const previewEnabled = hasAccessToPreview &&
        ...
        if (resolvedModel === DEFAULT_GEMINI_FLASH_LITE_MODEL) {
            ...
            if (hasAccessToPreview) {
                const previewEnabled = isGemini3Model(resolvedModel, config) ||
                    preferredModel === PREVIEW_GEMINI_MODEL_AUTO ||
                    configuredModel === PREVIEW_GEMINI_MODEL_AUTO;
 ...
 */
function applyDynamicSlicing(chain, resolvedModel, wrapsAround) {
