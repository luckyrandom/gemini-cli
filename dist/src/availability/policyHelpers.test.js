...
describe('policyHelpers', () => {
    ...
    describe('resolvePolicyChain behavior is identical between dynamic and legacy implementations', () => {
        ...
        testCases.forEach(({ name, model, useGemini31, hasAccess, authType, wrapsAround }) => {
            it(`achieves parity for: ${name}`, () => {
                const createBaseConfig = (dynamic) => createMockConfig({
                ...
                const legacyChain = resolvePolicyChain(createBaseConfig(false), model, wrapsAround);
                const dynamicChain = resolvePolicyChain(createBaseConfig(true), model, wrapsAround);
    ...
    describe('applyModelSelection', () => {
        ...
        it('does not consume sticky attempt if consumeAttempt is false', () => {
            ...
            const result = applyModelSelection(config, { model: 'gemini-pro', isChatModel: true }, {
