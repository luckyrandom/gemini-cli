...
import { DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_FLASH_MODEL, PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_FLASH_MODEL, DEFAULT_GEMINI_MODEL_AUTO, PREVIEW_GEMINI_MODEL_AUTO, GEMINI_MODEL_ALIAS_AUTO, } from '../../config/models.js';
...
describe('ApprovalModeStrategy', () => {
    ...
    it('should route to FLASH model if an approved plan exists (Gemini 2.5)', async () => {
        ...
        expect(decision).toEqual({
            ...
            metadata: {
                source: 'approval-mode',
                latencyMs: expect.any(Number),
                reasoning: 'Routing to Flash model because an approved plan exists at /path/to/plan.md.',
            },
        });
    });
    ...
    it('should route to Preview models when using "auto" alias', async () => {
        ...
        vi.mocked(mockConfig.getApprovalMode).mockReturnValue(ApprovalMode.DEFAULT);
        vi.mocked(mockConfig.getApprovedPlanPath).mockReturnValue('/path/to/plan.md');
        const implementationDecision = await strategy.route(mockContext, mockConfig, mockBaseLlmClient);
...
//# sourceMappingURL=approvalModeStrategy.test.js.map