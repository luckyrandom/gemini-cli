...
export class NumericalClassifierStrategy {
    ...
    async route(context, config, baseLlmClient, _localLiteRtLmClient) {
        ...
        try {
            ...
            const { threshold, groupLabel, modelAlias } = await this.getRoutingDecision(score, config);
            ...
            const selectedModel = resolveClassifierModel(model, modelAlias, useGemini3_1, useGemini3_1FlashLite, useCustomToolModel, config.getHasAccessToPreviewModel?.() ?? true, config);
    ...
    async getRoutingDecision(score, config) {
