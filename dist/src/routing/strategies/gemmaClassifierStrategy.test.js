...
describe('GemmaClassifierStrategy', () => {
    ...
    it('should throw an error if the model is not gemma3-1b-gpu-custom', async () => {
        ...
        await expect(strategy.route(mockContext, mockConfig, mockBaseLlmClient, mockLocalLiteRtLmClient)).rejects.toThrow('Only gemma3-1b-gpu-custom has been tested');
    });
    it('should call generateJson with the correct parameters', async () => {
        ...
        expect(mockGenerateJson).toHaveBeenCalledWith(expect.any(Array), expect.any(String), expect.any(String), expect.any(AbortSignal));
    });
    ...
    it('should filter out tool-related history before sending to classifier', async () => {
        ...
        mockGenerateJson.mockResolvedValue(mockApiResponse);
        await strategy.route(mockContext, mockConfig, mockBaseLlmClient, mockLocalLiteRtLmClient);
        const calls = mockGenerateJson.mock.calls;
...
//# sourceMappingURL=gemmaClassifierStrategy.test.js.map