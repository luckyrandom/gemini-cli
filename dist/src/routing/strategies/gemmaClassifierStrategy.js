...
export class GemmaClassifierStrategy {
    ...
    flattenChatHistory(turns) {
        ...
        const userRequest = lastTurn?.parts
            ?.map((part) => part.text)
            .filter(Boolean)
            .join('\n\n') ?? '';
...
`;
    ...
    async route(context, config, _baseLlmClient, client) {
        ...
        try {
            ...
            const jsonResponse = await client.generateJson(singleMessageHistory, LITERT_GEMMA_CLASSIFIER_SYSTEM_PROMPT, LITERT_GEMMA_CLASSIFIER_REMINDER, context.signal);
            const routerResponse = ClassifierResponseSchema.parse(jsonResponse);
            const reasoning = routerResponse.reasoning;
            const latencyMs = Date.now() - startTime;
            const selectedModel = resolveClassifierModel(context.requestedModel ?? config.getModel(), routerResponse.model_choice);
            return {
                model: selectedModel,
                metadata: {
...
//# sourceMappingURL=gemmaClassifierStrategy.js.map