...
export class ClassifierStrategy {
    ...
    async route(context, config, baseLlmClient, _localLiteRtLmClient) {
        ...
        try {
            ...
            const selectedModel = resolveClassifierModel(model, routerResponse.model_choice, useGemini3_1, useGemini3_1FlashLite, useCustomToolModel, config.getHasAccessToPreviewModel?.() ?? true, config);
            return {
                model: selectedModel,
                metadata: {
                    source: 'Classifier',
