...
import { UserTierId, ActionStatus, InitiationMethod, } from './types.js';
...
describe('CodeAssistServer', () => {
    ...
    it('should call getCodeAssistGlobalUserSetting endpoint', async () => {
        ...
        expect(requestGetSpy).toHaveBeenCalledWith('getCodeAssistGlobalUserSetting');
        expect(response).toEqual(mockResponse);
    });
    it('should call setCodeAssistGlobalUserSetting endpoint', async () => {
        ...
        expect(requestPostSpy).toHaveBeenCalledWith('setCodeAssistGlobalUserSetting', req);
        expect(response).toEqual(mockResponse);
    });
    it('should call loadCodeAssist during refreshAvailableCredits', async () => {
        ...
        vi.spyOn(server, 'loadCodeAssist').mockResolvedValue(mockResponse);
        ...
        expect(server.paidTier?.availableCredits).toEqual(mockPaidTier.availableCredits);
    });
...
`;
        ...
        it('should log InvalidChunkEvent when SSE chunk is not valid JSON', async () => {
            ...
            const mockRequest = vi.fn();
            const client = { request: mockRequest };
            const server = new CodeAssistServer(client, 'test-project', {}, 'test-session', UserTierId.FREE, undefined, undefined, config);
            const { Readable } = await import('node:stream');
            const mockStream = new Readable({
                read() { },
            });
            ...
            expect(results).toHaveLength(0);
            expect(logInvalidChunk).toHaveBeenCalledWith(config, expect.objectContaining({
        ...
        it('should safely process random response streams in generateContentStream (consumed/remaining credits)', async () => {
            ...
            const testServer = new CodeAssistServer(client, 'test-project', {}, 'test-session', UserTierId.FREE, undefined, { id: 'test-tier', name: 'tier', availableCredits: [] });
            const { Readable } = await import('node:stream');
            ...
            mockRequest.mockResolvedValueOnce({ data: mockStream });
            vi.spyOn(testServer, 'recordCodeAssistMetrics').mockResolvedValue(undefined);
            const stream = await testServer.generateContentStream({ model: 'test-model', contents: [] }, 'user-prompt-id', LlmRole.MAIN);
            for await (const _ of stream) {
                // Drain stream
            }
            // Should not crash
        ...
        it('should be resilient to metadata-only chunks without candidates in generateContentStream', async () => {
            const { mockRequest, client } = createTestServer();
            const testServer = new CodeAssistServer(client, 'test-project', {}, 'test-session', UserTierId.FREE);
            const { Readable } = await import('node:stream');
            ...
            expect(results[2].candidates?.[0].content?.parts?.[0].text).toBe(' World');
        });
    });
});
