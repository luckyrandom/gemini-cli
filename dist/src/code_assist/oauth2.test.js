...
import { FatalCancellationError, FatalAuthenticationError, } from '../utils/errors.js';
...
describe('oauth2', () => {
    ...
    describe('with encrypted flag false', () => {
        ...
        it('should throw FatalAuthenticationError in non-interactive session when manual auth is required', async () => {
            ...
            await expect(getOauthClient(AuthType.LOGIN_WITH_GOOGLE, mockConfigNonInteractive)).rejects.toThrow(FatalAuthenticationError);
            await expect(getOauthClient(AuthType.LOGIN_WITH_GOOGLE, mockConfigNonInteractive)).rejects.toThrow('Manual authorization is required but the current session is non-interactive.');
        });
        ...
        describe('error handling', () => {
            ...
            it('should clear the authorization timeout immediately upon successful web login to prevent memory leaks', async () => {
                ...
                const mockOAuth2Client = {
                    ...
                    setCredentials: vi.fn().mockImplementation(function (creds) {
