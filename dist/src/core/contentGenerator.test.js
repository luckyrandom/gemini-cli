...
describe('createContentGenerator', () => {
        ...
        expect(GoogleGenAI).toHaveBeenCalledWith({
            ...
            httpOptions: expect.objectContaining({
                headers: expect.objectContaining({
                    'User-Agent': expect.stringMatching(/GeminiCLI\/1\.2\.3\/gemini-pro \(.*; .*; terminal\)/),
                }),
            }),
        });
    ...
    it('should use standard User-Agent for a2a-server running outside VS Code', async () => {
        ...
        vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator);
        await createContentGenerator({ apiKey: 'test-api-key', authType: AuthType.USE_GEMINI }, mockConfig, undefined);
        expect(GoogleGenAI).toHaveBeenCalledWith(expect.objectContaining({
            httpOptions: expect.objectContaining({
                headers: expect.objectContaining({
                    'User-Agent': expect.stringMatching(/GeminiCLI-a2a-server\/1\.2\.3\/gemini-pro \(.*; .*; terminal\)/),
                }),
            }),
        }));
    });
    it('should include unified User-Agent for a2a-server (VS Code Agent Mode)', async () => {
        ...
        expect(GoogleGenAI).toHaveBeenCalledWith(expect.objectContaining({
            httpOptions: expect.objectContaining({
                headers: expect.objectContaining({
                    'User-Agent': expect.stringMatching(/CloudCodeVSCode\/1\.2\.3 \(aidev_client; os_type=.*; os_version=.*; arch=.*; host_path=VSCode\/1\.85\.0; proxy_client=geminicli\)/),
                }),
            }),
        }));
    });
    it('should include clientName prefix in User-Agent when specified (non-VSCode)', async () => {
        ...
        expect(GoogleGenAI).toHaveBeenCalledWith(expect.objectContaining({
            httpOptions: expect.objectContaining({
                headers: expect.objectContaining({
                    'User-Agent': expect.stringMatching(/GeminiCLI-my-client\/1\.2\.3\/gemini-pro \(.*; .*; terminal\)/),
                }),
            }),
        }));
    });
    ...
    it('should prefer an explicit baseUrl over GOOGLE_GEMINI_BASE_URL', async () => {
        ...
        vi.stubEnv('GEMINI_API_KEY', 'test-api-key');
        const config = await createContentGeneratorConfig(mockConfig, AuthType.USE_GEMINI, undefined, 'https://explicit.test.local');
        await createContentGenerator(config, mockConfig);
        expect(GoogleGenAI).toHaveBeenCalledWith(expect.objectContaining({
            httpOptions: expect.objectContaining({
    ...
    it('should reject invalid custom baseUrl values', async () => {
        await expect(createContentGenerator({
            ...
            authType: AuthType.USE_GEMINI,
            baseUrl: 'not-a-url',
        }, mockConfig)).rejects.toThrow('Invalid custom base URL: not-a-url');
    });
    it('should reject non-https remote custom baseUrl values', async () => {
        ...
        }, mockConfig)).rejects.toThrow('Custom base URL must use HTTPS unless it is localhost.');
    });
});
describe('createContentGeneratorConfig', () => {
    ...
    it('should configure for GATEWAY using dummy placeholder if GEMINI_API_KEY is set', async () => {
        vi.stubEnv('GEMINI_API_KEY', 'env-gemini-key');
        const config = await createContentGeneratorConfig(mockConfig, AuthType.GATEWAY);
