...
describe('LoggingContentGenerator', () => {
    ...
    describe('generateContent', () => {
        ...
        describe('Gaxios error parsing', () => {
            it('should parse raw ASCII buffer strings in Gaxios errors', async () => {
                ...
                await expect(loggingContentGenerator.generateContent(req, 'prompt-123', LlmRole.MAIN)).rejects.toSatisfy((error) => {
                    const gError = error;
        ...
        it('should NOT log error on AbortError (user cancellation)', async () => {
            ...
            await expect(loggingContentGenerator.generateContent(req, userPromptId, LlmRole.MAIN)).rejects.toThrow(abortError);
    ...
    describe('generateContentStream', () => {
        ...
        it('should NOT log error on AbortError during connection phase', async () => {
            ...
            await expect(loggingContentGenerator.generateContentStream(req, userPromptId, LlmRole.MAIN)).rejects.toThrow(abortError);
            expect(logApiError).not.toHaveBeenCalled();
        });
        it('should NOT log error on AbortError during stream iteration', async () => {
            ...
            vi.mocked(wrapped.generateContentStream).mockResolvedValue(createAbortingGenerator());
            const stream = await loggingContentGenerator.generateContentStream(req, userPromptId, LlmRole.MAIN);
            await expect(async () => {
                for await (const _ of stream) {
                    // consume stream
