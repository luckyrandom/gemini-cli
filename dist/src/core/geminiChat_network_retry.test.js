...
// Mock loggers
const { mockLogContentRetry, mockLogContentRetryFailure, mockLogNetworkRetryAttempt, } = vi.hoisted(() => ({
...
describe('GeminiChat Network Retries', () => {
    ...
    it('should retry when a 503 ApiError occurs during stream iteration', async () => {
        ...
        expect(mockLogNetworkRetryAttempt).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
    ...
    it('should retry on SSL error during connection phase (ERR_SSL_SSLV3_ALERT_BAD_RECORD_MAC)', async () => {
        ...
        const { retryWithBackoff } = await vi.importActual('../utils/retry.js');
    ...
    it('should retry on OpenSSL 3.x SSL error during stream iteration (ERR_SSL_SSL/TLS_ALERT_BAD_RECORD_MAC)', async () => {
        ...
        const stream = await chat.sendMessageStream({ model: 'test-model' }, 'test message', 'prompt-id-ssl3-mid-stream', new AbortController().signal, LlmRole.MAIN);
        const events = [];
        for await (const event of stream) {
            events.push(event);
        }
