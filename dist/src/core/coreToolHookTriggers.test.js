...
describe('executeToolWithHooks', () => {
    ...
    it('should prioritize continue: false over decision: block in BeforeTool', async () => {
        ...
        const result = await executeToolWithHooks(invocation, 'test_tool', abortSignal, mockTool, undefined, undefined, mockConfig);
    ...
    it('should apply modified tool input from BeforeTool hook', async () => {
        ...
        const result = await executeToolWithHooks(invocation, toolName, abortSignal, mockTool, undefined, undefined, mockConfig);
    ...
    it('should pass execution ID callback through for non-shell invocations', async () => {
        ...
        vi.mocked(mockHookSystem.fireAfterToolEvent).mockResolvedValue(undefined);
        await executeToolWithHooks(invocation, 'test_tool', abortSignal, mockTool, undefined, { setExecutionIdCallback }, mockConfig);
