...
import { logContentRetry, logContentRetryFailure, logNetworkRetryAttempt, } from '../telemetry/loggers.js';
...
const MID_STREAM_RETRY_OPTIONS = {
...
export class GeminiChat {
    ...
    async makeApiCallAndProcessStream(modelConfigKey, requestContents, prompt_id, abortSignal, role) {
        ...
        const onPersistent429Callback = async (authType, error) => handleFallback(this.context.config, lastModelToUse, authType, error);
    ...
    recordCompletedToolCalls(model, toolCalls) {
        const toolCallRecords = toolCalls.map((call) => {
            ...
            return {
                ...
                agentId: typeof call.response?.data?.['agentId'] === 'string'
                    ? call.response.data['agentId']
                    : undefined,
                resultDisplay,
                description: 'invocation' in call ? call.invocation?.getDescription() : undefined,
            };
        });
