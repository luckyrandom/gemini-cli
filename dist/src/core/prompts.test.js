...
describe('Core System Prompt (prompts.ts)', () => {
    ...
    it('should include the TASK MANAGEMENT PROTOCOL in legacy prompt when task tracker is enabled', () => {
        ...
        expect(prompt).toContain('**PLAN MODE INTEGRATION**: If an approved plan exists, you MUST use the `tracker_create_task` tool');
        expect(prompt).toMatchSnapshot();
    });
    it('should include the TASK MANAGEMENT PROTOCOL when task tracker is enabled', () => {
        ...
        const prompt = getCoreSystemPrompt(mockConfig);
        expect(prompt).toContain('# TASK MANAGEMENT PROTOCOL');
        expect(prompt).toContain('**PLAN MODE INTEGRATION**: If an approved plan exists, you MUST use the `tracker_create_task` tool to decompose it into discrete tasks before writing any code');
        expect(prompt).toMatchSnapshot();
    });
    ...
    it.each([
        [true, true],
        [false, false],
    ])('should handle CodebaseInvestigator (enabled=%s)', (enableCodebaseInvestigator, expectCodebaseInvestigator) => {
    ...
    describe('ApprovalMode in System Prompt', () => {
        ...
        it('should include PLAN mode instructions', () => {
            ...
            expect(prompt).not.toContain('`mcp_nonreadonly-server_write_data` (nonreadonly-server)');
            expect(prompt).toMatchSnapshot();
        });
