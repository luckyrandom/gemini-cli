...
export class GeminiClient {
    ...
    async fireAfterAgentHookSafe(currentRequest, prompt_id, turn, stopHookActive = false) {
        ...
        const hookOutput = await this.config
            .getHookSystem()
            ?.fireAfterAgentEvent(partToString(finalRequest), finalResponseText, stopHookActive);
        return hookOutput;
    }
    ...
    async *processTurn(request, signal, prompt_id, boundedTurns, isInvalidStreamRetry, displayContent) {
        ...
        if (loopResult.count > 1) {
            ...
            return yield* this._recoverFromLoop(loopResult, signal, prompt_id, boundedTurns, isInvalidStreamRetry, displayContent);
        }
    ...
    async *sendMessageStream(request, signal, prompt_id, turns = MAX_TURNS, isInvalidStreamRetry = false, displayContent, stopHookActive = false) {
        if (!isInvalidStreamRetry) {
        ...
        try {
            ...
            if (hooksEnabled && messageBus) {
                ...
                if (afterAgentOutput?.isBlockingDecision()) {
                    ...
                    turn = yield* this.sendMessageStream(continueRequest, signal, prompt_id, boundedTurns - 1, false, displayContent, true);
                }
            }
        }
        catch (error) {
    ...
    async tryCompressChat(prompt_id, force = false, abortSignal) {
        ...
        const { newHistory, info } = await this.compressionService.compress(this.getChat(), prompt_id, force, model, this.config, this.hasFailedCompressionAttempt, abortSignal);
    ...
    _recoverFromLoop(loopResult, signal, prompt_id, boundedTurns, isInvalidStreamRetry, displayContent, controllerToAbort) {
        ...
        return this.sendMessageStream(feedback, signal, prompt_id, boundedTurns - 1, isInvalidStreamRetry, displayContent);
    }
}
