...
vi.mock('node:fs', () => {
    const fsModule = {
        ...
        appendFileSync: vi.fn((path, data) => {
...
describe('Gemini Client (client.ts)', () => {
    ...
    beforeEach(async () => {
        ...
        mockConfig.toolRegistry = mockToolRegistry;
    ...
    describe('sendMessageStream', () => {
        it('calls AgentHistoryProvider.manageHistory when history truncation is enabled', async () => {
            ...
            client.agentHistoryProvider, 'manageHistory')
                .mockResolvedValue([
            ...
            // Assert
            expect(manageHistorySpy).toHaveBeenCalledWith(expect.any(Array), expect.any(AbortSignal));
        });
        ...
        it('yields UserCancelled when processTurn throws AbortError', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-id-abort-error');
            const events = await fromAsync(stream);
...
\`\`\`
      `.trim();
        ...
        it('should use getSystemInstructionMemory for system instruction when JIT is enabled', async () => {
            ...
            vi.mocked(mockConfig.getSystemInstructionMemory).mockReturnValue('Global JIT Memory');
        ...
        it('should use getSystemInstructionMemory for system instruction when JIT is disabled', async () => {
            ...
            vi.mocked(mockConfig.getSystemInstructionMemory).mockReturnValue('Legacy Memory');
            const { getCoreSystemPrompt } = await import('./prompts.js');
            const mockGetCoreSystemPrompt = vi.mocked(getCoreSystemPrompt);
            client.updateSystemInstruction();
        ...
        it('should update system instruction when MemoryChanged event is emitted', async () => {
            vi.mocked(mockConfig.getSystemInstructionMemory).mockReturnValue('Updated Memory');
            const { getCoreSystemPrompt } = await import('./prompts.js');
            const mockGetCoreSystemPrompt = vi.mocked(getCoreSystemPrompt);
            mockGetCoreSystemPrompt.mockClear();
            coreEvents.emit(CoreEvent.MemoryChanged, { fileCount: 2 });
            expect(mockGetCoreSystemPrompt).toHaveBeenCalledWith(mockConfig, 'Updated Memory');
        });
        it('should recursively call sendMessageStream with "Please continue." when InvalidStream event is received for Gemini 2 models', async () => {
            ...
            expect(mockTurnRunFn).toHaveBeenNthCalledWith(1, { model: 'gemini-2.0-flash', isChatModel: true }, initialRequest, expect.any(AbortSignal), undefined);
            // Second call with "Please continue."
            expect(mockTurnRunFn).toHaveBeenNthCalledWith(2, { model: 'gemini-2.0-flash', isChatModel: true }, [{ text: 'System: Please continue.' }], expect.any(AbortSignal), undefined);
        });
        ...
        it('should stop recursing after one retry when InvalidStream events are repeatedly received', async () => {
            ...
            expect(events
                ...
                .map((e) => e.value)).toEqual(['gemini-2.0-flash']);
            // Verify that turn.run was called twice
            expect(mockTurnRunFn).toHaveBeenCalledTimes(2);
        });
        ...
        describe('Loop Recovery (Two-Strike)', () => {
            ...
            it('should trigger recovery (Strike 1) and continue', async () => {
                ...
                // Act
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-id-loop-1');
                const events = [];
                for await (const event of stream) {
                    events.push(event);
                }
                ...
                expect(recoveryCall[0][0].text).toContain('System: Potential loop detected');
            ...
            it('should terminate (Strike 2) after recovery fails', async () => {
                ...
                // Act
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-id-loop-2');
                const events = [];
                for await (const event of stream) {
                    events.push(event);
                }
            ...
            it('should respect boundedTurns during recovery', async () => {
                ...
                // Act
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-id-loop-3', 1);
                const events = [];
                for await (const event of stream) {
                    events.push(event);
                }
            ...
            it('should suppress LoopDetected event on Strike 1', async () => {
                ...
                // Act
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-telemetry');
                const events = [];
                for await (const event of stream) {
                    events.push(event);
                }
            ...
            it('should escalate Strike 2 even if loop type changes', async () => {
                ...
                // Act
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-escalate');
                const events = [];
                for await (const event of stream) {
                    events.push(event);
                }
            ...
            it('should reset loop detector on new prompt', async () => {
                ...
                // Act
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-id-new');
                for await (const _ of stream) {
                    // Consume stream
                }
                // Assert
                expect(client['loopDetector'].reset).toHaveBeenCalledWith('prompt-id-new', 'Hi');
            });
        });
    });
    describe('generateContent', () => {
        ...
        describe('Hook System', () => {
            ...
            it('should fire BeforeAgent and AfterAgent exactly once for a simple turn', async () => {
                ...
                expect(mockHookSystem.fireAfterAgentEvent).toHaveBeenCalledWith(partToString(request), 'Hook Response', false);
                // Map should be empty
            ...
            it('should yield AgentExecutionBlocked and recurse in AfterAgent when hook returns decision: block', async () => {
                ...
                expect(mockHookSystem.fireAfterAgentEvent).toHaveBeenNthCalledWith(1, expect.any(String), expect.any(String), false);
                expect(mockHookSystem.fireAfterAgentEvent).toHaveBeenNthCalledWith(2, expect.any(String), expect.any(String), true);
            });
